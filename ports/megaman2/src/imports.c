...
/* DIAGNOSTICO: loga quem chama abort/raise (LR do jogo) p/ symbolicar o assert. */
extern volatile uintptr_t g_load_base;
static void b_abort(void) {
  void *lr = __builtin_return_address(0);
  fprintf(stderr, "[ABORT] caller=%p game_off=0x%lx\n", lr,
          g_load_base ? (unsigned long)((uintptr_t)lr - g_load_base) : 0);
  fflush(stderr); _exit(66);
}
static int b_raise(int sig) {
  void *lr = __builtin_return_address(0);
  fprintf(stderr, "[RAISE] sig=%d caller=%p game_off=0x%lx\n", sig, lr,
          g_load_base ? (unsigned long)((uintptr_t)lr - g_load_base) : 0);
