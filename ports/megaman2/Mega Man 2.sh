#!/bin/bash
# PORTMASTER: megaman2, Mega Man 2.sh
# Mega Man 2 Mobile (Cocos2d-x 3.9, Capcom) -> Mali-450 armhf so-loader.
...
GAMEDIR="/$directory/ports/megaman2"
cd "$GAMEDIR" || exit 1

> "$GAMEDIR/log.txt" && exec > >(tee "$GAMEDIR/log.txt") 2>&1

$ESUDO chmod +x "$GAMEDIR/megaman2" 2>/dev/null
...
$GPTOKEYB "megaman2" &
pm_platform_helper "$GAMEDIR/megaman2" >/dev/null
