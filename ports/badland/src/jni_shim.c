/*
 * jni_shim.c -- JNIEnv falso para BADLAND/Cocos2d-x.
 *
...
static char g_writable_path[512] = "/storage/roms/ports/badland/userdata/";
...
static void *string_for_tag(void *methodID) {
  ...
  if (methodID == &g_method_tags[MID_PACKAGE_NAME]) return jni_make_string("com.frogmind.badland");
  ...
  if (methodID == &g_method_tags[MID_ASSETS_PATH]) return jni_make_string("/storage/roms/ports/badland/assets/");
...
static jint jni_CallStaticIntMethod(void *env, void *clazz, void *mid, ...) {
  ...
  if (mid == &g_method_tags[MID_LOCATION_CODE]) {
    const char *e = getenv("BADLAND_LOC");
...
void jni_shim_init(void **out_vm, void **out_env) {
  g_jni_log = getenv("BADLAND_JNILOG") != NULL;
