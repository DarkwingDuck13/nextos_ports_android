      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_JUMP_SLOT: {
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (!found) {
              /* Fallback: resolve from the host global symbol scope (libc,
                 libGLESv2, libEGL, libm, libstdc++ are all NEEDED by the
                 loader and live in the global scope). The explicit table
                 above always takes priority, so shimmed symbols still win;
                 this only catches plain libc/GL/EGL passthroughs that the
                 Fusion engine needs but libTTapp did not. */
