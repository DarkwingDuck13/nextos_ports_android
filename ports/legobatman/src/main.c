...
enum {
  ...
  // START/L3 baseline (TFA original); swap if the pad reports them inverted.

  // Batman 3 (like Ninjago) swaps START/L3 vs TFA: START must send 0x0200
  // (engine reads it as Start/Pause), L3 sends 0x0800.
...
int main(int argc, char *argv[]) {
  ...
  debugPrintf("=== LEGO Batman 3: Beyond Gotham -> Mali-450 (Linux/SDL) ===\n");
  ...
  if (stat(GAMEDATA_DIR "/lego_black_android_1.fib", &st) < 0)
    debugPrintf("WARN: %s/lego_black_android_1.fib not found (assets may be missing)\n", GAMEDATA_DIR);
  ...
  while (running) {
    ...
    // Answer the frontend's modal dialog (see jni_fake ShowAlertDialog). In
    // Batman 3 the "Can't access Google Play / create a new save?" prompt IS the
    // New Game confirmation: pressing New Game raises it, "Yes" (neg, code 1)
    // creates the save and ENTERS the level, "No" (0) cancels back to the menu
    // (this was the "blocked" buzzer the pad hit). Always answer 1 so New Game
    // proceeds; the game auto-loads any existing save at boot for continuity.
    if (g_alert_pending && g.setAlertResponse) {
      g.setAlertResponse(fake_env, FUSION_OBJ, ALERT_RESP_NEW);
      debugPrintf("alert: fed response %d (new-game/proceed)\n", ALERT_RESP_NEW);
      g_alert_pending = 0;
    }
