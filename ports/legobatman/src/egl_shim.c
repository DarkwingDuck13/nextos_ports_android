...
/* GLSurfaceView model: the engine never calls eglSwapBuffers and expects the
   "render thread" (us, on the main loop) to own the window context and present
   each frame. Bind the bootstrap context on the calling thread so nativeRender
   draws to the window, and present with a direct swap. */
void egl_shim_main_make_current(void) {
  if (!egl_window || !egl_share_root) return;
  /* Create a dedicated context for the render (main) thread that SHARES with
     the bootstrap root. We must NOT keep egl_share_root itself current here,
     because the engine's loader thread needs to bind it briefly during its own
     eglCreateContext (doing so would otherwise give EGL_BAD_ACCESS). */
  pthread_mutex_lock(&egl_context_create_mutex);
  SDL_GL_SetAttribute(SDL_GL_SHARE_WITH_CURRENT_CONTEXT, 1);
  SDL_GL_MakeCurrent(egl_window, egl_share_root);
  SDL_GLContext mc = SDL_GL_CreateContext(egl_window);
  SDL_GL_SetAttribute(SDL_GL_SHARE_WITH_CURRENT_CONTEXT, 0);
  if (mc && SDL_GL_MakeCurrent(egl_window, mc) == 0) {
    has_real_gl = 1;
    debugPrintf("egl_shim: main render context created+bound [tid=%lx]\n",
                (unsigned long)pthread_self());
  } else {
    SDL_GL_MakeCurrent(egl_window, NULL);
    debugPrintf("egl_shim: main render context FAILED: %s\n", SDL_GetError());
  }
  pthread_mutex_unlock(&egl_context_create_mutex);
}

/* On this Mali fbdev stack SDL renders into the lower half of the 1280x1440
   framebuffer but never pans the display there (the engine never calls
   eglSwapBuffers). Pan the visible region down once so the rendered frames
   are shown. SCREEN_HEIGHT is the per-buffer height. */
/* SDL_GL_SwapWindow does not pan the fbdev on this Mali stack and the yoffset
   keeps getting reset to 0 (the off-screen half), so the rendered frame (which
   lands in the lower half) is never shown. Keep an fd open and re-assert the
   pan to the lower half on every present. */
static int g_fb_fd = -1;
static void egl_shim_pan_to_render_buffer(void) {
  if (g_fb_fd < 0) g_fb_fd = open("/dev/fb0", O_RDWR);
  if (g_fb_fd < 0) return;
  struct fb_var_screeninfo vi;
  if (ioctl(g_fb_fd, FBIOGET_VSCREENINFO, &vi) == 0 &&
      vi.yres_virtual >= (uint32_t)(SCREEN_HEIGHT * 2) &&
      vi.yoffset != (uint32_t)SCREEN_HEIGHT) {
...
int egl_shim_ensure_current(void) {
  ...
  /* If this thread never made a guest context current yet, the engine is
     issuing GL calls (e.g. early glCreateShader) before its eglMakeCurrent.
     Fall back to the bootstrap share-root context: every guest context is
     created sharing it, so resources made here are visible afterwards.
     Without this the first shaders are created with no context and come back
     as 0 -> empty programs -> broken/white rendering. */
  if (!ctx || !ctx->sdl_context) {
