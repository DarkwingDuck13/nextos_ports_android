...
static void my_vpad_update(void *self){
  ...
  if(g_pad_mask && p_GDM_getInstance){        /* OR o mask do gamepad físico (pós-update) */
    char *gdm=(char*)p_GDM_getInstance();
    if(gdm){ *(unsigned*)(gdm+4)|=g_pad_mask; *(unsigned*)(gdm+8)|=g_pad_mask; }
    if(getenv("MM_INPUTLOG")){
      static long lc=0; if((lc++ % 8)==0)
        fprintf(stderr,"[gdm] mask=0x%x +4=0x%x +8=0x%x\n", g_pad_mask,
                *(unsigned*)(gdm+4), *(unsigned*)(gdm+8));
    }
  }
}
...
#define VP_DPAD_CX  140.0f   /* centro real do dpad na tela (medido: 140,540 r~135) */
#define VP_DPAD_CY  540.0f
#define VP_DPAD_OFF 95.0f    /* toque na PONTA do dpad (senão só esquerda pega) */
...
/* Toque direcional no dpad da tela (id=TID_DPAD). O bitmask GDM move o Mega Man
   no chão, mas o PULO lê a direção horizontal da POSIÇÃO do toque no dpad virtual
   (Lib_MultiInput), não do bitmask -> sem toque, pulo sai reto pra cima. Mantemos
   um toque persistente na direção segurada (begin uma vez, move enquanto muda,
   end ao soltar). Coexiste com o toque do pulo (id=TID_JUMP) via multitouch. */
static int g_dpad_touch=0, g_dpad_dir=0;
static void update_dpad_touch(int rt,int lf,int up,int dn){
  int dir = (rt?1:0)|(lf?2:0)|(up?4:0)|(dn?8:0);
  if(!dir){ if(g_dpad_touch){ mm_tend(TID_DPAD,VP_DPAD_CX,VP_DPAD_CY); g_dpad_touch=0; g_dpad_dir=0; } return; }
  float x=VP_DPAD_CX + (rt?VP_DPAD_OFF:lf?-VP_DPAD_OFF:0.0f);
  float y=VP_DPAD_CY + (dn?VP_DPAD_OFF:up?-VP_DPAD_OFF:0.0f);
  if(!g_dpad_touch){ mm_tbegin(TID_DPAD,x,y); g_dpad_touch=1; g_dpad_dir=dir; }
  else if(dir!=g_dpad_dir){ /* mudou de direção: re-inicia o toque na nova posição */
    mm_tend(TID_DPAD,x,y); mm_tbegin(TID_DPAD,x,y); g_dpad_dir=dir;
  } else mm_tmove(TID_DPAD,x,y);
}
static void update_dpad(void){
  int rt=dp_rt||stk_x>0, lf=dp_lf||stk_x<0, up=dp_up||stk_y<0, dn=dp_dn||stk_y>0;
  unsigned m=0;
  if(rt) m|=0x2000;
  if(lf) m|=0x1000;
  if(up) m|=0x4000;
  if(dn) m|=0x8000;
  g_pad_mask = m;
  /* dpad-touch LIGADO por padrão: o pulo lê a direção do toque no dpad, não do
     bitmask (sem isso, andar+pular = pulo reto pra cima). Opt-out: MM_NODPADTOUCH. */
  if(!getenv("MM_NODPADTOUCH")) update_dpad_touch(rt,lf,up,dn);
}
...
static void *hook_thumb_call(const char *sym, void *repl){
  ...
  /* GUARD: o trampolim assume prólogo [instr0][instr1][BL getInstance] e realoca
     instr0/instr1. Se instr1 for LDR literal PC-relative (0x48xx-0x4Fxx) ou o[2]
     não for um BL (0xF000-0xF7FF), realocar corromperia o r7/pc (crash MM5). Nesse
     caso NÃO instala o hook (retorna NULL) -> direção cai no dpad-touch. */
  if((o[1]&0xF800)==0x4800 || (o[2]&0xF800)!=0xF000){
    fprintf(stderr,"hook %s: prologo incompativel (o1=0x%04x o2=0x%04x) -> pulando (usa dpad-touch)\n",
            sym,o[1],o[2]);
    return NULL;
  }
...
int main(int argc, char *argv[]) {
  ...
  void *apk = jni_make_string("/storage/roms/ports/megaman5/base.apk");
  ...
  while (running) {
    while (SDL_PollEvent(&e)) {
      switch (e.type) {
        ...
        /* RECONEXÃO: adaptadores baratos caem/reconectam (ou o user troca o
        ...
        case SDL_CONTROLLERAXISMOTION: {
          /* HISTERESE: engata em ±16000 mas só solta abaixo de ±6000. Adaptadores
             PS2 baratos dão uma queda momentânea no analógico ao apertar um botão;
             sem histerese isso zerava a direção e o pulo saía reto pra cima. */
          const int HI=16000, LO=6000;
          int v=e.caxis.value, av=v<0?-v:v;
          if (e.caxis.axis==SDL_CONTROLLER_AXIS_LEFTX) {
            if (av>HI) stk_x = v>0?1:-1; else if (av<LO) stk_x = 0; /* zona morta: mantém */
            update_dpad();
          } else if (e.caxis.axis==SDL_CONTROLLER_AXIS_LEFTY) {
            if (av>HI) stk_y = v>0?1:-1; else if (av<LO) stk_y = 0;
            update_dpad();
          }
          break;
        }
