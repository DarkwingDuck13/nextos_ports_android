...
/* 🔑 s8/s9: o handshake de startup do gfxdeviceworker/Choreographer trava aqui — a cond_wait é
   chamada de libunity+0x45232c (func 0x4521e8, queue-pop), cslot = &(obj->cond) = obj+0x88.
   Capturamos o OBJETO (cslot-0x88) p/ o choreo poder dirigir o produtor nativo (vtable[4]=0x452498)
   e destravar a fila. g_choreo_syncobj = objeto da fila do handshake. */
volatile void *g_choreo_syncobj;       /* objeto da fila do handshake (main) */
...
/* TER_FEEDLOG: loga quando o PRODUTOR/FEED nativo da fila do frame roda no NOSSO loader.
   Offsets confirmados na bancada: feed per-frame broadcast=lu+0x45265c; produtor startup
   broadcast=lu+0x452520. Se NUNCA logar = nosso doFrame não dispara o feed nativo (raiz da trava). */
static void feedlog(int bcast) {
  if (!getenv("TER_FEEDLOG") || !g_pt_ub) return;
  unsigned long ra = (unsigned long)__builtin_return_address(1);
  unsigned long off = ra - g_pt_ub;
  if (off >= 0x2000000) return;
  if (off == 0x45265c || off == 0x452520 || off == 0x4525e8 || (off >= 0x452490 && off <= 0x452700) || (off >= 0xbff000 && off <= 0xbff100)) {
    static int seen[32]; static int ns; int dup=0;
    for (int i=0;i<ns;i++) if (seen[i]==(int)off){dup=1;break;}
    if (!dup && ns<32){ seen[ns++]=(int)off;
      fprintf(stderr, "[FEED] %s nativo rodou! caller=lu+0x%lx tid=%d\n", bcast?"broadcast":"signal", off, (int)syscall(SYS_gettid)); fsync(2); }
  }
}
int pthread_cond_signal_fake(void **slot) { ct_signal(slot, 0); feedlog(0); return pthread_cond_signal(cond_get(slot)); }
int pthread_cond_broadcast_fake(void **slot) { ct_signal(slot, 1); feedlog(1); return pthread_cond_broadcast(cond_get(slot)); }
...
int pthread_cond_wait_fake(void **cslot, void **mslot) {
  /* captura o objeto do handshake do gfxdeviceworker (RA == libunity+0x45232c).
     obj = cslot - 0x88. 1ª thread a entrar aqui = main (queue dela); 2ª = gfxworker. */
  if (g_pt_ub) {
    unsigned long ra = (unsigned long)__builtin_return_address(0);
    unsigned long off = ra - g_pt_ub;
    /* TER_HSLOG: loga offsets distintos de TODA cond_wait vinda da libunity (acha o offset real
       do handshake do gfxdeviceworker que muda conforme text_base). */
    if (getenv("TER_HSLOG") && off < 0x2000000) {
      static unsigned long seen[64]; static int ns;
      int dup = 0; for (int i = 0; i < ns; i++) if (seen[i] == off) { dup = 1; break; }
      if (!dup && ns < 64) { seen[ns++] = off;
        void *obj = (void *)((char *)cslot - 0x88); void *vt = *(void **)obj;
        fprintf(stderr, "[HSLOG] cond_wait caller=libunity+0x%lx obj=%p vt=%p tid=%d\n",
                off, obj, vt, (int)syscall(SYS_gettid)); fsync(2); }
    }
    unsigned long hs_off = getenv("ELD_HSOFF") ? strtoul(getenv("ELD_HSOFF"), 0, 16) : 0x45232c;
    if (off == hs_off) {
      void *obj = (void *)((char *)cslot - 0x88);
      if (!g_choreo_syncobj) {
        g_choreo_syncobj = obj;
        void *vt = *(void **)obj;
        void *prod = vt ? *(void **)((char *)vt + 0x20) : NULL;
        fprintf(stderr, "[HANDSHAKE] main sync obj=%p vtable=%p vt[4]=prod=%p (lu+0x452498=%p) tid=%d\n",
                obj, vt, prod, (void *)(g_pt_ub + 0x452498), (int)syscall(SYS_gettid)); fsync(2);
      } else if (obj != g_choreo_syncobj && !g_choreo_syncobj_gfx) {
