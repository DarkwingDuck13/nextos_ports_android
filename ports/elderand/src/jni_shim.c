...
static void *make_jstring(const char *value) {
  char *p = strdup(value ? value : "");
  if (g_jstring_count < MAX_JSTRINGS) {
    g_jstrings[g_jstring_count].handle = p;
    g_jstrings[g_jstring_count].value = p;
    g_jstring_count++;
  }
  return (void *)p;
}
static int is_jstring_handle(void *p) {
  for (int i = 0; i < g_jstring_count; i++)
    if (g_jstrings[i].handle == p) return 1;
  return 0;
}
/* tamanho LEGÍVEL de um arquivo (stat falha p/ /proc → lê o conteúdo e conta bytes). */
static long file_readable_len(const char *path) {
  if (!path) return 0;
  struct stat st;
  if (stat(path, &st) == 0 && st.st_size > 0) return (long)st.st_size;
  FILE *f = fopen(path, "rb");
  if (!f) return 0;
  long n = 0; char buf[4096]; size_t r;
...
/* ---- Registry de method/field IDs por NOME+ASSINATURA (recon Unity) ---- */
struct mid_entry { const char *name; const char *sig; };
static struct mid_entry g_midreg[8192];
static int g_midreg_count = 0;

static void *reg_mid(const char *name, const char *sig) {
  for (int i = 0; i < g_midreg_count; i++) {
    int same_name = (g_midreg[i].name == name) ||
                    (name && g_midreg[i].name && strcmp(g_midreg[i].name, name) == 0);
    int same_sig = (g_midreg[i].sig == sig) ||
                   (sig && g_midreg[i].sig && strcmp(g_midreg[i].sig, sig) == 0);
    if (same_name && same_sig)
      return &g_midreg[i];
  }
  if (g_midreg_count >= 8192) return &g_midreg[8191];  /* clamp estável (tags não podem mudar) */
  int i = g_midreg_count++;
  /* strdup: o pairip passa nomes em buffers TEMPORÁRIOS (stack/heap liberado) → guardar o
   * ponteiro cru deixa nm DANGLING (mid_name vira lixo, strcmp falha). Copiar resolve. */
  g_midreg[i].name = name ? strdup(name) : NULL;
  g_midreg[i].sig = sig ? strdup(sig) : NULL;
  return &g_midreg[i];
}
static const char *mid_name(void *tag) {
  if ((char *)tag >= (char *)g_midreg &&
      (char *)tag < (char *)(g_midreg + 8192))
...
extern int g_vmtrace;   /* def em bloco jobj; trace da VM anti-tamper do pairip */
#define VMT(...) do { if (g_vmtrace) fprintf(stderr, "[VMT] " __VA_ARGS__); } while (0)
/* fwd decls do sistema de objetos Java emulados (def perto de jni_AllocObject) */
struct jobj { void *clazz; char *str; long lval; const unsigned char *abuf; int alen; int apos; };
struct jobj *jobj_find(void *p);
const char *class_name_of(void *clazz);
#define JCLS(o) (jobj_find(o) ? class_name_of(((struct jobj*)(o))->clazz) : "-")
...
#include <stdlib.h>
#include <stdarg.h>
#define ASSET_BASE "/storage/roms/ports/elderand/"
...
/* Object[] real (NewObjectArray/Get/SetObjectArrayElement) — o ExecuteProgram do pairip
 * monta o args[] via NewObjectArray; sem isso (stub→null) o invoke recebe null → NPE. */
#define MAX_OARR 64
struct oarr { void **elems; int len; };
static struct oarr g_oarr[MAX_OARR];
static int g_oarr_n = 0;
static void *oarr_new(int len, void *init) {
  int i = g_oarr_n++ % MAX_OARR;
  if (g_oarr[i].elems) free(g_oarr[i].elems);
  g_oarr[i].elems = (void **)malloc((len > 0 ? len : 1) * sizeof(void *));
  g_oarr[i].len = len;
  for (int k = 0; k < len; k++) g_oarr[i].elems[k] = init;
  return &g_oarr[i];
}
static struct oarr *oarr_find(void *h) {
  if ((char *)h >= (char *)g_oarr && (char *)h < (char *)(g_oarr + MAX_OARR))
    return (struct oarr *)h;
  return NULL;
}
/* PAIRIP: cria um jbyteArray com cópia de `data` (p/ alimentar executeVM com o
 * programa IAP lido do disco). Retorna handle usável como jbyteArray no shim. */
void *jni_make_bytearray(const void *data, int len) {
  struct barr *b = (struct barr *)barr_new(len);
  if (b && b->buf && data && len > 0) memcpy(b->buf, data, len);
  return b;
}
...
static void *asset_open(const char *path) {
  ...
  /* Addressables/StreamingAssets no Android usa URIs "jar:file://<apk>!/assets/aa/...".
   * Normaliza p/ o caminho relativo após "!/assets/" (ou "/assets/"). */
  const char *bang = strstr(p, "!/assets/");
  if (bang) p = bang + 9;                       /* após "!/assets/" */
  else { const char *as = strstr(p, "/assets/"); if (as && !strncmp(p, "jar:", 4)) p = as + 8; }
  if (!strncmp(p, "assets/", 7)) p += 7;
  if (!strncmp(p, "file://", 7)) p += 7;
  snprintf(full, sizeof(full), ASSET_BASE "%s", p);
  FILE *fp = fopen(full, "rb");
  if (!fp) {
    snprintf(full, sizeof(full), ASSET_BASE "assets/%s", p);
    fp = fopen(full, "rb");
  }
...
static struct { const char *name; int tag; } g_classreg[4096];
...
static void *class_for(const char *name) {
  ...
  /* NÃO usar ring buffer: as TAGS precisam ser ESTÁVEIS (o pairip guarda a jclass de
   * FindClass e compara depois em NewObject; se a tag mudar de slot → File vira NULL → NPE). */
  if (g_classreg_n >= 4096) return &g_classreg[4095].tag;  /* clamp estável */
  int i = g_classreg_n++;
  g_classreg[i].name = strdup(name);   /* strdup: nomes do pairip são temporários (dangling) */
  return &g_classreg[i].tag;
}
uintptr_t g_pairip_base;   /* base de libpairipcore; setado em main.c */
static void rel(const char *tag, void *lr) {
  if (!g_vmtrace) return;
  uintptr_t a = (uintptr_t)lr;
  if (g_pairip_base && a >= g_pairip_base && a < g_pairip_base + 0x80000)
    fprintf(stderr, "    <%s caller=pairip+0x%lx>\n", tag, (unsigned long)(a - g_pairip_base));
  else
    fprintf(stderr, "    <%s caller=%p>\n", tag, lr);
}
static int class_name_method(const char *nm) {
  return nm && (!strcmp(nm, "getName") ||
                !strcmp(nm, "getCanonicalName") ||
                !strcmp(nm, "getTypeName"));
}
static void dot_class_name(const char *src, char *dst, size_t cap) {
  if (!dst || cap == 0) return;
  if (!src || !*src) src = "java/lang/Object";
  size_t i = 0;
  for (; src[i] && i + 1 < cap; i++)
    dst[i] = (src[i] == '/') ? '.' : src[i];
  dst[i] = 0;
}
static void slash_class_name(const char *src, char *dst, size_t cap) {
  if (!dst || cap == 0) return;
  if (!src || !*src) src = "java/lang/Object";
  size_t i = 0;
  for (; src[i] && i + 1 < cap; i++)
...
static int is_class_handle(void *obj) {
  const char *cn = class_name_of(obj);
  return cn && strcmp(cn, "?") != 0;
}
static void *class_from_jname(const char *name) {
  char tmp[256];
  slash_class_name(name, tmp, sizeof tmp);
  return class_for(tmp);
}
static void *class_name_jstring(void *clazz) {
  const char *cn = class_name_of(clazz);
  if (!cn || !strcmp(cn, "?")) cn = "java/lang/Object";
  char tmp[256];
  dot_class_name(cn, tmp, sizeof tmp);
  return make_jstring(tmp);
}
static const char *object_class_name(void *obj) {
  if (!obj) return "java/lang/Object";
  if (obj == &g_obj_keyevent) return "android/view/KeyEvent";
  if (obj == &g_current_activity) return "android/app/Activity";
  if (obj == &g_gamepad_device) return "android/view/InputDevice";
  if (obj == &g_assetmgr) return "android/content/res/AssetManager";
  if (obj == &g_appinfo) return "android/content/pm/ApplicationInfo";
  if (obj == &g_fmod_device_obj) return "org/fmod/FMODAudioDevice";
  if (obj == &g_message_sentinel) return "android/os/Message";
  if (obj == &g_long_box_sentinel) return "java/lang/Long";
  if (obj == &g_empty_list) return "java/util/List";
  if (obj == &g_iterator) return "java/util/Iterator";
  if (is_jstring_handle(obj)) return "java/lang/String";
  { struct jobj *jo = jobj_find(obj);
    if (jo) {
      const char *cn = class_name_of(jo->clazz);
      return (cn && strcmp(cn, "?")) ? cn : "java/lang/Object";
    }
  }
  if (oarr_find(obj)) return "[Ljava/lang/Object;";
  if (barr_find(obj)) return "[B";
  if (proxy_handle(obj)) return "java/lang/reflect/Proxy";
  return "java/lang/Object";
}
static void *jni_find_library_result(const char *ln) {
  if (ln && strstr(ln, "il2cpp")) return make_jstring(ASSET_BASE "libil2cpp.so");
  if (ln && strstr(ln, "main")) return make_jstring(ASSET_BASE "libmain.so");
  if (ln && strstr(ln, "unity")) return make_jstring(ASSET_BASE "libunity.so");
  if (ln && strstr(ln, "fmodstudio")) return make_jstring(ASSET_BASE "libfmodstudio.so");
  if (ln && strstr(ln, "fmod")) return make_jstring(ASSET_BASE "libfmod.so");
  return make_jstring("");
}
static void *jni_FindClass(void *env, const char *name) {
  (void)env;
  debugPrintf("jni_shim: FindClass(%s)\n", name);
  if (g_vmtrace) { fprintf(stderr, "[VMT] FindClass(%s)\n", name); rel("FindClass", __builtin_return_address(0)); }
...
static void *jni_FromReflectedMethod(void *env, void *method) {
  (void)env;
  if (getenv("TER_REFLOG")) debugPrintf("[REF] FromReflectedMethod(%p) -> same\n", method);
  return method;
}
static void *jni_ToReflectedMethod(void *env, void *clazz, void *methodID, unsigned char isStatic) {
  (void)env; (void)clazz; (void)isStatic;
  if (getenv("TER_REFLOG")) debugPrintf("[REF] ToReflectedMethod(%p) -> same\n", methodID);
  return methodID;
}
...
static void *jni_GetObjectField(void *env, void *obj, void *fieldID) {
  ...
  if (obj == &g_appinfo && nm) {
    if (strcmp(nm, "sourceDir") == 0 || strcmp(nm, "publicSourceDir") == 0)
      return make_jstring("/storage/roms/ports/elderand");
    if (strcmp(nm, "splitSourceDirs") == 0 || strcmp(nm, "splitPublicSourceDirs") == 0) {
      void *pack = make_jstring("/storage/roms/ports/elderand");
      void *arr = oarr_new(1, pack);
      debugPrintf("jni_shim: ApplicationInfo.%s -> [%s]\n", nm, "/storage/roms/ports/elderand");
      return arr;
    }
  }
  if (getenv("TER_KBFIX") &&
...
static void *jni_GetStaticFieldID(void *env, void *clazz, const char *name,
  ...
  if (name && strcmp(name, "currentActivity") == 0) {
    debugPrintf("jni_shim: GetStaticFieldID(currentActivity) -> activity field fake\n");
...
static void *jni_CallObjectMethodV(void *env, void *obj, void *methodID,
  ...
  VMT("CallObject obj=%p cls=%s m=%s%s\n", obj, JCLS(obj), nm?nm:"?", mid_sig(methodID)?:"");
  static int fake_obj;
  if (nm) {
    if (strcmp(nm, "getClass") == 0) {
      void *clazz = class_for(object_class_name(obj));
      if (getenv("TER_REFLOG")) debugPrintf("[REF] getClass obj=%p -> %s\n", obj, class_name_of(clazz));
      return clazz;
    }
    if (class_name_method(nm) && is_class_handle(obj)) {
      if (getenv("TER_REFLOG")) debugPrintf("[REF] Class.%s(%s)\n", nm, class_name_of(obj));
      return class_name_jstring(obj);
    }
    if (strcmp(nm, "loadClass") == 0) {
      void *name_j = va_arg(ap, void *);
      const char *cn = resolve_jstring(name_j);
      void *clazz = class_from_jname(cn);
      debugPrintf("jni_shim: loadClass(%s) -> %s\n", cn ? cn : "?", class_name_of(clazz));
      return clazz;
    }
    if (strcmp(nm, "getSuperclass") == 0 && is_class_handle(obj))
      return class_for("java/lang/Object");
    if (strcmp(nm, "getComponentType") == 0 && is_class_handle(obj)) {
      const char *cn = class_name_of(obj);
      return (cn && cn[0] == '[') ? class_for("java/lang/Object") : NULL;
    }
    if (getenv("TER_KBFIX")) {
    ...
    if (strcmp(nm, "getAssetPackPath") == 0) {
      ...
      debugPrintf("jni_shim: getAssetPackPath(%s) -> /storage/roms/ports/elderand\n", pn ? pn : "?");
      return make_jstring("/storage/roms/ports/elderand");
    }
    if (strcmp(nm, "getPackageCodePath") == 0 || strcmp(nm, "getPackageResourcePath") == 0)
      return make_jstring("/storage/roms/ports/elderand");
    ...
    if (strcmp(nm, "findLibrary") == 0) {
      ...
      return jni_find_library_result(ln);
    }
    ...
    if (strcmp(nm, "getFilesDir") == 0 || strcmp(nm, "getExternalFilesDir") == 0 ||
      ...
      return make_jstring("/storage/roms/ports/elderand/userdata");
    ...
    if (class_name_method(nm)) {
      static int gn = 0;
      if (gn++ < 40) debugPrintf("[REF] %s on non-Class obj=%p -> java.lang.Object\n", nm, obj);
    ...
    if (getenv("TER_KBFIX") && class_name_method(nm))
      return make_jstring("java.lang.Object");
...
static void *jni_CallObjectMethodA(void *env, void *obj, void *methodID, const jvalue *args) {
  ...
  if (nm) {
    if (strcmp(nm, "getClass") == 0)
      return class_for(object_class_name(obj));
    if (class_name_method(nm) && is_class_handle(obj))
      return class_name_jstring(obj);
    if (strcmp(nm, "loadClass") == 0) {
      const char *cn = args ? resolve_jstring(args[0].l) : "";
      return class_from_jname(cn);
    }
    if (strcmp(nm, "findLibrary") == 0) {
    ...
    if (strcmp(nm, "getAssetPackPath") == 0) {
      ...
      debugPrintf("jni_shim: getAssetPackPathA(%s) -> /storage/roms/ports/elderand\n", pn ? pn : "?");
      return make_jstring("/storage/roms/ports/elderand");
    }
    if (strcmp(nm, "getPackageCodePath") == 0 || strcmp(nm, "getPackageResourcePath") == 0)
      return make_jstring("/storage/roms/ports/elderand");
...
static unsigned char jni_CallBooleanMethodV(void *env, void *obj,
  ...
  (void)obj;
  const char *nm = mid_name(methodID);
  VMT("CallBoolean obj=%p cls=%s m=%s%s\n", obj, JCLS(obj), nm?:"?", mid_sig(methodID)?:"");
  if (nm) {
    ...
    /* 🔑 Play Core (Asset Delivery) AUSENTE: sem isso o Unity tenta o caminho de
     * asset-pack/OBB (ZipFile do main.0.*.obb inexistente) → crash. TRUE faz o
     * Unity usar os assets do APK direto (bin/Data via AssetManager), que temos. */
    if (strcmp(nm, "playCoreApiMissing") == 0) {
      int missing = getenv("ELD_PLAYCORE") ? 0 : 1;
      debugPrintf("jni_shim: playCoreApiMissing -> %s\n", missing ? "true (sem Play Core, usa APK)" : "false (Play Core fake/PAD)");
      return (unsigned char)missing;
    }
...
static void jni_CallVoidMethodV(void *env, void *obj, void *methodID, va_list ap) {
  ...
  VMT("CallVoid obj=%p cls=%s m=%s%s\n", obj, JCLS(obj), nm?:"?", mid_sig(methodID)?:"");
...
static void *jni_CallStaticObjectMethodV(void *env, void *clazz,
  ...
  (void)clazz;
  const char *nm = mid_name(methodID);
  VMT("CallStaticObject cls=%s m=%s%s\n", class_name_of(clazz), nm?:"?", mid_sig(methodID)?:"");
  if (nm && !strcmp(nm, "forName")) {
    void *name_j = va_arg(ap, void *);
    const char *cn = resolve_jstring(name_j);
    void *klass = class_from_jname(cn);
    debugPrintf("jni_shim: Class.forName(%s) -> %s\n", cn ? cn : "?", class_name_of(klass));
    return klass;
  }
  if (nm && (!strcmp(nm, "getSystemClassLoader") || !strcmp(nm, "getClassLoader"))) {
    static int fake_classloader;
    return &fake_classloader;
  }
  if (nm && (!strcmp(nm, "getMethodID") || !strcmp(nm, "getConstructorID"))) {
    (void)va_arg(ap, void *);
    void *name_j = NULL;
    void *sig_j = NULL;
    if (!strcmp(nm, "getMethodID")) {
      name_j = va_arg(ap, void *);
      sig_j = va_arg(ap, void *);
      (void)va_arg(ap, int);
    } else {
      name_j = make_jstring("<init>");
      sig_j = va_arg(ap, void *);
    }
    const char *mn = resolve_jstring(name_j);
    const char *sig = resolve_jstring(sig_j);
    debugPrintf("jni_shim: ReflectionHelper.%s(%s, %s) -> reflected method\n",
                nm, mn ? mn : "?", sig ? sig : "");
    return reg_mid(mn, sig);
  }
  if (nm && !strcmp(nm, "getFieldID")) {
    ...
    debugPrintf("jni_shim: ReflectionHelper.getFieldID(%s, %s) -> reflected field\n",
                fnm ? fnm : "?", sig ? sig : "");
    return reg_mid(fnm, sig);
  }
  if (nm && !strcmp(nm, "getFieldSignature")) {
    void *field = va_arg(ap, void *);
    if (field == &g_pressedstates_field_obj) return make_jstring("[Z");
    const char *sig = mid_sig(field);
    return make_jstring((sig && *sig) ? sig : "Ljava/lang/Object;");
  }
  /* 🌉 BRIDGE VMRunner.invoke(String name, Object[] args): o ExecuteProgram do pairip chama isto
   * (no app real é DEX que lê o asset e chama executeVM). Fazemos NÓS: ler asset[name] + executeVM
   * com o Object[] REAL que o ExecuteProgram montou (não dummy). */
  extern void *pairip_vmrunner_invoke(void *nameStr, void *argsArray);
  if (nm && strcmp(nm, "invoke") == 0 && getenv("ELD_PAIRIP")) {
    void *nameStr = va_arg(ap, void *);
    void *argsArr = va_arg(ap, void *);
    fprintf(stderr, "[BRIDGE] VMRunner.invoke('%s', args=%p)\n", resolve_jstring(nameStr), argsArr);
    return pairip_vmrunner_invoke(nameStr, argsArr);
  }
...
static void *jni_CallStaticObjectMethodA(void *env, void *clazz, void *methodID, const jvalue *args) {
  ...
  if (nm && !strcmp(nm, "forName")) {
    const char *cn = args ? resolve_jstring(args[0].l) : "";
    void *klass = class_from_jname(cn);
    debugPrintf("jni_shim: Class.forNameA(%s) -> %s\n", cn ? cn : "?", class_name_of(klass));
    return klass;
  }
  if (nm && (!strcmp(nm, "getSystemClassLoader") || !strcmp(nm, "getClassLoader"))) {
    static int fake_classloader;
  ...
  if (nm && (!strcmp(nm, "getMethodID") || !strcmp(nm, "getConstructorID"))) {
    ...
    void *sig_j = NULL;
    if (!strcmp(nm, "getMethodID")) {
      name_j = args ? args[1].l : NULL;
      sig_j = args ? args[2].l : NULL;
    } else {
      name_j = make_jstring("<init>");
      sig_j = args ? args[1].l : NULL;
    }
    const char *mn = resolve_jstring(name_j);
    const char *sig = resolve_jstring(sig_j);
    debugPrintf("jni_shim: ReflectionHelper.%sA(%s, %s) -> reflected method\n",
                nm, mn ? mn : "?", sig ? sig : "");
    return reg_mid(mn, sig);
  }
  if (nm && !strcmp(nm, "getFieldID")) {
    ...
    debugPrintf("jni_shim: ReflectionHelper.getFieldIDA(%s, %s) -> reflected field\n",
                fnm ? fnm : "?", sig ? sig : "");
    return reg_mid(fnm, sig);
  }
  if (nm && !strcmp(nm, "getFieldSignature")) {
...
static void static_void_dispatch(void *methodID, void *arg0) {
  const char *nm = mid_name(methodID);
  if (nm && !strcmp(nm, "load")) {
    debugPrintf("jni_shim: System.load(%s) -> ok\n", resolve_jstring(arg0));
    return;
  }
  if (nm && !strcmp(nm, "init")) {
    debugPrintf("jni_shim: static init%s -> ok\n", mid_sig(methodID) ? mid_sig(methodID) : "");
    return;
  }
...
static void jni_CallStaticVoidMethod(void *env, void *clazz, void *methodID,
  ...
  void *arg0 = NULL;
  va_list ap; va_start(ap, methodID);
  if (nm && (!strcmp(nm, "load") || !strcmp(nm, "init")))
    arg0 = va_arg(ap, void *);
  va_end(ap);
  static_void_dispatch(methodID, arg0);
}
...
static void jni_CallStaticVoidMethodA(void *env, void *clazz, void *methodID, const jvalue *args) {
  (void)env; (void)clazz;
  static_void_dispatch(methodID, args ? args[0].l : NULL);
}
...
static void *jni_NewString(void *env, const unsigned short *chars, int len) {
  (void)env;
  if (!chars || len <= 0) return make_jstring("");
  char tmp[1024];
  int n = 0;
  for (int i = 0; i < len && n + 4 < (int)sizeof tmp; i++) {
  ...
  tmp[n] = 0;
  debugPrintf("jni_shim: NewString(len=%d) -> %s\n", len, tmp);
  return make_jstring(tmp);
}
static jint jni_GetStringLength(void *env, void *jstr) {
  (void)env;
  const unsigned char *s = (const unsigned char *)resolve_jstring(jstr);
  int n = 0;
  while (s && *s) {
    if ((*s & 0xc0) != 0x80) n++;
    s++;
...
static const unsigned short *jni_GetStringChars(void *env, void *jstr, void *isCopy) {
  ...
  const unsigned char *s = (const unsigned char *)resolve_jstring(jstr);
  int cap = jni_GetStringLength(env, jstr) + 1;
  unsigned short *out = (unsigned short *)calloc(cap > 0 ? cap : 1, sizeof(unsigned short));
  if (!out) return NULL;
  int n = 0;
  while (s && *s && n + 1 < cap) {
    unsigned int c;
...
static void *jni_GetObjectClass(void *env, void *obj) {
  (void)env;
  const char *cn = object_class_name(obj);
  void *clazz = class_for(cn);
  if (getenv("TER_REFLOG")) debugPrintf("[REF] GetObjectClass obj=%p -> %s\n", obj, cn);
  return clazz;
}
...
static long jni_CallLongMethodV(void *env, void *obj, void *methodID, va_list ap) {
  ...
  extern long g_file_sentinel;
  const char *nm = mid_name(methodID);
  VMT("CallLong obj=%p cls=%s m=%s%s\n", obj, JCLS(obj), nm?:"?", mid_sig(methodID)?:"");
  /* File.length() num jobj-File (path em ->str). Para /proc/* o stat reporta 0, então LEMOS
   * o conteúdo real e devolvemos o nº de bytes (o anti-tamper do pairip precisa do tamanho). */
  { struct jobj *fo = jobj_find(obj);
    if (fo && fo->str && nm && strcmp(nm, "length") == 0) {
      long L = file_readable_len(fo->str);
      const char *ov = getenv("ELD_MAPSLEN");
      if (ov && strstr(fo->str, "/proc/self/maps")) L = atol(ov);   /* experimento */
      VMT("File.length('%s') -> %ld\n", fo->str, L);
      return L;
    }
  }
  if (obj == (void *)&g_file_sentinel && nm && strcmp(nm, "length") == 0) {
    fprintf(stderr, "[PAIRIP-FILE] File.length() -> 0 (proc file)\n");
    return 0;  /* /proc/self/maps reporta length 0 no Android real */
  }
  if (nm) {
    if (strcmp(nm, "getEventTime") == 0) return g_hk_inject.eventTime;
    if (strcmp(nm, "getDownTime") == 0) return g_hk_inject.downTime;
    if (strcmp(nm, "longValue") == 0) { struct jobj *lo = jobj_find(obj); return lo ? lo->lval : g_doframe_nanos; }
...
/* CallLongMethodA (idx 54): a VM do pairip chama File.length() por esta variante. */
static long jni_CallLongMethodA(void *env, void *obj, void *methodID, void *args) {
  (void)args; return jni_CallLongMethod(env, obj, methodID);
}

/* Exception handling */
static long g_jni_exc_pending = 0;
static char g_jni_exc_msg[256];
static void *g_jni_exc_obj = 0;   /* objeto throwable corrente (Throw/ThrowNew) */
/* Throw (idx 13): libunity re-lança a exceção do OBB ausente e DEPOIS faz
 * ExceptionOccurred — se voltar null → brk (assert). Setamos pending+obj. */
static int jni_Throw(void *env, void *obj) {
  (void)env;
  g_jni_exc_pending = 1;
  if (obj) g_jni_exc_obj = obj;
  return 0;
}
static unsigned char jni_ExceptionCheck(void *env) {
  (void)env;
  /* 🔑 reflete a exceção pendente (era return 0 fixo → libunity nunca via o throw
   * do ZipFile do OBB inexistente e processava objeto fake → crash em 0xbfa610). */
  return g_jni_exc_pending ? 1 : 0;
}
/* ThrowNew(env, clazz, message): o pairip chama isto quando um CHECK falha (tamper/integridade).
 * Antes caía no stub genérico (swallow) → a VM achava que não havia erro e seguia no LIXO → crash.
 * Aqui LOGAMOS a mensagem (revela o erro) e setamos exceção-pendente (ExceptionOccurred!=0) p/ a VM
 * desenrolar limpo. ENV ELD_THROW_OK=1: ignora (retorna sem setar) p/ comparar comportamento. */
static int jni_ThrowNew(void *env, void *clazz, const char *msg) {
  (void)env; (void)clazz;
  fprintf(stderr, "[THROWNEW] pairip lançou exceção: \"%s\"\n", msg ? msg : "(null)");
  rel("ThrowNew", __builtin_return_address(0));
  if (msg) { strncpy(g_jni_exc_msg, msg, sizeof g_jni_exc_msg - 1); g_jni_exc_msg[sizeof g_jni_exc_msg-1]=0; }
  if (!getenv("ELD_THROW_OK")) g_jni_exc_pending = 1;
  return 0;
}
static void jni_ExceptionClear(void *env) { (void)env; g_jni_exc_pending = 0; }
static void *jni_ExceptionOccurred(void *env) {
  (void)env;
  if (!g_jni_exc_pending) return NULL;
  return g_jni_exc_obj ? g_jni_exc_obj : (void *)1;  /* throwable real ou sentinela não-null */
}

/* Array */
extern long g_pairip_args_sentinel; extern int g_pairip_args_len;
static jint jni_GetArrayLength(void *env, void *array) {
  (void)env;
  if (array == (void *)&g_pairip_args_sentinel) {
    fprintf(stderr, "[PAIRIP-ARGS] GetArrayLength -> %d\n", g_pairip_args_len);
    return g_pairip_args_len;
  }
  ...
  { struct oarr *o = oarr_find(array); if (o) return o->len; }
  struct barr *b = barr_find(array);
  return b ? b->len : 0;
}
/* GetObjectArrayElement: args do doFrame -> o Long boxed (slot 0) */
/* PAIRIP args[]: array de objetos passado p/ executeVM. Instrumentado p/ ver acessos. */
long g_pairip_args_sentinel = 0xDEC0DE;
static long g_pairip_args_obj[16];   /* objetos-dummy não-nulos (cada slot é um "objeto") */
int g_pairip_args_len = 4;
static void *jni_GetObjectArrayElement(void *env, void *array, jint idx) {
  (void)env;
  if (array == (void *)&g_pairip_args_sentinel) {
    fprintf(stderr, "[PAIRIP-ARGS] GetObjectArrayElement(idx=%d)\n", idx);
    if (idx >= 0 && idx < 16) return &g_pairip_args_obj[idx];
    return NULL;
  }
  ...
  { struct oarr *o = oarr_find(array); if (o && idx >= 0 && idx < o->len) return o->elems[idx]; }
  return NULL;
}
...
long g_file_sentinel = 0xF11E;   /* java.io.File fake (pairip: new File("/proc/self/maps")) */

/* ===== Emulação genérica de objetos Java p/ a VM anti-tamper do pairip =====
 * A VM do pairip executa `new-instance` via AllocObject (idx 27) e depois invoke-direct
 * <init>. AllocObject estava STUB (→0/null) → todo `new X()` virava null → a VM lançava
 * "Attempt to execute instance method of null object" e morria. Damos objetos reais. */
#define MAX_JOBJS 512
static struct jobj g_jobjs[MAX_JOBJS];
static int g_jobj_n;
static struct jobj *jobj_alloc(void *clazz) {
  if (g_jobj_n >= MAX_JOBJS) g_jobj_n = 0;   /* ring */
  struct jobj *o = &g_jobjs[g_jobj_n++];
  memset(o, 0, sizeof *o);
  o->clazz = clazz;
  return o;
}
struct jobj *jobj_find(void *p);
struct jobj *jobj_find(void *p) {
  if ((char *)p >= (char *)g_jobjs && (char *)p < (char *)(g_jobjs + MAX_JOBJS))
    return (struct jobj *)p;
  return NULL;
}
const char *class_name_of(void *clazz) {
  for (int i = 0; i < g_classreg_n; i++)
    if (&g_classreg[i].tag == clazz) return g_classreg[i].name;
  return "?";
}
int g_vmtrace;   /* setado por main.c (ELD_VMTRACE) p/ logar a fase da VM */
/* AllocObject (idx 27): a VM cria objetos por aqui (new-instance). */
static void *jni_AllocObject(void *env, void *clazz) {
  (void)env;
  struct jobj *o = jobj_alloc(clazz);
  if (g_vmtrace) fprintf(stderr, "[ALLOC] AllocObject(%s) -> %p\n", class_name_of(clazz), (void *)o);
  return o;
}

/* ponteiro plausível p/ deref (rejeita lixo de vararg de construtor sem-arg, ex.
 * UnityCoreAssetPacksStatusCallbacks.<init>()V → first_arg = registrador-lixo
 * 0x28000000000000, fora do range 48-bit do usuário → crash em s[0]). */
static int jptr_ok(const void *p) {
  uintptr_t a = (uintptr_t)p;
  return a >= 0x10000 && a < 0x0001000000000000UL;
}
/* impl comum de NewObject. first_arg = 1º argumento do construtor (jstring p/ File). */
static void *jni_new_object_impl(void *clazz, void *mid, void *first_arg) {
  ...
  if (!jptr_ok(first_arg)) first_arg = NULL;   /* construtor sem-arg: vararg = lixo */
  /* 🔑 new ZipFile(path): se o arquivo NÃO existe (OBB ausente — dados estão em
   * bin/Data, não em OBB), lança FileNotFoundException → libunity cai no catch e
   * usa os assets do APK (bin/Data). Sem isso, devolvíamos um File fake e libunity
   * processava lixo → crash 0xbfa610 lendo a std::string do path .obb. */
  if (clazz == class_for("java/util/zip/ZipFile") && first_arg) {
    const char *p = resolve_jstring(first_arg);
    if (p && p[0] && access(p, F_OK) != 0) {
      debugPrintf("jni_shim: new ZipFile('%s') -> NULL (OBB ausente, sem exceção)\n", p);
      if (!getenv("ELD_ZIPTHROW")) return NULL;   /* null direto (sem exceção) */
      g_jni_exc_pending = 1;                       /* ELD_ZIPTHROW=1: variante exceção */
      return NULL;
    }
  }
  /* Construção genérica de objeto Java p/ a VM anti-tamper do pairip. A detecção de classe
   * por ponteiro é frágil (pairip guarda jclass que pode não casar nosso tag), então criamos
   * SEMPRE um objeto real (não-null) e, se o 1º arg é um path (jstring começando com '/'),
   * marcamos como java.io.File servindo esse caminho. NUNCA retornar null (→ NPE na VM). */
  const char *s = first_arg ? resolve_jstring(first_arg) : NULL;
  if (s && s[0] == '/') {
    struct jobj *o = jobj_alloc(clazz);
    o->str = strdup(s);   /* File path (ex.: /proc/self/maps) */
    o->lval = (long)first_arg;
    VMT("NewObject -> File('%s') obj=%p\n", o->str, (void*)o);
    return o;
  }
  if (g_vmtrace) {   /* fase VM do pairip: nunca null (→ NPE na VM) */
    struct jobj *o = jobj_alloc(clazz);
    /* 🔑 Long(value)/Integer(value): ExecuteProgram boxa o valor REAL (ex.: base ELF de uma
     * lib que a VM usa p/ resolver/hashear). Preserva o valor cru → longValue() devolve ele. */
    o->lval = (long)first_arg;
    VMT("NewObject(cls=%s) -> obj=%p (arg='%s' lval=%#lx)\n", class_name_of(clazz), (void*)o, s?s:"", o->lval);
    return o;
  }
  return NULL;   /* boot normal: comportamento original (sem regressão) */
}
static void *jni_NewObject(void *env, void *clazz, void *mid, ...) {
  (void)env;
  va_list ap; va_start(ap, mid); void *a0 = va_arg(ap, void *); va_end(ap);
  return jni_new_object_impl(clazz, mid, a0);
}
/* NewObjectArray (172): cria Object[len] inicializado com `init`. */
static void *jni_NewObjectArray(void *env, jint len, void *clazz, void *init) {
  (void)env; (void)clazz;
  void *a = oarr_new(len, init);
  VMT("NewObjectArray(len=%d) -> %p\n", len, a);
  return a;
}
/* SetObjectArrayElement (174) */
static void jni_SetObjectArrayElement(void *env, void *arr, jint idx, void *val) {
  (void)env;
  struct oarr *o = oarr_find(arr);
  if (o && idx >= 0 && idx < o->len) o->elems[idx] = val;
}
static void *jni_NewObjectV(void *env, void *clazz, void *mid, va_list ap){
  (void)env; void *a0 = va_arg(ap, void *); return jni_new_object_impl(clazz, mid, a0);
}
static void *jni_NewObjectA(void *env, void *clazz, void *mid, void *args){
  (void)env; void *a0 = args ? *(void **)args : NULL; return jni_new_object_impl(clazz, mid, a0);
}
...
void jni_shim_init(void **out_vm, void **out_env) {
  ...
  jni_env_vtable[27] = (uintptr_t)jni_AllocObject;  /* AllocObject — VM pairip new-instance */
  ...
  jni_env_vtable[54] = (uintptr_t)jni_CallLongMethodA;  /* pairip: File.length() via A */
  ...
  jni_env_vtable[172] = (uintptr_t)jni_NewObjectArray;        /* pairip ExecuteProgram monta args[] */
  jni_env_vtable[173] = (uintptr_t)jni_GetObjectArrayElement; /* doFrame args[0]=Long */
  jni_env_vtable[174] = (uintptr_t)jni_SetObjectArrayElement; /* pairip popula args[] */
  ...
  jni_env_vtable[207] = (uintptr_t)jni_SetBooleanArrayRegion;
  jni_env_vtable[228] = (uintptr_t)jni_ExceptionCheck;  /* ExceptionCheck = idx 228 (era 205 errado) */
