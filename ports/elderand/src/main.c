...
/* ---- dl_iterate_phdr p/ o ANTI-TAMPER do pairip ----
 * O pairip enumera as libs carregadas via dl_iterate_phdr e monta uma hash-map por NOME.
 * Sob qemu/PC os dlpi_name vêm com PATH LONGO do toolchain (".../Amlogic-old.../libc.so") →
 * estoura o SSO da std::string interna do pairip → bucket vira lixo → SIGBUS num atomic.
 * Damos a ele só o BASENAME de cada lib (curto, estilo Android) — sem path → sem overflow.
 * Mantemos bases/phdr REAIS (o self-integrity hasheia memória pela base). */
static int (*g_pairip_phdr_cb)(struct dl_phdr_info *, size_t, void *);
static int pairip_phdr_tramp(struct dl_phdr_info *info, size_t sz, void *d) {
  struct dl_phdr_info i2 = *info;
  const char *nm = info->dlpi_name;
  const char *bn = nm ? strrchr(nm, '/') : NULL;
  i2.dlpi_name = bn ? bn + 1 : (nm ? nm : "");
  return g_pairip_phdr_cb(&i2, sz, d);
}
int pairip_dl_iterate_phdr(int (*cb)(struct dl_phdr_info *, size_t, void *), void *data);
int pairip_dl_iterate_phdr(int (*cb)(struct dl_phdr_info *, size_t, void *), void *data) {
  g_pairip_phdr_cb = cb;
  return dl_iterate_phdr(pairip_phdr_tramp, data);
}

/* ---- /proc/self/maps FORJADO p/ o anti-tamper do pairip ----
 * pairip lê /proc/self/maps via syscall(openat/read) e parseia num hash-map. Sob qemu/emuelec
 * o maps mostra paths NÃO-Android (toolchain, /tmp, /storage) → a estrutura interna do pairip
 * corrompe (bucket vira bytes do conteúdo) → SIGBUS. Servimos um maps com PATHS estilo Android
 * (/data/app/.../lib/arm64/libX.so, /apex/.../libc.so), preservando os ENDEREÇOS REAIS (o
 * self-integrity hasheia memória pela base). Interceptamos só "/proc/self/maps". */
#include <sys/syscall.h>
static const char *android_path_for(const char *real) {
  const char *bn = real ? strrchr(real, '/') : NULL;
  bn = bn ? bn + 1 : (real ? real : "");
  static char buf[256];
  if (!*bn || bn[0] == '[') return real ? real : "";   /* [heap]/[stack]/anon: manter */
  /* libs do app vs sistema */
  if (strstr(bn,"pairipcore")||strstr(bn,"unity")||strstr(bn,"il2cpp")||strstr(bn,"libmain")||
      strstr(bn,"fmod")||strstr(bn,"RMS")||strstr(bn,"Firebase")||strstr(bn,"resonance")||strstr(bn,"burst")) {
    snprintf(buf,sizeof buf,"/data/app/~~AbCd==/com.pid.elderand-XyZ==/lib/arm64/%s",bn); return buf;
  }
  if (!strncmp(bn,"libc.so",7)||!strncmp(bn,"libc-",5)) return "/apex/com.android.runtime/lib64/bionic/libc.so";
  if (!strncmp(bn,"libm.so",7)||!strncmp(bn,"libm-",5)) return "/apex/com.android.runtime/lib64/bionic/libm.so";
  if (!strncmp(bn,"libdl.so",8)) return "/apex/com.android.runtime/lib64/bionic/libdl.so";
  if (!strncmp(bn,"ld-linux",8)||!strncmp(bn,"ld-",3)) return "/apex/com.android.runtime/lib64/bionic/linker64";
  snprintf(buf,sizeof buf,"/system/lib64/%s",bn); return buf;
}
static int forge_maps_fd(void) {
  FILE *r = fopen("/proc/self/maps","r");
  if (!r) return -1;
  char tmp[] = "/tmp/eldmapsXXXXXX";
  int fd = mkstemp(tmp);
  if (fd < 0) { fclose(r); return -1; }
  unlink(tmp);
  FILE *w = fdopen(dup(fd), "w");
  char line[1024];
  while (fgets(line, sizeof line, r)) {
    char addr[64], perm[8], off[32], dev[16], inode[32]; char path[768]; path[0]=0;
    int nf = sscanf(line, "%63s %7s %31s %15s %31s %767[^\n]", addr, perm, off, dev, inode, path);
    if (nf >= 5) {
      const char *ap = (nf >= 6 && path[0]) ? android_path_for(path) : "";
      if (ap[0]) fprintf(w, "%s %s %s %s %s                  %s\n", addr,perm,off,dev,inode,ap);
      else       fprintf(w, "%s %s %s %s %s \n", addr,perm,off,dev,inode);
    } else fputs(line, w);
  }
  fclose(w); fclose(r);
  lseek(fd, 0, SEEK_SET);
...
/* opendir/readdir do pairip: ele enumera /proc/self/map_files (nomes = ranges de endereço)
 * p/ a integridade. Sob qemu/emuelec isso corrompe a hash-map interna → SIGBUS. Servimos um
 * conjunto VAZIO p/ esse dir (teste) — outros dirs passam direto. */
#include <dirent.h>
static int g_mapfiles_dir;
void *pairip_opendir(const char *name);
void *pairip_opendir(const char *name) {
  if (getenv("ELD_VMTRACE")) fprintf(stderr, "[PAIRIP-DIR] opendir('%s')\n", name?name:"(null)");
  if (name && strstr(name, "map_files")) return &g_mapfiles_dir;
  return opendir(name);
}
void *pairip_readdir(void *d);
void *pairip_readdir(void *d) {
  if (d == &g_mapfiles_dir) return NULL;   /* dir vazio */
  return readdir(d);
}
int pairip_closedir(void *d);
int pairip_closedir(void *d) {
  if (d == &g_mapfiles_dir) return 0;
  return closedir(d);
}

/* BRIDGE: VMRunner.invoke(name, args) — chamado pelo ExecuteProgram do pairip via
 * CallStaticObjectMethod. Lê o asset IAP[name] e chama o nativo executeVM(bytes, args)
 * com o Object[] REAL montado pelo ExecuteProgram. */
void *g_pairip_env;   /* JNIEnv* (setado no bloco ELD_PAIRIP) */
void *pairip_vmrunner_invoke(void *nameStr, void *argsArray);
void *pairip_vmrunner_invoke(void *nameStr, void *argsArray) {
  extern void *jni_find_native(const char *);
  extern void *jni_make_bytearray(const void *, int);
  const char *name = nameStr ? (const char *)nameStr : "";
  void *exec_vm = jni_find_native("executeVM");
  const char *adir = getenv("ELD_PAIRIP_ASSETDIR");
  if (!adir) adir = "/storage/roms/ports/elderand/assets";
  char path[512]; snprintf(path, sizeof path, "%s/%s", adir, name);
  FILE *fp = fopen(path, "rb");
  if (!exec_vm || !fp) { fprintf(stderr, "[BRIDGE] FALHOU (exec=%p fp=%p path=%s)\n", exec_vm, (void*)fp, path); if(fp)fclose(fp); return NULL; }
  fseek(fp, 0, SEEK_END); long sz = ftell(fp); fseek(fp, 0, SEEK_SET);
  void *buf = malloc(sz); size_t rd = fread(buf, 1, sz, fp); fclose(fp);
  void *jarr = jni_make_bytearray(buf, (int)rd);
  fprintf(stderr, "[BRIDGE] executeVM(name='%s' %zuB, args=%p)\n", name, rd, argsArray);
  void *r = ((void *(*)(void *, void *, void *, void *))exec_vm)(g_pairip_env, NULL, jarr, argsArray);
  free(buf);
  fprintf(stderr, "[BRIDGE] executeVM('%s') -> %p\n", name, r);
  return r;
}

long pairip_syscall(long n, long a0, long a1, long a2, long a3, long a4, long a5);
long pairip_syscall(long n, long a0, long a1, long a2, long a3, long a4, long a5) {
  const char *path = NULL;
  if (n == SYS_openat) path = (const char *)a1;
#ifdef SYS_open
  else if (n == SYS_open) path = (const char *)a0;
#endif
  if (getenv("ELD_SYSTRACE")) {
    if (path) fprintf(stderr, "[PAIRIP-SYS] n=%ld path='%s'\n", n, path);
    else fprintf(stderr, "[PAIRIP-SYS] n=%ld a0=%#lx a1=%#lx a2=%#lx\n", n, a0, a1, a2);
  }
  if (path && strstr(path, "/proc/self/maps")) {
    int fd = forge_maps_fd();
    if (getenv("ELD_VMTRACE")) fprintf(stderr, "[PAIRIP-MAPS] openat('%s') -> forjado fd=%d\n", path, fd);
    if (fd >= 0) return fd;
  }
...
/* recovery por-init: pula init_array entries que crasham (lazy PLT pairip não resolvido).
 * Compartilhado com so_execute_init_array (so_util.c). */
sigjmp_buf g_eld_init_jmp;
volatile int g_eld_init_armed = 0;
volatile unsigned long g_eld_init_skipped = 0;
volatile unsigned long g_eld_init_crash_pc = 0;   /* offset libunity do último crash de init */
volatile unsigned long g_eld_init_crash_lr = 0;
volatile unsigned long g_eld_init_crash_fault = 0;
static void raw_hex(char *p, unsigned long v) {  /* 16 hex digits, no null */
  for (int i = 15; i >= 0; i--) { p[i] = "0123456789abcdef"[v & 0xf]; v >>= 4; }
}
static void on_crash(int sig, siginfo_t *si, void *uc_) {
  ...
  /* RAW dump (signal-safe, sempre dispara ANTES de qualquer recovery/fprintf):
     escreve direto no fd 2 via write(). g_eld_init_armed pode mandar p/ longjmp
     cross-thread (crasha de novo) → este write garante ao menos o PC/LR/fault. */
  {
    uintptr_t tbr = (uintptr_t)text_base, ibr = g_il2cpp_base;
    char b[160]; const char *tag = "RAW";
    memcpy(b, "\n[RAWCRASH] sig=", 16); b[16] = '0' + (sig % 10); b[17] = ' ';
    memcpy(b + 18, "pc=", 3); raw_hex(b + 21, pc0);
    memcpy(b + 37, " lr=", 4); raw_hex(b + 41, lr0);
    memcpy(b + 57, " fault=", 7); raw_hex(b + 64, (uintptr_t)si->si_addr);
    b[80] = '\n';
    (void)write(2, b, 81);
    char o[80];
    if (pc0 >= tbr && pc0 < tbr + text_size) { memcpy(o, "[RAW] pc=unity+", 15); raw_hex(o + 15, pc0 - tbr); o[31] = '\n'; (void)write(2, o, 32); }
    else if (ibr && pc0 >= ibr && pc0 < ibr + 0x4000000) { memcpy(o, "[RAW] pc=il2cpp+", 16); raw_hex(o + 16, pc0 - ibr); o[32] = '\n'; (void)write(2, o, 33); }
  ...
  if (g_eld_init_armed) {
    ...
    g_eld_init_crash_pc = (pc0 >= tb0 && pc0 < tb0 + text_size) ? (pc0 - tb0) : pc0;
    g_eld_init_crash_lr = (lr0 >= tb0 && lr0 < tb0 + text_size) ? (lr0 - tb0) : lr0;
    g_eld_init_crash_fault = (uintptr_t)si->si_addr;
    siglongjmp(g_eld_init_jmp, 1);
  }
  ...
  /* ---- FNV/pairip: estado do loop de hash (buffer/offset/count w26) ----
     Bloco @pairip+0x58e7c: x23=[sp+256]; ptrA=[x23+104]; buffer=[ptrA+8];
     offset=[sp+272]; count=w26 (live VM reg). No fault o loop não mexe em
     x23/x8/sp → dá p/ ler a verdade do count e do container. */
  {
    extern uintptr_t g_pairip_base;
    if (g_pairip_base && pc >= g_pairip_base && pc < g_pairip_base + 0x90000) {
      unsigned long rel = pc - g_pairip_base;
      uintptr_t spv = uc->uc_mcontext.sp;
      uint64_t *R = (uint64_t *)uc->uc_mcontext.regs;
      fprintf(stderr, "[FNV] pairip_base=0x%lx pc=pairip+0x%lx sp=0x%lx\n",
              g_pairip_base, rel, spv);
      fprintf(stderr, "[FNV] w26(count)=0x%lx (%d signed)  x20(hash)=0x%lx\n",
              (unsigned long)(R[26] & 0xffffffff), (int)(int32_t)R[26],
              (unsigned long)R[20]);
      crash_classify("x8(buffer-live)", R[8]);
      crash_classify("x10(cursor@fault)", R[10]);
      if (addr_readable(spv + 256)) {
        uintptr_t x23 = *(uintptr_t *)(spv + 256);
        unsigned long off = addr_readable(spv + 272) ? *(uint32_t *)(spv + 272) : 0;
        unsigned long v280 = addr_readable(spv + 280) ? *(uint32_t *)(spv + 280) : 0;
        fprintf(stderr, "[FNV] [sp+256]x23=0x%lx [sp+272]offset=0x%lx [sp+280]=0x%lx\n",
                x23, off, v280);
        crash_dump_qwords("x23ctx", x23, 24);   /* contexto/reg-file da VM */
        if (addr_readable(x23 + 104)) {
          uintptr_t ptrA = *(uintptr_t *)(x23 + 104);
          crash_classify("ptrA=[x23+104]", ptrA);
          crash_dump_qwords("ptrA", ptrA, 12);   /* struct container */
          if (addr_readable(ptrA + 8)) {
            uintptr_t buf = *(uintptr_t *)(ptrA + 8);
            crash_classify("buffer=[ptrA+8]", buf);
            crash_dump_qwords("buf_head", buf, 8);
            if (addr_readable(buf + off))
              crash_dump_qwords("buf@offset", buf + off, 8);
          }
        }
      }
      if (addr_readable(spv + 528)) crash_dump_qwords("sp+528(key?)", spv + 528, 8);
      dbg_sync();
    }
  }
 ...
 * em /storage/roms/ports/elderand/shot.ppm (verificação autônoma de IMAGEM no fbdev Mali). */
static void ter_screenshot_maybe(void) {
  ...
  FILE *f = fopen("/storage/roms/ports/elderand/shot.ppm","wb");
  if (f) { fprintf(f,"P6\n%d %d\n255\n", w, h);
  ...
  if (!getenv("TER_GP_RAWJS")) {
    if (!sdl_tried) {
      ...
      /* 🎮 Mapeamento conhecido-bom p/ o adaptador genérico "USB Gamepad" (vendor 0810 / prod 0001,
         ...
         este controle; outros pads usam o mapa normal. NextOS pode sobrepor: TER_GP_MAP="<string>".
...
static void ter_ctrl_feed(void) {
  ...
  /* TER_SWAPAB: troca A<->B (NextOS pediu). A=Action1(confirma), B=Action2(volta). */
  ...
  /* TER_SWAPLR: inverte L1<->L2 e R1<->R2 (NextOS pediu; pode tirar a dupla-funcao do R1). */
...
/* RSS/Swap atual do processo (de /proc/self/status) — p/ medir o efeito do free do so_base. */
static void rss_probe(const char *tag) {
  FILE *f = fopen("/proc/self/status", "r");
  if (!f) return;
  char line[256]; long rss = -1, swap = -1;
  while (fgets(line, sizeof line, f)) {
    if (!strncmp(line, "VmRSS:", 6))  rss  = atol(line + 6);
    else if (!strncmp(line, "VmSwap:", 7)) swap = atol(line + 7);
  }
  fclose(f);
  fprintf(stderr, "[RSS:%s] VmRSS=%ldkB VmSwap=%ldkB\n", tag, rss, swap);
}
...
static void *choreo_driver_thread(void *arg) {
  ...
  int started = 0;
  /* TER_CHOREO_MSG: além do doFrame, re-dirige handleMessage a cada frame. No fluxo real do
     Android o Looper processa a Message (handleMessage→postFrameCallback re-arma o vsync) toda
     vez; nosso handleMessage rodava SÓ 1× (sendToTarget síncrono). Se o queue da MAIN é
     alimentado pelo caminho do Handler (não pelo doFrame direto), re-dirigir aqui destrava. */
  int drive_msg = getenv("TER_CHOREO_MSG") ? 1 : 0;
  ...
  for (;;) {
    ...
    if (drive_msg) jni_handlemessage(g_choreo_env);
...
static void *fmod_audio_thread(void *arg) {
  ...
  /* back-pressure: colchão p/ absorver jitter de scheduling (o jogo ocupa os 4 núcleos do Mali). 6
     blocos default = continuidade SEM engasgo, latência ~256ms (NextOS validou ~perfeito). Tunável
...
/* ===== LAZY PLT (Unity 2021.3.42) — sonda/handler =====
 * O libunity tem ~177 slots .got.plt de funções internas (libc++) em valor-lazy
 * cru (sem reloc) -> blr p/ 0xbf7c0 (PLT0 não-rebaseado) = crash. ELD_LAZYPROBE
 * patcha todos pra este stub p/ enxergar QUEM é chamado (RA) no boot. */
static int g_lazy_log = 0;
static uintptr_t g_lazy_seen[512]; static int g_lazy_nseen = 0;
static char g_lazy_scratch[1 << 16] __attribute__((aligned(16)));
/* decodifica o `bl <plt>` em RA-4 -> PLT -> adrp+ldr -> offset LINK-TIME do slot GOT. */
static unsigned long lazy_slot_of_ra(uintptr_t ra) {
  if (!g_unity_base) return 0;
  uint32_t bl = *(uint32_t *)(ra - 4);
  if ((bl & 0xfc000000u) != 0x94000000u) return 0;
  long imm = ((long)(int32_t)(bl << 6)) >> 4;            /* sign-ext imm26, *4 */
  uintptr_t plt = (ra - 4) + imm;
  uintptr_t plt_link = plt - g_unity_base;
  uint32_t adrp = *(uint32_t *)plt, ldr = *(uint32_t *)(plt + 4);
  if ((adrp & 0x9f000000u) != 0x90000000u) return 0;
  long immlo = (adrp >> 29) & 3, immhi = (adrp >> 5) & 0x7ffff;
  long pageoff = ((immhi << 2) | immlo) << 12;
  uintptr_t adrp_page = (plt_link & ~0xfffUL) + (uintptr_t)pageoff;
  unsigned ldr_off = ((ldr >> 10) & 0xfff) * 8;
  return (unsigned long)(adrp_page + ldr_off);
}
long lazy_noop_fn(long a, long b, long c, long d) { (void)a;(void)b;(void)c;(void)d; return 0; }
static long lazy_stub(long a0, long a1, long a2, long a3) {
  (void)a0;(void)a1;(void)a2;(void)a3;
  uintptr_t ra = (uintptr_t)__builtin_return_address(0);
  if (getenv("ELD_LAZYALL")) {            /* loga TODA chamada (sem dedup) p/ achar freq */
    if (g_lazy_nseen < 4000) { g_lazy_nseen++;
      fprintf(stderr, "[LZ] slot=0x%lx a0=0x%lx\n", lazy_slot_of_ra(ra), a0); }
  } else {
    int seen = 0;
    for (int i = 0; i < g_lazy_nseen; i++) if (g_lazy_seen[i] == ra) { seen = 1; break; }
    if (!seen && g_lazy_nseen < 512) {
      g_lazy_seen[g_lazy_nseen++] = ra;
      unsigned long off = g_unity_base ? (unsigned long)(ra - g_unity_base) : 0;
      fprintf(stderr, "[LAZYPLT] slot=0x%lx RA=libunity+0x%lx a0=0x%lx (n=%d)\n",
              lazy_slot_of_ra(ra), off, a0, g_lazy_nseen);
    }
  }
  (void)g_lazy_log; (void)g_lazy_scratch; (void)ra;
  /* objeto falso seguro: cada campo = &g_noop_vtable -> usado como objeto, [0] aponta
   * p/ vtable cujos slots = lazy_noop_fn (no-op) -> chamadas virtuais não crasham.
   * Reduz drasticamente crashes de init (lazy PLT não resolvido). */
  extern long lazy_noop_fn(long, long, long, long);
  /* operator new(a0): a0 = tamanho pequeno -> bloco FRESCO (cada new é distinto). */
  int is_new = (a0 > 0 && a0 < (16L << 20));
  if (!is_new) {
    /* getter/singleton (arg=ponteiro/nada): CACHE POR SLOT -> mesma função retorna
     * sempre o MESMO objeto (consistência de singleton, corta a cascata de globais). */
    unsigned long slot = lazy_slot_of_ra(ra);
    if (slot) {
      static struct { unsigned long slot; void *obj; } sc[512]; static int nsc = 0;
      for (int i = 0; i < nsc; i++) if (sc[i].slot == slot) return (long)(uintptr_t)sc[i].obj;
      uintptr_t *o = (uintptr_t *)calloc(1, 65536);
      uintptr_t *v = (uintptr_t *)calloc(256, 8);
      if (v) for (int i = 0; i < 256; i++) v[i] = (uintptr_t)lazy_noop_fn;
      if (o) o[0] = (uintptr_t)v;
      if (nsc < 512) { sc[nsc].slot = slot; sc[nsc].obj = o; nsc++; }
      return (long)(uintptr_t)o;
    }
  }
  size_t sz = is_new ? (size_t)a0 + 64 : 65536;
  uintptr_t *buf = (uintptr_t *)calloc(1, sz);
  uintptr_t *vt = (uintptr_t *)calloc(256, 8);
  if (vt) for (int i = 0; i < 256; i++) vt[i] = (uintptr_t)lazy_noop_fn;
  if (buf) buf[0] = (uintptr_t)vt;
  return (long)(uintptr_t)buf;
}
/* impls reais de internos (pthread/libc) lazy, bindados por slot GOT (offset link-time).
 * f79e20 = call_once com mutex: lock/unlock retornam 0 (sucesso); self=tid consistente. */
static long my_lock0(void *m) { (void)m; return 0; }   /* pthread_mutex_lock -> 0 */
static long my_unlock0(void *m) { (void)m; return 0; }  /* pthread_mutex_unlock -> 0 */
static long my_self_tid(long a) { (void)a; return (long)syscall(SYS_gettid); }
/* libc++ std::string REAL (24B). short: [0]=size<<1, [1..23]=data. long: [0..7]=cap|1,
 * [8..15]=size, [16..23]=ptr. ctor=vazio(zera 24B); copy=cópia SSO-aware; dtor=free se long. */
static long my_str_ctor(void *out) { if (out) memset(out, 0, 24); return (long)(uintptr_t)out; }
static long my_str_copy(void *dest, void *src) {
  if (!dest) return 0;
  if (!src) { memset(dest, 0, 24); return (long)(uintptr_t)dest; }
  unsigned char *s = (unsigned char *)src;
  if (s[0] & 1) {                               /* long string */
    unsigned long size = *(unsigned long *)(s + 8);
    char *sptr = *(char **)(s + 16);
    char *np = (char *)malloc(size + 1);
    if (np && sptr) memcpy(np, sptr, size + 1);
    *(unsigned long *)dest = size | 1UL;
    *(unsigned long *)((char *)dest + 8) = size;
    *(char **)((char *)dest + 16) = np;
  } else {                                       /* short: cópia rasa 24B (SSO inline) */
    memcpy(dest, src, 24);
  }
  return (long)(uintptr_t)dest;
}
static long my_str_noop(void *a, long b) { (void)b; return (long)(uintptr_t)a; }
static long my_ret_a0(long a, long b, long c, long d) { (void)b;(void)c;(void)d; return a; }   /* método -> this */
static long my_ret_a1(long a, long b, long c, long d) { (void)a;(void)c;(void)d; return b; }   /* ctor(?, out) -> out */
static void my_str_dtor(void *s) { if (s) { unsigned char *p = (unsigned char *)s; if (p[0] & 1) free(*(char **)(p + 16)); } }
/* tabela {offset GOT link-time -> func}. Aplicada APÓS a sonda patchar tudo p/ stub. */
static void eld_lazy_overrides(uintptr_t got_lazy_base_unused) {
  (void)got_lazy_base_unused;
  extern int sf_vfprintf(void *, const char *, __builtin_va_list);
  extern int sf_fputc(int, void *);
  extern void *memcpy(void *, const void *, unsigned long);
  struct { uintptr_t off; void *f; } T[] = {
    {0x1385210, (void *)my_lock0},     /* bfcc0  pthread_mutex_lock */
    {0x1385A48, (void *)my_unlock0},   /* c0d30  pthread_mutex_unlock */
    {0x1385280, (void *)my_self_tid},  /* bfda0  pthread_self/gettid */
    {0x1385488, (void *)sf_vfprintf},  /* c01b0  logger vfprintf */
    {0x1385400, (void *)sf_fputc},     /* c00a0  logger fputc '\n' */
    {0x13859a8, (void *)memcpy},       /* c0bf0  memcpy(dst,src,n) */
    {0x13856a8, (void *)my_str_ctor},  /* c05f0  std::string ctor vazio */
    {0x1385888, (void *)my_str_noop},  /* c09b0  método(str,1) */
    {0x1385a90, (void *)my_str_copy},  /* c0dc0  std::string copy(dest,src) */
    {0x1385020, (void *)my_str_dtor},  /* bf8e0  std::string dtor */
    {0x1385620, (void *)my_str_ctor},  /* c04e0  std::string ctor (2º bloco) */
    {0x1385a10, (void *)my_str_noop},  /* c0cc0  método(str,1) */
    {0x13855d8, (void *)my_str_copy},  /* c0450  std::string copy(x19+0x28,src) */
    {0x1384fc0, (void *)my_str_dtor},  /* bf820  std::string dtor */
    {0x1385998, (void *)my_lock0},     /* c0bd0  lock/once(&global) -> 0 */
    {0x1385970, (void *)my_ret_a0},    /* c0b80  método(this,..) -> this */
    {0x1385048, (void *)my_ret_a1},    /* bf930  ctor(1,out) -> out */
  };
  (void)my_str_noop;
  for (unsigned i = 0; i < sizeof T/sizeof T[0]; i++)
    *(uintptr_t *)((uintptr_t)text_base + T[i].off) = (uintptr_t)T[i].f;
  fprintf(stderr, "[LAZYPLT] %u overrides aplicados\n", (unsigned)(sizeof T/sizeof T[0]));
}

/* ===== eld_lazy_overrides_dev — libunity REAL do device (1.3.22, md5 8fe7856d) =====
 * Capturado via frida lendo a GOT resolvida do app rodando (bancada Moto G100).
 * Essa libunity tem 354 rela.plt (loader resolve) + só 12 lazy (sem rela): libc + __sF.
 * GOT base 0x1380f88 (≠ 0x1385000 da versão antiga). Gated ELD_DEVLIB=1. */
static void eld_lazy_overrides_dev(void) {
  extern char __sF[];
  extern int sf_fputc(int, void *);
  extern unsigned long sf_fread(void *, unsigned long, unsigned long, void *);
  extern unsigned long sf_fwrite(const void *, unsigned long, unsigned long, void *);
  void *H = (void *)0; (void)H;
  struct { uintptr_t off; void *f; const char *nm; } T[] = {
    {0x1381b90, dlsym(RTLD_DEFAULT, "calloc"),  "calloc"},
    {0x1381fd8, dlsym(RTLD_DEFAULT, "realloc"), "realloc"},
    {0x1382108, dlsym(RTLD_DEFAULT, "free"),    "free"},
    {0x13822b8, dlsym(RTLD_DEFAULT, "strdup"),  "strdup"},
    {0x1382370, (void *)__sF,                   "__sF"},        /* dado: array FILE* bionic */
    {0x1382a28, (void *)sf_fputc,               "fputc(sf)"},   /* FILE* = &__sF[i] */
    {0x1383410, dlsym(RTLD_DEFAULT, "pthread_create"), "pthread_create"},
    {0x13834d8, dlsym(RTLD_DEFAULT, "malloc"),  "malloc"},
    {0x13837d8, (void *)sf_fread,               "fread(sf)"},
    {0x13838f0, (void *)sf_fwrite,              "fwrite(sf)"},
    {0x1383c08, dlsym(RTLD_DEFAULT, "siglongjmp"), "siglongjmp"},
    /* 0x1381ab0 -> ponteiro INTERNO libunity (0x1380d18) = RELATIVE, loader já trata */
  };
  int ok = 0;
  for (unsigned i = 0; i < sizeof T / sizeof T[0]; i++) {
    if (!T[i].f) { fprintf(stderr, "[DEVLIB] FALTA %s\n", T[i].nm); continue; }
    *(uintptr_t *)((uintptr_t)text_base + T[i].off) = (uintptr_t)T[i].f;
    ok++;
  }
  fprintf(stderr, "[DEVLIB] %d/12 binds lazy aplicados (libunity 1.3.22)\n", ok);
}

/* stdio __sF shims (bionic_shims.c) — mapeiam &__sF[i] -> glibc stdin/out/err */
extern char __sF[];
extern int sf_fputc(int, void *), sf_putc(int, void *), sf_fputs(const char *, void *);
extern unsigned long sf_fwrite(const void *, unsigned long, unsigned long, void *);
extern unsigned long sf_fread(void *, unsigned long, unsigned long, void *);
extern int sf_fflush(void *), sf_vfprintf(void *, const char *, __builtin_va_list);
extern int sf_fprintf(void *, const char *, ...), sf_fileno(void *), sf_ferror(void *), sf_feof(void *);
extern void sf_clearerr(void *);
extern int sf_setvbuf(void *, char *, int, unsigned long), sf_getc(void *), sf_fgetc(void *);
extern char *sf_fgets(char *, int, void *);
static const struct { const char *n; void *f; } g_stdio_shims[] = {
  {"fputc",(void*)sf_fputc},{"putc",(void*)sf_fputc},{"fputs",(void*)sf_fputs},
  {"fwrite",(void*)sf_fwrite},{"fread",(void*)sf_fread},{"fflush",(void*)sf_fflush},
  {"vfprintf",(void*)sf_vfprintf},{"fprintf",(void*)sf_fprintf},{"fileno",(void*)sf_fileno},
  {"ferror",(void*)sf_ferror},{"feof",(void*)sf_feof},{"clearerr",(void*)sf_clearerr},
  {"setvbuf",(void*)sf_setvbuf},{"getc",(void*)sf_getc},{"fgetc",(void*)sf_fgetc},
  {"fgets",(void*)sf_fgets},
};
static void eld_wire_stdio_setimport(void) {
  extern void set_import(const char *, void *);
  set_import("__sF", (void *)__sF);
  for (unsigned i = 0; i < sizeof g_stdio_shims/sizeof g_stdio_shims[0]; i++)
    set_import(g_stdio_shims[i].n, g_stdio_shims[i].f);
}
static void eld_wire_stdio_patchgot(void) {
  patch_got("__sF", (void *)__sF);
  for (unsigned i = 0; i < sizeof g_stdio_shims/sizeof g_stdio_shims[0]; i++)
    patch_got(g_stdio_shims[i].n, g_stdio_shims[i].f);
}

/* 🔑 SAFEGUARD: watchdog que mata o processo sozinho após ELD_MAXSECONDS (default 90s).
   Roda numa thread PRÓPRIA que só dorme e chama exit_group(SYS) DIRETO — não depende de ssh,
   nem do my_exit shim, nem de fila/cond. Um busy-loop em 2 threads não trava os outros 2 cores,
   então o watchdog SEMPRE roda. Evita travar o device (sshd saturado) quando o boot trava num
   spin. ELD_MAXSECONDS=0 desliga. */
static void *eld_hard_watchdog(void *arg) {
  long secs = (long)(intptr_t)arg;
  struct timespec ts = { secs, 0 };
  nanosleep(&ts, NULL);
  const char m[] = "[WATCHDOG] ELD_MAXSECONDS estourou -> teardown Mali + exit_group\n";
  (void)write(2, m, sizeof(m) - 1);
  /* 🔑 teardown do fbdev/Mali ANTES de morrer — senão o driver Utgard trava o display e o device
     fica inacessível (sshd morto). Best-effort, thread-agnóstico. */
  { extern void egl_shim_emergency_teardown(void); egl_shim_emergency_teardown(); }
  syscall(SYS_exit_group, 124);   /* mata TODAS as threads, bypassa qualquer shim */
  _exit(124);
  return NULL;
}

/* TER_THRCENSUS=segundos: depois de N s (Unity já renomeou as threads) varre
 * /proc/self/task/<tid>/comm + state. Cruzar com TER_JOBLOG (tid->start-routine)
 * dá start-routine -> nome final (acha o "UnityGfxDeviceW" p/ o patch single-threaded). */
static void *eld_thr_census(void *arg) {
  long secs = (long)(intptr_t)arg; if (secs < 1) secs = 22;
  struct timespec ts = { secs, 0 }; nanosleep(&ts, NULL);
  DIR *d = opendir("/proc/self/task");
  if (!d) { write(2, "[CENSUS] opendir falhou\n", 24); return NULL; }
  struct dirent *e;
...
int main(int argc, char **argv) {
  ...
  if (getenv("TER_THRCENSUS")) { pthread_t ce; pthread_create(&ce, NULL, eld_thr_census, (void *)(intptr_t)atol(getenv("TER_THRCENSUS"))); }
  g_main_tid = (int)syscall(SYS_gettid);  /* p/ o sem_shim distinguir main de workers */
  { long mxs = getenv("ELD_MAXSECONDS") ? atol(getenv("ELD_MAXSECONDS")) : 90;
    if (mxs > 0) { pthread_t wd; pthread_create(&wd, NULL, eld_hard_watchdog, (void *)(intptr_t)mxs);
                   pthread_detach(wd);
                   fprintf(stderr, "[WATCHDOG] auto-exit em %lds (ELD_MAXSECONDS)\n", mxs); } }
  ...
  eld_wire_stdio_setimport();  /* fputc/fwrite/vfprintf/__sF -> map_sf glibc (logger do init) */
  ...
  /* libunity REAL do device (1.3.22): 12 binds lazy via captura frida (bancada Moto). */
  if (getenv("ELD_DEVLIB")) eld_lazy_overrides_dev();
  /* LAZY PLT probe: ~177 slots internos (libc++) sem reloc -> sonda */
  if (getenv("ELD_LAZYPROBE")) {
    extern int so_patch_lazy_plt(uintptr_t, uintptr_t *, uintptr_t *);
    uintptr_t lz = 0, gb = 0;
    int n = so_patch_lazy_plt((uintptr_t)lazy_stub, &lz, &gb);
    fprintf(stderr, "[LAZYPLT] %d slots lazy (val=0x%lx got=0x%lx) -> stub\n",
            n, (unsigned long)lz, (unsigned long)gb);
    eld_lazy_overrides(gb);
  }
  ...
  /* 🔑 pthread TLS: key_create estava em imports.gen.c (set_import→sh_key_create)
   * mas get/setspecific/key_delete NÃO → set_import no-op → resolviam pro glibc REAL.
   * Mismatch: sh_key_create devolve key 1,2,3.. e glibc getspecific(1) dá lixo/null.
   * (era o crash do nativeRecreateGfxState: getter TLS @0x420344 retornava null.)
   * patch_got escreve a GOT direto → os 4 usam o MESMO shim. */
  patch_got("pthread_key_create", (void *)sh_key_create);
  patch_got("pthread_key_delete", (void *)sh_key_delete);
  patch_got("pthread_getspecific", (void *)sh_getspecific);
  patch_got("pthread_setspecific", (void *)sh_setspecific);
   ...
   * (libunity 0x437c10, chamada de 0x436a60 com retorno IGNORADO) tenta abrir
   * "main.0.<pkg>.obb" (inexistente — dados estão em bin/Data via APK). No so-loader
   * o caminho "sem OBB" corrompe uma std::local → crash 0xbfa610. Como o OBB não
   * existe (igual ao device real, cujo dir obb está vazio), pulamos a função inteira
   * (ret imediato, sp intacto pois é a 1ª instrução) → Unity usa os assets do APK. */
  if (getenv("ELD_DEVLIB") && !getenv("ELD_NOSKIPOBB")) {
    extern void so_make_text_writable(void), so_make_text_executable(void);
    so_make_text_writable();
  ...
  /* 🎯 PAIRIP: a lazy PLT (177 slots .got.plt) é resolvida pela VM ExecuteProgram do
   * libpairipcore (chamada pelo _init do libunity). Carrega libpairipcore, wire
   * ExecuteProgram, e chama _init -> resolve a PLT antes do init_array. */
  if (getenv("ELD_PAIRIP")) {
    so_module *m_unity = so_save();
    unsigned long slot_antes = *(unsigned long *)((uintptr_t)text_base + 0x1385210);
    size_t ps = 24UL * 1024 * 1024;
    void *pheap = mmap(NULL, ps, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    uintptr_t pairip_exec = 0;
    uintptr_t pairip_onload = 0;
    if (pheap != MAP_FAILED && so_load("libpairipcore.so", pheap, ps) >= 0) {
      extern uintptr_t g_pairip_base; g_pairip_base = (uintptr_t)text_base;
      fprintf(stderr, "[PAIRIP] libpairipcore: text=%p+%zu\n", text_base, text_size);
      /* 🔑 pairip DESCRIPTOGRAFA partes de si MESMO em runtime (loop XOR in-place escrevendo em
       * +0x503e4). Se a seção estiver R-X/R--, o strb FALHA (SIGSEGV). Mapeia TODA a libpairipcore
       * como RWX p/ permitir a auto-descriptografia. */
      { long pg = sysconf(_SC_PAGESIZE);
        uintptr_t a = (uintptr_t)text_base & ~(pg - 1);
        size_t sz = (text_size + 0x20000 + pg) & ~(pg - 1);
        if (mprotect((void *)a, sz, PROT_READ | PROT_WRITE | PROT_EXEC) == 0)
          fprintf(stderr, "[PAIRIP] libpairipcore RWX (%zuB) p/ auto-descript\n", sz);
        else fprintf(stderr, "[PAIRIP] mprotect RWX FALHOU: %s\n", strerror(errno)); }
      so_relocate();
      { extern void recon_fill_passthrough(void); recon_fill_passthrough(); }
      so_resolve(dynlib_functions, dynlib_numfunctions, 0);
      /* 🎯 SELF-INTEGRITY: o pairip usa dl_iterate_phdr p/ achar e hashear a PRÓPRIA lib
       * (anti-tamper). O import dele caía no stub-zero -> achava nada -> count lixo ->
       * over-read no FNV. (1) registra libpairipcore em g_so_mods (contexto = pairip aqui);
       * (2) aponta o GOT do pairip p/ o dl_iterate_phdr REAL (itera g_so_mods + libc). */
      { extern void so_record_phdr(const char *);
        extern int pairip_dl_iterate_phdr(int (*)(struct dl_phdr_info *, size_t, void *), void *);
        extern int so_patch_got(const char *, uintptr_t);
        so_record_phdr("libpairipcore.so");
        int np = so_patch_got("dl_iterate_phdr", (uintptr_t)&pairip_dl_iterate_phdr);
        { extern long pairip_syscall(long,long,long,long,long,long,long);
          int ns = so_patch_got("syscall", (uintptr_t)&pairip_syscall);
          /* pairip lê /proc/self/maps via seu syscall-wrapper INLINE (svc) em +0x153f0
           * (x0=num,x1..x6=args — mesma ABI do nosso pairip_syscall). Patch binário:
           * ldr x16,#8; br x16; .quad &pairip_syscall → todo raw-syscall passa por nós. */
          uint32_t *sw = (uint32_t *)((uintptr_t)text_base + 0x153f0);
          { long pg = sysconf(_SC_PAGESIZE);
            uintptr_t pa = (uintptr_t)sw & ~(pg - 1);
            mprotect((void *)pa, pg * 2, PROT_READ | PROT_WRITE | PROT_EXEC); }
          sw[0] = 0x58000050;   /* ldr x16, pc+8  */
          sw[1] = 0xd61f0200;   /* br x16         */
          *(uint64_t *)(sw + 2) = (uint64_t)(uintptr_t)&pairip_syscall;
          __builtin___clear_cache((char *)sw, (char *)sw + 32);
          fprintf(stderr, "[PAIRIP] syscall GOT=%d + inline svc @+0x153f0 PATCHED\n", ns); }
        if (getenv("ELD_FNVCAP")) {
          /* EXPERIMENTO: limita os loops FNV (cmp w26,w9 → cmp w9,#1024) p/ não estourar o heap
           * no over-read (x26=0xfffffcef). Se a VM seguir, o hash não é checado a sério. */
          long pg = sysconf(_SC_PAGESIZE);
          uintptr_t off[] = {0x58da4, 0x58e20, 0x58eb8};
          for (unsigned k = 0; k < sizeof off/sizeof off[0]; k++) {
            uint32_t *ip = (uint32_t *)((uintptr_t)text_base + off[k]);
            uintptr_t pa = (uintptr_t)ip & ~(pg - 1);
            mprotect((void *)pa, pg * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
            *ip = 0x7110013f;   /* cmp w9, #1024 */
            __builtin___clear_cache((char *)ip, (char *)ip + 4);
          }
          fprintf(stderr, "[PAIRIP] FNV loops CAPADOS (1024)\n");
        }
        { extern void *pairip_opendir(const char *); extern void *pairip_readdir(void *);
          extern int pairip_closedir(void *);
          int a=so_patch_got("opendir",(uintptr_t)&pairip_opendir);
          int b=so_patch_got("readdir",(uintptr_t)&pairip_readdir);
          int c=so_patch_got("closedir",(uintptr_t)&pairip_closedir);
          fprintf(stderr, "[PAIRIP] opendir/readdir/closedir interceptados (%d/%d/%d)\n",a,b,c); }
        fprintf(stderr, "[PAIRIP] registrado em g_so_mods + dl_iterate_phdr real (got patches=%d)\n", np); }
      so_finalize(); so_flush_caches();
      so_execute_init_array();
      so_free_temp();   /* libpairipcore carregado: libera o arquivo cru (so_base global=pairip aqui) */
      pairip_exec = so_find_addr_safe("ExecuteProgram");
      pairip_onload = so_find_addr_safe("JNI_OnLoad");
      fprintf(stderr, "[PAIRIP] ExecuteProgram=0x%lx JNI_OnLoad=0x%lx\n",
              (unsigned long)pairip_exec, (unsigned long)pairip_onload);
    } else fprintf(stderr, "[PAIRIP] FALHOU carregar libpairipcore (heap=%p)\n", pheap);
    so_use(m_unity);  /* volta o contexto p/ libunity */
    if (pairip_exec) {
      patch_got("ExecuteProgram", (void *)pairip_exec);
      fprintf(stderr, "[PAIRIP] ExecuteProgram wired no GOT do libunity\n");
    }
    /* roda o JNI_OnLoad do pairip (o resolver pesado) com nosso jni_shim */
    void *penv_g = NULL;
    if (pairip_onload) {
      jni_shim_set_package("com.pid.elderand", 0);
      void *pvm = NULL, *penv = NULL; jni_shim_init(&pvm, &penv);
      penv_g = penv;
      g_pairip_env = penv;   /* p/ o bridge VMRunner.invoke */
      fprintf(stderr, "[PAIRIP] chamando libpairipcore JNI_OnLoad(vm=%p)...\n", pvm);
      int jr = ((int (*)(void *, void *))pairip_onload)(pvm, NULL);
      fprintf(stderr, "[PAIRIP] pairip JNI_OnLoad = 0x%x\n", jr);
    }
    /* 🎯 NOVO: chama executeVM(byte[] programa, Object[] args) com os BYTES do programa
     * resolvedor lido do disco. O nativo executeVM decifra + roda a VM IAP -> resolve a
     * GOT do libunity. (O prévio só chamava _init, que falha pq o registry está vazio.) */
    {
      extern void *jni_find_native(const char *);
      extern void *jni_make_bytearray(const void *, int);
      void *exec_vm = jni_find_native("executeVM");
      extern int g_vmtrace; if (getenv("ELD_VMTRACE")) g_vmtrace = 1;
      /* 🔑 pairip auto-descriptografa em runtime (XOR in-place em +0x503e4) → precisa RWX.
       * so_finalize já passou (deixou R-X) → re-aplica RWX em TODA a libpairipcore agora. */
      { extern uintptr_t g_pairip_base; long pg = sysconf(_SC_PAGESIZE);
        uintptr_t a = g_pairip_base & ~(pg - 1);
        if (g_pairip_base && mprotect((void *)a, 0x90000, PROT_READ|PROT_WRITE|PROT_EXEC) == 0)
          fprintf(stderr, "[PAIRIP] libpairipcore RWX p/ auto-descript (base=%#lx)\n", (unsigned long)g_pairip_base);
      }
      fprintf(stderr, "[PAIRIP] executeVM nativo = %p\n", exec_vm);
      const char *prog = getenv("ELD_PAIRIP_PROG");
      if (!prog) prog = "iSE2wyMILH8qDvya";
      char path[256];
      const char *adir = getenv("ELD_PAIRIP_ASSETDIR");
      if (!adir) adir = "/storage/roms/ports/elderand/assets";
      snprintf(path, sizeof path, "%s/%s", adir, prog);
      FILE *fp = fopen(path, "rb");
      if (getenv("ELD_PAIRIP_BRIDGE")) { if (fp) fclose(fp); fp = NULL; exec_vm = NULL;
        fprintf(stderr, "[PAIRIP] modo BRIDGE: pulando executeVM direto; _init→VMRunner.invoke dirige\n"); }
      if (exec_vm && fp) {
        fseek(fp, 0, SEEK_END); long fsz = ftell(fp); fseek(fp, 0, SEEK_SET);
        int skip = getenv("ELD_PAIRIP_SKIP") ? atoi(getenv("ELD_PAIRIP_SKIP")) : 0;
        if (skip < 0 || skip > fsz) skip = 0;
        long sz = fsz - skip;
        void *fbuf = malloc(fsz);
        size_t rd = fread(fbuf, 1, fsz, fp); fclose(fp);
        void *buf = (char *)fbuf + skip;
        fprintf(stderr, "[PAIRIP] programa '%s' lido: %zu bytes (magic %02x%02x%02x%02x)\n",
                prog, rd, ((unsigned char*)buf)[0], ((unsigned char*)buf)[1],
                ((unsigned char*)buf)[2], ((unsigned char*)buf)[3]);
        void *jarr = jni_make_bytearray(buf, (int)sz);
        extern int so_patch_lazy_plt(uintptr_t, uintptr_t *, uintptr_t *);
        uintptr_t lz0 = 0, gb0 = 0; int lazy_before = so_patch_lazy_plt(0, &lz0, &gb0);
        fprintf(stderr, "[PAIRIP] chamando executeVM(env=%p, prog '%s'[%ld]); slots lazy ANTES=%d...\n",
                penv_g, prog, sz, lazy_before);
        /* guarda contra over-read da VM: arma o recovery por-init p/ não matar o processo */
        extern sigjmp_buf g_eld_init_jmp; extern volatile int g_eld_init_armed;
        void *r = NULL; int vm_crashed = 0;
        if (sigsetjmp(g_eld_init_jmp, 1)) { vm_crashed = 1; g_eld_init_armed = 0; }
        else {
          g_eld_init_armed = 1;
          extern long g_pairip_args_sentinel;
          void *vmargs = getenv("ELD_NULLARGS") ? NULL : (void *)&g_pairip_args_sentinel;
          r = ((void *(*)(void *, void *, void *, void *))exec_vm)(penv_g, NULL, jarr, vmargs);
          g_eld_init_armed = 0;
        }
        int lazy_after = so_patch_lazy_plt(0, &lz0, &gb0);
        fprintf(stderr, "[PAIRIP] executeVM ret=%p crashed=%d. slots lazy: %d -> %d (RESOLVIDOS=%d) %s\n",
                r, vm_crashed, lazy_before, lazy_after, lazy_before - lazy_after,
                lazy_after < lazy_before ? "✅✅ RESOLVEU SLOTS!" : "(nenhum)");
        fflush(stderr);
        FILE *rf = fopen("/dev/shm/eld_pairip_result.txt", "w");
        if (rf) { fprintf(rf, "prog=%s ret=%p crashed=%d lazy %d->%d RESOLVIDOS=%d\n",
                          prog, r, vm_crashed, lazy_before, lazy_after, lazy_before - lazy_after);
                  fclose(rf); }
        free(fbuf);
      } else {
        fprintf(stderr, "[PAIRIP] SKIP executeVM (exec_vm=%p fp=%p path=%s)\n",
                exec_vm, (void *)fp, path);
        if (fp) fclose(fp);
      }
    }
    void (*initfn)(int, char **, char **) = (void *)((uintptr_t)text_base + 0x149c000);
    char *av[] = { (char *)"elderand", NULL }, *ev[] = { NULL };
    fprintf(stderr, "[PAIRIP] chamando _init (roda a VM ExecuteProgram)...\n");
    initfn(1, av, ev);
    unsigned long slot_depois = *(unsigned long *)((uintptr_t)text_base + 0x1385210);
    fprintf(stderr, "[PAIRIP] _init OK. slot 0x1385210: 0x%lx -> 0x%lx %s\n",
            slot_antes, slot_depois, slot_depois != slot_antes ? "✅ RESOLVIDO!" : "❌ inalterado");
    { extern int so_patch_lazy_plt(uintptr_t, uintptr_t *, uintptr_t *);
      uintptr_t lz = 0, gb = 0; int nlz = so_patch_lazy_plt(0, &lz, &gb);  /* handler=0: só conta */
      fprintf(stderr, "[PAIRIP] slots ainda lazy (==0xbf7c0): %d (eram ~177)\n", nlz); }
  }
  /* ELD_NOFATAL (default ON): a FatalError do Unity (f7c354) termina em brk@0x452484.
   * Inits pulados (ELD_INITGUARD) deixam guards "in progress" -> recursive-init fatal.
   * Patcha o brk -> ret: FatalError loga e VOLTA em vez de abortar. */
  if (!getenv("ELD_NOFATAL_OFF")) {
    extern void so_make_text_writable(void), so_make_text_executable(void);
    so_make_text_writable();
    *(uint32_t *)((uintptr_t)text_base + 0x452484) = 0xd65f03c0u; /* brk -> ret */
    *(uint32_t *)((uintptr_t)text_base + 0xf7c354) = 0xd65f03c0u; /* FatalError-logger -> ret (não formata/crasha) */
    so_make_text_executable(); so_flush_caches();
  ...
  if (onload) {
    extern sigjmp_buf g_eld_init_jmp; extern volatile int g_eld_init_armed;
    int ver = 0x10006;
    if (getenv("ELD_INITGUARD") && sigsetjmp(g_eld_init_jmp, 1)) {
      fprintf(stderr, "[F0] JNI_OnLoad CRASHOU -> seguindo mesmo assim\n");
      g_eld_init_armed = 0;
    } else {
      if (getenv("ELD_INITGUARD")) g_eld_init_armed = 1;
      ver = ((int (*)(void *, void *))onload)(vm, NULL);
      g_eld_init_armed = 0;
    }
    fprintf(stderr, "[F0] JNI_OnLoad = 0x%x\n", ver);
    /* JNI_OnLoad chama 10 RegisterNatives em sequência; se uma crasha, aborta as
     * outras (incl. a do nativeRender). Chama CADA uma individualmente com recovery
     * usando nosso env -> as que funcionam registram os natives. */
    if (getenv("ELD_REGNATIVES")) {
      extern sigjmp_buf g_eld_init_jmp; extern volatile int g_eld_init_armed;
      static const uintptr_t regfns[] = { 0x453138, 0x4531a8, 0x453218, 0x450600,
        0x421abc, 0x44c588, 0x44ec0c, 0x437588, 0x42b744, 0x452118 };
      for (unsigned i = 0; i < sizeof regfns/sizeof regfns[0]; i++) {
        void (*rf)(void *) = (void *)((uintptr_t)text_base + regfns[i]);
        if (sigsetjmp(g_eld_init_jmp, 1)) { g_eld_init_armed = 0;
          fprintf(stderr, "[REGNAT] regfn[%u]@0x%lx CRASHOU -> pulado\n", i, regfns[i]); continue; }
        g_eld_init_armed = 1; rf(env); g_eld_init_armed = 0;
        fprintf(stderr, "[REGNAT] regfn[%u]@0x%lx OK\n", i, regfns[i]);
      }
    }
  } else {
  ...
  patch_got("__FD_SET_chk", (void *)my_FD_SET_chk);
    patch_got("dlopen", (void *)my_dlopen);
    patch_got("dlsym", (void *)my_dlsym);
    /* 🔑 sysconf: il2cpp importa sysconf p/ Environment.ProcessorCount. Sem patch cai no
     * glibc REAL → constante BIONIC _SC_NPROCESSORS_ONLN(97) vira outra coisa no glibc →
     * retorna 0 → ConcurrentDictionary(concurrencyLevel=0) lança → quebra serialização
     * (NodeCanvas/ParadoxNotion). my_sysconf devolve 4 p/ 96/97 (bionic). */
    { extern long my_sysconf(int); patch_got("sysconf", (void *)my_sysconf); }
    patch_got("pthread_key_create", (void *)sh_key_create);  /* mesmo shim TLS do libunity */
    patch_got("pthread_key_delete", (void *)sh_key_delete);
    patch_got("pthread_getspecific", (void *)sh_getspecific);
    patch_got("pthread_setspecific", (void *)sh_setspecific);
    patch_got("exit", (void *)my_exit);
    ...
    patch_pthread_shim();
    eld_wire_stdio_patchgot();  /* stdio __sF -> map_sf (F1 libil2cpp) */
  ...
  /* 🧹 RAM: libera as CÓPIAS do arquivo .so cru (so_base) de cada módulo já carregado.
   * libil2cpp=70MB + libunity=21MB ficavam em malloc o run inteiro sem uso (só servem
   * em tempo de load: sec_hdr/shstrtab; em runtime os símbolos estão na imagem text_base
   * e o phdr foi copiado p/ g_so_mods). Corta ~91MB de RSS -> elimina o swap (era ~144MB)
   * -> boot determinístico. (s10: hipótese RAM do NextOS.) */
  { size_t freed = 0;
    extern int malloc_trim(size_t) __attribute__((weak));
    rss_probe("antes-free-sobase");
    if (g_m_unity)  { so_free_module_temp(g_m_unity);  freed += 21; }
    if (g_m_il2cpp) { so_free_module_temp(g_m_il2cpp); freed += 70; }
    if (malloc_trim) malloc_trim(0);   /* devolve o heap liberado ao kernel (glibc) */
    fprintf(stderr, "[RAM] so_base liberado (~%zuMB) -> RSS menor, menos swap\n", freed);
    rss_probe("apos-free-sobase");
  }
  ...
  void *fn;
  /* ELD_FAKESINGLETON: globais de singleton ficam null (init que os seta foi pulado).
   * Popula com objeto falso (vtable no-op) p/ os getters não retornarem null e o
   * initJni/GfxDevice seguir. Lista cresce conforme descobrimos os crashes. */
  if (getenv("ELD_FAKESINGLETON")) {
    extern long lazy_noop_fn(long, long, long, long);
    static const uintptr_t singles[] = { 0x140f7e0 };
    for (unsigned i = 0; i < sizeof singles/sizeof singles[0]; i++) {
      uintptr_t *g = (uintptr_t *)((uintptr_t)text_base + singles[i]);
      if (!*g) {
        uintptr_t *obj = (uintptr_t *)calloc(1, 65536);
        uintptr_t *vt = (uintptr_t *)calloc(256, 8);
        if (vt) for (int k = 0; k < 256; k++) vt[k] = (uintptr_t)lazy_noop_fn;
        if (obj) { obj[0] = (uintptr_t)vt; *g = (uintptr_t)obj; }
        fprintf(stderr, "[FAKESGL] singleton 0x%lx populado=%p\n", singles[i], (void *)*g);
      }
    }
  }
  /* força a janela fbdev (initJni que chama fromSurface pode crashar antes de setá-la) */
  if (g_fbdev_win.w == 0) {
    const char *ew = getenv("TER_SCREEN_W"), *eh = getenv("TER_SCREEN_H");
    g_fbdev_win.w = (unsigned short)(ew ? atoi(ew) : 1280);
    g_fbdev_win.h = (unsigned short)(eh ? atoi(eh) : 720);
    if (!g_fbdev_win.w) g_fbdev_win.w = 1280;
    if (!g_fbdev_win.h) g_fbdev_win.h = 720;
    fprintf(stderr, "[F2] janela fbdev forçada %ux%u\n", g_fbdev_win.w, g_fbdev_win.h);
  }
  if ((fn = jni_find_native("initJni"))) {
    extern sigjmp_buf g_eld_init_jmp; extern volatile int g_eld_init_armed;
    fprintf(stderr, "[F2] initJni...\n");
    if (getenv("ELD_INITGUARD") && sigsetjmp(g_eld_init_jmp, 1)) {
      g_eld_init_armed = 0; fprintf(stderr, "[F2] initJni CRASHOU -> seguindo\n");
    } else {
      if (getenv("ELD_INITGUARD")) g_eld_init_armed = 1;
  ...
  if ((fn = jni_find_native("nativeRecreateGfxState"))) {
    ...
    { extern sigjmp_buf g_eld_init_jmp; extern volatile int g_eld_init_armed;
      if (getenv("ELD_INITGUARD") && sigsetjmp(g_eld_init_jmp, 1)) {
        g_eld_init_armed = 0; fprintf(stderr, "[F2] RecreateGfxState CRASHOU -> seguindo\n");
      } else {
        if (getenv("ELD_INITGUARD")) g_eld_init_armed = 1;
  ...
  /* ELD_CHOREO_MAIN: dirige doFrame (game logic) da PRÓPRIA thread main (já anexada ao
   * il2cpp + thread do contexto GL) em vez da thread choreo separada — evita o crash do
   * GC scan cross-thread (il2cpp+0xcea55c) que a thread choreo dispara. */
  extern int jni_choreo_doframe(void *env, long nanos);
  extern int jni_choreo_captured(void);
  int choreo_main = getenv("ELD_CHOREO_MAIN") ? 1 : 0;
  if (choreo_main) fprintf(stderr, "[CHOREO_MAIN] doFrame dirigido da thread main\n");
  /* ELD_GCOFF: desabilita o GC do il2cpp durante o boot (resolve il2cpp_gc_disable por
   * NOME — offset 1.3.22=0xcdcde0). A thread choreo dirige doFrame → Update aloca → GC
   * stop-the-world escaneia threads e crasha (il2cpp+0xcea55c). Sem GC no boot, sem scan. */
  if (getenv("ELD_GCOFF") && g_m_il2cpp) {
    so_module *cm = so_save(); so_use(g_m_il2cpp);
    void (*gc_disable)(void) = (void (*)(void))so_find_addr_safe("il2cpp_gc_disable");
    so_use(cm);
