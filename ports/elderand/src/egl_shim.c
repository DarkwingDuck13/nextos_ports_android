...
/* 🔑 TEARDOWN DE EMERGÊNCIA p/ NÃO TRAVAR O DEVICE (Mali-450 Utgard fbdev): se o processo morre
   com o contexto GL vivo / GPU não-idle, o driver Mali trava o display → "tela preta + reboot" →
   sshd morto (lição LCS/Bully). Chamado pelo watchdog ANTES do exit_group. Thread-agnóstico,
   best-effort: solta o contexto real, termina o EGL real, e deixa o SDL liberar o fbdev (SDL sabe
   o estado completo). Tudo via dlsym/SDL — seguro mesmo se a render-thread está travada. */
void egl_shim_emergency_teardown(void) {
  static int once = 0; if (once) return; once = 1;
  if (r_eglMakeCurrent && g_real_dpy) r_eglMakeCurrent(g_real_dpy, NULL, NULL, NULL);
  void (*r_term)(void *) = (void (*)(void *))dlsym(RTLD_DEFAULT, "eglTerminate");
  if (r_term && g_real_dpy) r_term(g_real_dpy);
  SDL_QuitSubSystem(SDL_INIT_VIDEO);   /* SDL destrói window+ctx+fbdev internamente */
}
