...
static const char *g_package_name = "jp.co.capcom.rxdoff";
...
static int ter_env_on_jni(const char *name) {
  const char *s = getenv(name);
  if (!s || !*s) return 0;
  if (!strcmp(s, "0") || !strcasecmp(s, "false") || !strcasecmp(s, "no") ||
      !strcasecmp(s, "off")) return 0;
  return 1;
}

static int ter_env_int_default_jni(const char *name, int def) {
  const char *s = getenv(name);
  if (!s || !*s) return def;
  char *end = NULL;
  long v = strtol(s, &end, 10);
...
static void *make_jstring(const char *value) {
  char *s = strdup(value ? value : "");
  if (g_jstring_count < MAX_JSTRINGS) {
    g_jstrings[g_jstring_count].handle = s;
    g_jstrings[g_jstring_count].value = s;
    g_jstring_count++;
  }
  return (void *)s;
}
...
static int g_empty_obj_array;           /* Object[0] para APIs Android opcionais */
...
#include <stdlib.h>
#include <stdarg.h>
#define ASSET_BASE "/storage/roms/ports/rockmanxdive/"
...
static void *jstring_from_barr(void *arr, int off, int len) {
  struct barr *b = barr_find(arr);
  if (!b || !b->buf) return make_jstring("");
  if (off < 0) off = 0;
  if (off > b->len) off = b->len;
  if (len < 0 || off + len > b->len) len = b->len - off;
  char *tmp = (char *)malloc((size_t)len + 1);
  if (!tmp) return make_jstring("");
  if (len > 0) memcpy(tmp, b->buf + off, (size_t)len);
  tmp[len] = 0;
  void *r = make_jstring(tmp);
  free(tmp);
  return r;
}
...
void *jni_AAssetManager_fromJava(void *env, void *assetManager) {
  (void)env; (void)assetManager;
  return &g_assetmgr;
}

void *jni_AAssetManager_open(void *mgr, const char *path, int mode) {
  (void)mgr; (void)mode;
  return asset_open(path);
}

long jni_AAsset_getLength64(void *asset) {
  struct astream *s = astream_find(asset);
  return s ? s->size : 0;
}

int jni_AAsset_read(void *asset, void *buf, unsigned long count) {
  struct astream *s = astream_find(asset);
  if (!s || !s->fp || !buf) return -1;
  size_t n = fread(buf, 1, count, s->fp);
  return n > 0 ? (int)n : 0;
}

long jni_AAsset_seek(void *asset, long offset, int whence) {
  struct astream *s = astream_find(asset);
  if (!s || !s->fp) return -1;
  if (fseek(s->fp, offset, whence) != 0) return -1;
  return ftell(s->fp);
}

int jni_AAsset_openFileDescriptor64(void *asset, long *start, long *length) {
  struct astream *s = astream_find(asset);
  if (!s || !s->fp) return -1;
  if (start) *start = 0;
  if (length) *length = s->size;
  return dup(fileno(s->fp));
}

void jni_AAsset_close(void *asset) {
  struct astream *s = astream_find(asset);
...
static int g_handler_thread_obj, g_handler_obj, g_looper_obj, g_choreographer_obj;
static int g_locale_obj;
...
static const char *class_name_of(void *clazz) {
  for (int i = 0; i < g_classreg_n; i++)
    if ((void *)&g_classreg[i].tag == clazz) return g_classreg[i].name;
  return "?";
}
...
static void *jni_FromReflectedMethod(void *env, void *method) {
  ...
  if (method == &g_run_method_sentinel ||
      method == &g_doframe_method_sentinel ||
      method == &g_handlemsg_method_sentinel) {
    debugPrintf("jni_shim: FromReflectedMethod(%s sentinel)\n",
                method == &g_run_method_sentinel ? "run" :
                method == &g_doframe_method_sentinel ? "doFrame" : "handleMessage");
  }
  return method;
}
...
static void *jni_ToReflectedMethod(void *env, void *clazz, void *methodID, unsigned char isStatic) {
  ...
  (void)clazz;
  (void)isStatic;
  if (methodID == &g_run_method_sentinel ||
      methodID == &g_doframe_method_sentinel ||
      methodID == &g_handlemsg_method_sentinel) {
    debugPrintf("jni_shim: ToReflectedMethod(%s sentinel)\n",
                methodID == &g_run_method_sentinel ? "run" :
                methodID == &g_doframe_method_sentinel ? "doFrame" : "handleMessage");
  }
  return methodID;
}
...
static void *jni_CallObjectMethodV(void *env, void *obj, void *methodID,
  ...
  if (nm) {
    ...
    if (obj == (void *)&g_locale_obj) {
      if (strcmp(nm, "toLanguageTag") == 0) return make_jstring("en-US");
      if (strcmp(nm, "getLanguage") == 0) return make_jstring("en");
      if (strcmp(nm, "getCountry") == 0) return make_jstring("US");
    }
    ...
    if (strcmp(nm, "getAssetPackPath") == 0)
      return make_jstring("/storage/roms/ports/rockmanxdive");
    ...
    if (jni_is_run_method(obj)) {
      ...
      if (strcmp(nm, "toString") == 0) return make_jstring("public abstract void java.lang.Runnable.run()");
    ...
    if (obj == (void *)&g_doframe_method_sentinel) {
      ...
      if (strcmp(nm, "toString") == 0) return make_jstring("public abstract void android.view.Choreographer$FrameCallback.doFrame(long)");
    ...
    if (obj == (void *)&g_handlemsg_method_sentinel) {
      ...
      if (strcmp(nm, "toString") == 0) return make_jstring("public abstract boolean android.os.Handler$Callback.handleMessage(android.os.Message)");
    ...
    if (strcmp(nm, "getLooper") == 0) {
      debugPrintf("jni_shim: HandlerThread.getLooper -> Looper sentinel\n");
      return &g_looper_obj;
    }
    ...
    if (strcmp(nm, "findLibrary") == 0) {
      ...
      if (ln && strstr(ln, "cri_ware_unity"))
        return make_jstring(ASSET_BASE "libcri_ware_unity.so");
      if (ln && strstr(ln, "burst_generated"))
        return make_jstring(ASSET_BASE "lib_burst_generated.so");
      if (ln && strstr(ln, "pairipcore"))
        return make_jstring(ASSET_BASE "libpairipcore.so");
      if (ln && strstr(ln, "stub"))
        return make_jstring(ASSET_BASE "libstub.so");
    ...
    if (strcmp(nm, "getBytes") == 0) {
      ...
      debugPrintf("jni_shim: String.getBytes -> byte[%zu]\n", strlen(s));
      return barr_from_bytes((const unsigned char *)s, (int)strlen(s));
    }
    /* AssetManager bridge */
    ...
    if (strcmp(nm, "putString") == 0) {
      ...
      if (key && strcmp(key, "Console_L10N_key") == 0 && (!val || !val[0]))
        val = getenv("TER_RXD_LANG") ? getenv("TER_RXD_LANG") : "English";
    ...
    if (strcmp(nm, "getObbDirs") == 0) {
      debugPrintf("jni_shim: getObbDirs -> Object[0]\n");
      return &g_empty_obj_array;
    }
    if (strcmp(nm, "getObbDir") == 0) {
      debugPrintf("jni_shim: getObbDir -> NULL (dados extraidos)\n");
      return NULL;
    }
    ...
    if (strcmp(nm, "getFilesDir") == 0 || strcmp(nm, "getExternalFilesDir") == 0 ||
      ...
      return make_jstring("/storage/roms/ports/rockmanxdive/userdata");
    ...
    if (strcmp(nm, "getString") == 0) {
      ...
      if (sig && strcmp(sig, "(Ljava/lang/String;)Ljava/lang/String;") == 0) {
        if (key && strcmp(key, "unity.build-id") == 0) {
          debugPrintf("[BUNDLE] getString(unity.build-id) -> \"\"\n");
          return make_jstring("");
        }
        const char *stored1 = prefs_get_string(key);
        debugPrintf("[BUNDLE] getString key='%s' -> %s\n", key, stored1 ? "ARMAZENADO" : "vazio");
        return make_jstring(stored1 ? stored1 : "");
      }
      ...
      if (!stored && key && strcmp(key, "Console_L10N_key") == 0) {
        const char *lang = getenv("TER_RXD_LANG") ? getenv("TER_RXD_LANG") : "English";
        debugPrintf("[PREFS] getString key='Console_L10N_key' -> default %s\n", lang);
        return make_jstring(lang);
      }
...
static void *jni_CallObjectMethodA(void *env, void *obj, void *methodID, const jvalue *args) {
  ...
  if (nm && jni_is_run_method(obj)) {
    if (strcmp(nm, "getName") == 0) return make_jstring("run");
    if (strcmp(nm, "toString") == 0) return make_jstring("public abstract void java.lang.Runnable.run()");
    return &g_run_method_sentinel;
  }
  if (nm && obj == (void *)&g_doframe_method_sentinel) {
    if (strcmp(nm, "getName") == 0) return make_jstring("doFrame");
    if (strcmp(nm, "toString") == 0) return make_jstring("public abstract void android.view.Choreographer$FrameCallback.doFrame(long)");
    return &g_doframe_method_sentinel;
  }
  if (nm && obj == (void *)&g_handlemsg_method_sentinel) {
    if (strcmp(nm, "getName") == 0) return make_jstring("handleMessage");
    if (strcmp(nm, "toString") == 0) return make_jstring("public abstract boolean android.os.Handler$Callback.handleMessage(android.os.Message)");
    return &g_handlemsg_method_sentinel;
  }
  ...
  if (nm && strcmp(nm, "getLooper") == 0) {
    debugPrintf("jni_shim: HandlerThread.getLooperA -> Looper sentinel\n");
    return &g_looper_obj;
  }
  if (nm && obj == (void *)&g_locale_obj) {
    if (strcmp(nm, "toLanguageTag") == 0) return make_jstring("en-US");
    if (strcmp(nm, "getLanguage") == 0) return make_jstring("en");
    if (strcmp(nm, "getCountry") == 0) return make_jstring("US");
  }
  if (nm && strcmp(nm, "getObbDirs") == 0) {
    debugPrintf("jni_shim: getObbDirsA -> Object[0]\n");
    return &g_empty_obj_array;
  }
  if (nm && strcmp(nm, "getObbDir") == 0) {
    debugPrintf("jni_shim: getObbDirA -> NULL (dados extraidos)\n");
    return NULL;
  }
  if (nm && strcmp(nm, "getString") == 0) {
    ...
    if (sig && strcmp(sig, "(Ljava/lang/String;)Ljava/lang/String;") == 0) {
      if (key && strcmp(key, "unity.build-id") == 0) {
        debugPrintf("[BUNDLE] getStringA(unity.build-id) -> \"\"\n");
        return make_jstring("");
      }
      const char *stored = prefs_get_string(key);
      return make_jstring(stored ? stored : "");
    }
    const char *stored = prefs_get_string(key);
    if (!stored && key && strcmp(key, "Console_L10N_key") == 0) {
      const char *lang = getenv("TER_RXD_LANG") ? getenv("TER_RXD_LANG") : "English";
    ...
    if (stored) return make_jstring(stored);
    return args && args[1].l ? args[1].l : make_jstring("");
  }
...
static jint jni_CallIntMethodA(void *env, void *obj, void *methodID,
  ...
  if (nm) {
    ...
    if (obj == (void *)&g_gamepad_device) {
      ...
      if (strcmp(nm, "getSources") == 0)         return 0x1000611;
      if (strcmp(nm, "getId") == 0)              return 1;
      if (strcmp(nm, "getControllerNumber") == 0) return 1;
      if (strcmp(nm, "getKeyboardType") == 0)    return 0;
      if (strcmp(nm, "supportsSource") == 0)     return 1;
    ...
    if (strcmp(nm, "getRepeatCount") == 0) return g_hk_inject.repeat;
    if (strcmp(nm, "getScanCode") == 0) return g_hk_inject.scancode;
    if (strcmp(nm, "getInt") == 0 && args) {
      const char *key = resolve_jstring(args[0].l);
      int d = args[1].i;
      int i = prefs_find(key);
      int v = (i >= 0 && g_prefs[i].has_i) ? g_prefs[i].ival : d;
...
static void unity_pausejava_and_unload(void *env, void *obj) {
  int on = ter_env_on_jni("TER_RXD_UNLOAD_CALLBACK") || ter_env_on_jni("TER_UNITY_UNLOAD_CALLBACK");
  void *pause = jni_find_native("nativePause");
  void *focus = jni_find_native("nativeFocusChanged");
  void *done = jni_find_native("nativeDone");
  int paused = 0, done_ret = -1;
  int f = g_render_frame;
  int from = ter_env_int_default_jni("TER_RXD_UNLOAD_FROM", -1);
  int until = ter_env_int_default_jni("TER_RXD_UNLOAD_UNTIL", -1);
  int in_window = on && (from < 0 || f >= from) && (until < 0 || f <= until);
  int focus_until = ter_env_int_default_jni("TER_RXD_UNLOAD_FOCUS_UNTIL", until);
  int focus_from = ter_env_int_default_jni("TER_RXD_UNLOAD_FOCUS_FROM", from);
  int focus_off = in_window && focus &&
                  (focus_from < 0 || f >= focus_from) &&
                  (focus_until < 0 || f <= focus_until) &&
                  !ter_env_on_jni("TER_RXD_UNLOAD_NO_FOCUS");
  int keep_focus_from = ter_env_int_default_jni("TER_RXD_UNLOAD_KEEP_FOCUS_FROM", -1);
  if (in_window && pause) paused = ((int (*)(void *, void *))pause)(env, obj);
  if (focus_off) ((void (*)(void *, void *, int))focus)(env, obj, 0);
  if (on && focus && keep_focus_from >= 0 && f >= keep_focus_from)
    ((void (*)(void *, void *, int))focus)(env, obj, 1);
  if (in_window && paused && done && !ter_env_on_jni("TER_RXD_UNLOAD_NO_DONE"))
    done_ret = ((int (*)(void *, void *))done)(env, obj);
  static unsigned n;
  if (n++ < 48 || (n % 120) == 0) {
    debugPrintf("jni_shim: pauseJavaAndCallUnloadCallback -> pause=%p ret=%d focus=%p done=%p done_ret=%d on=%d win=%d from=%d until=%d f=%d\n",
                pause, paused, focus, done, done_ret, on ? 1 : 0, in_window ? 1 : 0,
                from, until, f);
  }
}
...
static void jni_CallVoidMethodV(void *env, void *obj, void *methodID, va_list ap) {
  ...
  if (nm && strcmp(nm, "start") == 0 && obj == (void *)&g_handler_thread_obj) {
    debugPrintf("jni_shim: HandlerThread.start -> no-op\n");
    return;
  }
  if (nm && (strcmp(nm, "postFrameCallback") == 0 || strcmp(nm, "postFrameCallbackDelayed") == 0)) {
    debugPrintf("jni_shim: Choreographer.%s -> driver-thread assume frames\n", nm);
    return;
  }
  if (nm && strcmp(nm, "pauseJavaAndCallUnloadCallback") == 0) {
    unity_pausejava_and_unload(env, obj);
    return;
  }
...
static void jni_CallVoidMethodA(void *env, void *obj, void *methodID, const jvalue *args) {
  ...
  if (nm && strcmp(nm, "start") == 0 && obj == (void *)&g_handler_thread_obj) {
    debugPrintf("jni_shim: HandlerThread.startA -> no-op\n");
    return;
  }
  if (nm && (strcmp(nm, "postFrameCallback") == 0 || strcmp(nm, "postFrameCallbackDelayed") == 0)) {
    debugPrintf("jni_shim: Choreographer.%sA -> driver-thread assume frames\n", nm);
    return;
  }
  if (nm && strcmp(nm, "pauseJavaAndCallUnloadCallback") == 0) {
    unity_pausejava_and_unload(env, obj);
...
static void *jni_CallStaticObjectMethodV(void *env, void *clazz,
  ...
  if (nm && !strcmp(nm, "getInstance")) {
    debugPrintf("jni_shim: Choreographer.getInstance -> Choreographer sentinel\n");
    return &g_choreographer_obj;
  }
  if (nm && !strcmp(nm, "getDefault")) {
    debugPrintf("jni_shim: Locale.getDefault -> en-US\n");
    return &g_locale_obj;
  }
...
static void *jni_CallStaticObjectMethodA(void *env, void *clazz, void *methodID, const jvalue *args) {
  ...
  const char *nm = mid_name(methodID);
  if (nm && !strcmp(nm, "getInstance")) {
    debugPrintf("jni_shim: Choreographer.getInstanceA -> Choreographer sentinel\n");
    return &g_choreographer_obj;
  }
  if (nm && !strcmp(nm, "getDefault")) {
    debugPrintf("jni_shim: Locale.getDefaultA -> en-US\n");
    return &g_locale_obj;
  }
  if (nm && !strcmp(nm, "newInterfaceProxy")) {
    long h = args ? (long)args[0].j : 0;
    void *proxy = malloc(16);
    proxy_register(proxy, h);
    ...
    if (g_next_proxy_is_framecb) {
      g_framecb_proxy = proxy;
      g_next_proxy_is_framecb = 0;
      debugPrintf("jni_shim: [CHOREO] FrameCallback capturado(A): proxy=%p handle=%ld\n", proxy, h);
    }
    return proxy;
  }
  if (getenv("TER_KBFIX") && nm && !strcmp(nm, "getFieldID")) {
...
static void jni_CallStaticVoidMethodA(void *env, void *clazz, void *methodID,
  ...
  (void)clazz;
  (void)args;
  debugPrintf("jni_shim: CallStaticVoidMethodA(%s)\n", mid_name(methodID) ? mid_name(methodID) : "?");
}
...
static void *jni_GetStaticObjectField(void *env, void *clazz, void *fieldID) {
  ...
  if (nm && strcmp(nm, "MEDIA_MOUNTED") == 0)
...
static jint jni_ThrowNew(void *env, void *clazz, const char *msg) {
  (void)env; (void)clazz;
  debugPrintf("jni_shim: ThrowNew(%s) ignorado\n", msg ? msg : "");
  return 0;
}
static jint jni_PushLocalFrame(void *env, jint capacity) {
...
static jint jni_EnsureLocalCapacity(void *env, jint capacity) {
  (void)env; (void)capacity;
  return 0;
}
static void jni_SetObjectField(void *env, void *obj, void *fieldID, void *val) {
  (void)env; (void)obj; (void)fieldID; (void)val;
}
static void jni_SetIntField(void *env, void *obj, void *fieldID, jint val) {
  (void)env; (void)obj; (void)fieldID; (void)val;
}
static void jni_SetFloatField(void *env, void *obj, void *fieldID, float val) {
  (void)env; (void)obj; (void)fieldID; (void)val;
}

static void *jni_NewString(void *env, const uint16_t *chars, jint len) {
  (void)env;
  if (len < 0) len = 0;
  int cap = len < 1023 ? len : 1023;
  char tmp[1024];
  for (int i = 0; i < cap; i++) {
    uint16_t c = chars ? chars[i] : 0;
...
static const uint16_t *jni_GetStringChars(void *env, void *jstr, void *isCopy) {
  (void)env;
  if (isCopy) *(jboolean *)isCopy = 1;
  static _Thread_local uint16_t buf[1024];
  const char *s = resolve_jstring(jstr);
  int i = 0;
  for (; s && s[i] && i < 1023; i++) buf[i] = (uint16_t)(unsigned char)s[i];
  buf[i] = 0;
  return buf;
}
static void jni_ReleaseStringChars(void *env, void *jstr, const uint16_t *chars) {
...
static struct objarr g_objarrs[128];
static int g_objarr_n;
static struct objarr *objarr_find(void *array) {
  for (int i = 0; i < g_objarr_n; i++)
    if ((void *)&g_objarrs[i].tag == array) return &g_objarrs[i];
  return NULL;
}
static void *jni_NewObjectArray(void *env, jint len, void *clazz, void *init) {
  (void)env; (void)clazz;
  if (len < 0) len = 0;
  if (g_objarr_n >= (int)(sizeof g_objarrs / sizeof g_objarrs[0])) g_objarr_n = 0;
  struct objarr *a = &g_objarrs[g_objarr_n++];
  a->len = len;
  a->vals = len ? calloc((size_t)len, sizeof(void *)) : NULL;
  for (int i = 0; i < len; i++) a->vals[i] = init;
  debugPrintf("jni_shim: NewObjectArray(len=%d init=%p) -> %p\n", len, init, &a->tag);
  return &a->tag;
}
static jint jni_GetArrayLength(void *env, void *array) {
  (void)env;
  if (array == (void *)&g_empty_obj_array) return 0;
  ...
  struct objarr *oa = objarr_find(array);
  if (oa) return oa->len;
...
static void *jni_GetObjectArrayElement(void *env, void *array, jint idx) {
  ...
  struct objarr *oa = objarr_find(array);
  if (oa && idx >= 0 && idx < oa->len) return oa->vals[idx];
  return NULL;
}
static void jni_SetObjectArrayElement(void *env, void *array, jint idx, void *val) {
  (void)env;
  struct objarr *oa = objarr_find(array);
  if (oa && idx >= 0 && idx < oa->len) oa->vals[idx] = val;
}
...
long jni_choreo_handle(void) {
  void *proxy = g_framecb_proxy;
  return proxy ? proxy_handle(proxy) : 0;
}
...
static void *jni_NewObject_common(void *env, void *clazz, void *mid) {
  (void)env; (void)mid;
  const char *cn = class_name_of(clazz);
  if (cn && strstr(cn, "org/fmod/FMODAudioDevice")) {
    debugPrintf("jni_shim: NewObject(FMODAudioDevice) -> device fake\n"); return &g_fmod_device_obj;
  }
  if (cn && strstr(cn, "android/os/HandlerThread")) {
    debugPrintf("jni_shim: NewObject(HandlerThread) -> HandlerThread sentinel\n");
    return &g_handler_thread_obj;
  }
  if (cn && strstr(cn, "android/os/Handler")) {
    debugPrintf("jni_shim: NewObject(Handler) -> Handler sentinel\n");
    return &g_handler_obj;
  }
  if (cn && strstr(cn, "android/os/Message")) {
    debugPrintf("jni_shim: NewObject(Message) -> Message sentinel\n");
    return &g_message_sentinel;
  }
  static int newobj_log;
  if (newobj_log++ < 40) debugPrintf("jni_shim: NewObject(%s) -> NULL\n", cn ? cn : "?");
  return NULL;   /* comportamento atual (jni_stub=0) p/ as demais classes — sem regressão */
}
static void *jni_NewStringObjectV(void *clazz, void *mid, va_list ap) {
  const char *cn = class_name_of(clazz);
  const char *sig = mid_sig(mid);
  if (!cn || !strstr(cn, "java/lang/String")) return NULL;
  if (sig && strcmp(sig, "([BLjava/lang/String;)V") == 0) {
    void *bytes = va_arg(ap, void *);
    (void)va_arg(ap, void *); /* charset name */
    void *s = jstring_from_barr(bytes, 0, -1);
    debugPrintf("jni_shim: NewObject(java/lang/String [B,charset) -> \"%s\"\n", resolve_jstring(s));
    return s;
  }
  if (sig && strcmp(sig, "([B)V") == 0) {
    void *bytes = va_arg(ap, void *);
    void *s = jstring_from_barr(bytes, 0, -1);
    debugPrintf("jni_shim: NewObject(java/lang/String [B) -> \"%s\"\n", resolve_jstring(s));
    return s;
  }
  if (sig && strcmp(sig, "([BII)V") == 0) {
    void *bytes = va_arg(ap, void *);
    int off = va_arg(ap, int);
    int len = va_arg(ap, int);
    void *s = jstring_from_barr(bytes, off, len);
    debugPrintf("jni_shim: NewObject(java/lang/String [B,%d,%d) -> \"%s\"\n", off, len, resolve_jstring(s));
    return s;
  }
  if (sig && strcmp(sig, "(Ljava/lang/String;)V") == 0) {
    void *src = va_arg(ap, void *);
    return make_jstring(resolve_jstring(src));
  }
  return NULL;
}
static void *jni_NewStringObjectA(void *clazz, void *mid, const jvalue *args) {
  const char *cn = class_name_of(clazz);
  const char *sig = mid_sig(mid);
  if (!cn || !strstr(cn, "java/lang/String")) return NULL;
  if (sig && strcmp(sig, "([BLjava/lang/String;)V") == 0) {
    void *s = jstring_from_barr(args ? args[0].l : NULL, 0, -1);
    debugPrintf("jni_shim: NewObjectA(java/lang/String [B,charset) -> \"%s\"\n", resolve_jstring(s));
    return s;
  }
  if (sig && strcmp(sig, "([B)V") == 0) {
    void *s = jstring_from_barr(args ? args[0].l : NULL, 0, -1);
    debugPrintf("jni_shim: NewObjectA(java/lang/String [B) -> \"%s\"\n", resolve_jstring(s));
    return s;
  }
  if (sig && strcmp(sig, "([BII)V") == 0) {
    int off = args ? args[1].i : 0;
    int len = args ? args[2].i : -1;
    void *s = jstring_from_barr(args ? args[0].l : NULL, off, len);
    debugPrintf("jni_shim: NewObjectA(java/lang/String [B,%d,%d) -> \"%s\"\n", off, len, resolve_jstring(s));
    return s;
  }
  if (sig && strcmp(sig, "(Ljava/lang/String;)V") == 0)
    return make_jstring(args ? resolve_jstring(args[0].l) : "");
  return NULL;
}
static void *jni_NewObject(void *env, void *clazz, void *mid, ...) {
  va_list ap; va_start(ap, mid);
  void *s = jni_NewStringObjectV(clazz, mid, ap);
  va_end(ap);
  if (s) return s;
  return jni_NewObject_common(env, clazz, mid);
}
...
static void *jni_NewObjectA(void *env, void *clazz, void *mid, void *args) {
  void *s = jni_NewStringObjectA(clazz, mid, (const jvalue *)args);
  if (s) return s;
  return jni_NewObject_common(env, clazz, mid);
}
...
void jni_shim_init(void **out_vm, void **out_env) {
  ...
  jni_env_vtable[26] = (uintptr_t)jni_EnsureLocalCapacity;
  ...
  jni_env_vtable[104] = (uintptr_t)jni_SetObjectField;
  jni_env_vtable[109] = (uintptr_t)jni_SetIntField;
  jni_env_vtable[111] = (uintptr_t)jni_SetFloatField;
  ...
  jni_env_vtable[143] = (uintptr_t)jni_CallStaticVoidMethodA; /* A (jvalue*) */
