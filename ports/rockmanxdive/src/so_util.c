...
void hook_arm64(uintptr_t addr, uintptr_t dst) {
  ...
  uintptr_t p0 = addr & ~((uintptr_t)pgsz - 1);
  uintptr_t p1 = (addr + 15) & ~((uintptr_t)pgsz - 1);
  if (mprotect((void *)p0, (size_t)pgsz, PROT_READ | PROT_WRITE | PROT_EXEC) != 0) {
    debugPrintf("hook_arm64: mprotect RWX failed addr=%p errno=%d\n", (void *)addr, errno);
    return;
  }
  if (p1 != p0 &&
      mprotect((void *)p1, (size_t)pgsz, PROT_READ | PROT_WRITE | PROT_EXEC) != 0) {
    debugPrintf("hook_arm64: mprotect RWX second page failed addr=%p errno=%d\n", (void *)addr, errno);
    mprotect((void *)p0, (size_t)pgsz, PROT_READ | PROT_EXEC);
    return;
  }
...
#define SO_MAX_PH 20
struct so_phdr_mod g_so_mods[8];
int g_so_nmods = 0;
void so_record_phdr(const char *name) {
  if (g_so_nmods >= (int)(sizeof(g_so_mods) / sizeof(g_so_mods[0]))) {
    debugPrintf("so_record_phdr: module table full, skipping %s\n", name ? name : "?");
    return;
  }
