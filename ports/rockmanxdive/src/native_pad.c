...
extern void *ter_il2cpp_sym(const char *nm);
...
/* ---- il2cpp API: resolvida por símbolos exportados do libil2cpp do jogo ativo. ---- */
...
static f_dom_get NP_dom_get;
static f_dom_asms NP_dom_asms;
static f_asm_img NP_asm_img;
static f_cls_from_name NP_cls_from_name;
static f_cls_method NP_cls_method;
static f_string_new NP_string_new;
static f_array_new NP_array_new;
static f_gch_new NP_gch_new;
static f_obj_new NP_obj_new;
static f_rt_invoke NP_rt_invoke;
static int np_api_ready;

static int np_resolve_il2cpp_api(void) {
  if (np_api_ready) return 1;
  if (!g_il2cpp_base) return 0;
  NP_dom_get       = (f_dom_get)ter_il2cpp_sym("il2cpp_domain_get");
  NP_dom_asms      = (f_dom_asms)ter_il2cpp_sym("il2cpp_domain_get_assemblies");
  NP_asm_img       = (f_asm_img)ter_il2cpp_sym("il2cpp_assembly_get_image");
  NP_cls_from_name = (f_cls_from_name)ter_il2cpp_sym("il2cpp_class_from_name");
  NP_cls_method    = (f_cls_method)ter_il2cpp_sym("il2cpp_class_get_method_from_name");
  NP_string_new    = (f_string_new)ter_il2cpp_sym("il2cpp_string_new");
  NP_array_new     = (f_array_new)ter_il2cpp_sym("il2cpp_array_new");
  NP_gch_new       = (f_gch_new)ter_il2cpp_sym("il2cpp_gchandle_new");
  NP_obj_new       = (f_obj_new)ter_il2cpp_sym("il2cpp_object_new");
  NP_rt_invoke     = (f_rt_invoke)ter_il2cpp_sym("il2cpp_runtime_invoke");
  if (!NP_dom_get || !NP_dom_asms || !NP_asm_img || !NP_cls_from_name ||
      !NP_cls_method || !NP_string_new || !NP_array_new || !NP_gch_new ||
      !NP_obj_new || !NP_rt_invoke) {
    static int logged;
    if (!logged) { logged = 1; fprintf(stderr, "[NATPAD] API il2cpp incompleta; adiando\n"); fsync(2); }
    return 0;
  }
  np_api_ready = 1;
  return 1;
...
static unsigned long np_off_or(const char *ns, const char *cn, const char *mn, int argc,
  ...
  if (!o && getenv("TER_NPFALLBACK")) o = fallback;
  return o;
}
...
void np_frame(void) {
  ...
  if (!np_resolve_il2cpp_api()) return;
