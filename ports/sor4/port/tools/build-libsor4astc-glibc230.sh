...
set -e
cd "$(dirname "$0")/.."          # ports/sor4
SRC="$(pwd)/build"               # sor4astc.c, etc1_encode.c, etc1_encode.h
ASTCENC5_SRC="${ASTCENC5_SRC:-/home/nextos/deadcells-deploy/astc-encoder/Source}"

S=$(mktemp -d); mkdir -p "$S/Source"
cp "$ASTCENC5_SRC"/*.cpp "$ASTCENC5_SRC"/*.h "$S/Source/"
cp "$SRC/sor4astc.c" "$SRC/etc1_encode.c" "$SRC/etc1_encode.h" "$S/"
...
g++ -O2 -fPIC -shared -std=c++17 -DASTCENC_DECOMPRESS_ONLY=1 -fno-strict-aliasing -I Source -I . \
    -o libsor4astc.so Source/astcenc_*.cpp sor4astc.c etc1_encode.o -lm
