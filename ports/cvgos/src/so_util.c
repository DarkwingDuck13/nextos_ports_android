 ...
 *
 * Portado do so_util aarch64 (Terraria) para ELF32-ARM seguindo a ARM ELF ABI:
 * classe ELFCLASS32, e_machine EM_ARM, relocs REL (.rel.dyn/.rel.plt) com ADDEND
 * IMPLÍCITO (o valor já presente em *ptr), tipos R_ARM_*, entradas de init_array
 * de 4 bytes, e preservação do bit Thumb (st_value bit0) nos endereços de função.
 * Preserva a API multi-módulo do Terraria (so_save/so_use, so_record_phdr,
 * so_register_eh_frame, so_patch_got, g_so_mods) — só troca o word-size do ELF e
 * o tratamento de relocations para ELF32-ARM (espelhando o so_util do Shantae).
 *
...
void hook_arm64(uintptr_t addr, uintptr_t dst) {
  /* nome mantido por compat com a API; aqui é ARM32 (patch de salto absoluto). */
  if (addr == 0)
    return;
...
void so_finalize(void) {
  /* Só endurece o segmento executável (RX); resto fica RWX (heap mmap RWX) p/ o
   * pool de trampolins do hook_arm64 poder executar. */
...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  /* Mapeia TODOS os PT_LOAD no seu p_vaddr (NDK moderno gera >2 segmentos). */
  for (int i = 0; i < elf_hdr->e_phnum; i++) {
    if (prog_hdr[i].p_type != PT_LOAD)
      continue;
...
int so_relocate(void) {
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
     ...
     * ELF32: cada entrada é 4 bytes (Elf32_Relr) e cada bitmap cobre 31 words. */
    else if (strcmp(sh_name, ".relr.dyn") == 0) {
      uint32_t *relr = (uint32_t *)((uintptr_t)load_base + sec_hdr[i].sh_addr);
      size_t n = sec_hdr[i].sh_size / sizeof(uint32_t);
      uintptr_t base = (uintptr_t)load_base;
      uint32_t *where = NULL;
      for (size_t k = 0; k < n; k++) {
        uint32_t e = relr[k];
        if ((e & 1) == 0) {            /* endereço: aplica 1 reloc + avança */
          where = (uint32_t *)(base + e);
          *where++ += (uint32_t)base;
        } else {                       /* bitmap dos próximos 31 words */
          for (int b = 0; (e >>= 1) != 0; b++)
            if (e & 1) where[b] += (uint32_t)base;
          where += 31;
        }
      }
    }
 ...
 * do unwinder NÃO os vê -> exceção C++ não acha o landing pad. */
extern void __register_frame(void *begin) __attribute__((weak));
int so_register_eh_frame(void) {
  if (!getenv("CUP_EHREG")) return -1;   /* __register_frame pode crashar (formato) */
  if (!&__register_frame) return -1;     /* ausente na libgcc_s (weak) */
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
    if (!strcmp(shstrtab + sec_hdr[i].sh_name, ".eh_frame") && sec_hdr[i].sh_size) {
      void *eh = (void *)((uintptr_t)load_base + sec_hdr[i].sh_addr);
      __register_frame(eh);
      return 0;
    }
  }
 ...
 * uma CÓPIA estável dos phdrs (p_vaddr já é offset relativo à base neste modelo)
 * + a base (dlpi_addr=load_virtbase). */
#define SO_MAX_PH 20
...
void so_record_phdr(const char *name) {
  ...
  for (int i = 0; i < n; i++) {
    m->ph[i] = prog_hdr[i];
    /* p_vaddr já é offset relativo à base neste modelo ELF32; se por acaso vier
     * absoluto (>= base), volta p/ offset. */
      ...
      for (int j = 0; j < n; j++) {
        ...
        if (j < 6)
          debugPrintf("init_array[%d/%d] = %p (off 0x%lx)\n", j, n, (void *)fn,
                      (unsigned long)(fn - (uintptr_t)load_base));
        if (fn == 0 || fn == (uintptr_t)-1 || fn == (uintptr_t)load_base) {
...
uintptr_t so_find_addr_safe(const char *symbol) {
  for (int i = 0; i < num_syms; i++) {
    char *name = dynstrtab + syms[i].st_name;
    if (strcmp(name, symbol) == 0) {
      /* import (UNDEF, st_value=0): sem este guard devolvíamos load_base+0 =
         base do módulo -> ponteiro lixo. */
...
/* patcheia TODOS os slots da GOT (GLOB_DAT + JUMP_SLOT) de um simbolo. Retorna a contagem. */
