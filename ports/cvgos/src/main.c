...
/* armv7 glibc mcontext: registradores individuais (não há array .regs[]).
 * Mapeia índice de registrador -> valor p/ o dump de diagnóstico do crash. */
static unsigned long mc_reg(const mcontext_t *m, int n) {
  switch (n) {
  case 0: return m->arm_r0;   case 1: return m->arm_r1;
  case 2: return m->arm_r2;   case 3: return m->arm_r3;
  case 4: return m->arm_r4;   case 5: return m->arm_r5;
  case 6: return m->arm_r6;   case 7: return m->arm_r7;
  case 8: return m->arm_r8;   case 9: return m->arm_r9;
  case 10: return m->arm_r10; case 11: return m->arm_fp;
  case 12: return m->arm_ip;  case 13: return m->arm_sp;
  case 14: return m->arm_lr;  case 15: return m->arm_pc;
...
static void on_crash(int sig, siginfo_t *si, void *uc_) {
  ...
  if (g_skipbad && sig == SIGSEGV && pc0 >= ARENA_LO && pc0 < ARENA_HI) {
    if (lr0 && lr0 != pc0) {
      uc0->uc_mcontext.arm_pc = lr0;    /* retoma no retorno */
      uc0->uc_mcontext.arm_r0 = 0;      /* valor de retorno = null/0 */
      if (g_skipbad_n++ < 60)
  ...
  uintptr_t x19 = mc_reg(&uc->uc_mcontext, 4);   /* r4 (callee-saved, ~x19 arm64) */
  crash_classify("r4(dispatch_obj)", x19);
  crash_dump_qwords("r4", x19, 40);
  /* r0..r3 = args; r12(ip) = scratch de chamada */
  crash_classify("r0", mc_reg(&uc->uc_mcontext, 0));
  crash_classify("r5", mc_reg(&uc->uc_mcontext, 5));
  /* r6/r7: candidatos a 'this'/objeto pai */
  crash_classify("r6", mc_reg(&uc->uc_mcontext, 6));
  crash_classify("r7", mc_reg(&uc->uc_mcontext, 7));
  ...
  /* r3/r9/r12: ponteiros recorrentes — que região? */
 ...
 * em /storage/roms/ports/cvgos/shot.ppm (verificação autônoma de IMAGEM no fbdev Mali). */
static void ter_screenshot_maybe(void) {
  ...
  FILE *f = fopen("/storage/roms/ports/cvgos/shot.ppm","wb");
  if (f) { fprintf(f,"P6\n%d %d\n255\n", w, h);
...
static void ter_ctrl_feed(void) {
  ...
  /* TER_SWAPAB: troca A<->B (pedido). A=Action1(confirma), B=Action2(volta). */
  ...
  /* TER_SWAPLR: inverte L1<->L2 e R1<->R2 (pedido; pode tirar a dupla-funcao do R1). */
...
/* ARMv7: trampolim arm64 original stubado (Terraria/Cuphead menu hack, não
 * portado — só precisamos compilar; hook desativado em runtime). */
void onew_spy_tramp(void) { }
...
/* ARMv7: trampolim arm64 original stubado (não portado). */
long waitall_orig_tramp(void *thiz, long a1) { (void)thiz; (void)a1; return 0; }
...
/* ARMv7: trampolim arm64 original stubado (não portado). */
long signal_orig_tramp(void *sem, long count) { (void)sem; (void)count; return 0; }
...
/* ARMv7: trampolins arm64 originais stubados (não portados). */
long cr1_tramp(void *it) { (void)it; return 0; }
long cr2_tramp(void *it) { (void)it; return 0; }
...
/* ARMv7: trampolim arm64 original stubado (não portado). */
int tapinput_tramp(void *self) { (void)self; return 0; }
...
int main(int argc, char **argv) {
  ...
  { uintptr_t tp; __asm__ volatile("mrc p15, 0, %0, c13, c0, 3" : "=r"(tp));  /* TPIDRURO (armv7 thread ptr) */
