...
/* ---- bionic threading: pthread_cond_timedwait_relative_np (Android-only) ----
 * Unity/il2cpp usam MUITO p/ waits temporizados (job-system, GC). glibc nao tem ->
 * dlsym falha -> GOT null -> fault a cada frame ("main thread trapped"). Impl real:
 * converte o timeout RELATIVO -> ABSOLUTO (CLOCK_REALTIME, default bionic) e delega. */
#include <pthread.h>
#include <time.h>
#include <errno.h>
int pthread_cond_timedwait_relative_np(pthread_cond_t *cond, pthread_mutex_t *mutex,
                                       const struct timespec *reltime){
  if(!reltime) return pthread_cond_wait(cond, mutex);
  struct timespec abs; clock_gettime(CLOCK_REALTIME, &abs);
  abs.tv_sec += reltime->tv_sec;
  abs.tv_nsec += reltime->tv_nsec;
  if(abs.tv_nsec >= 1000000000L){ abs.tv_sec += abs.tv_nsec/1000000000L; abs.tv_nsec %= 1000000000L; }
...
int my_sigaction(int sig, const struct bionic_sigaction *act, struct bionic_sigaction *oldact) {
  ...
  if (sig==1 && !getenv("CVGOS_NOSIGHUPIGN")) { (void)oldact; return 0; }  /* SIGHUP: nao deixa a Unity marcar trap=1 */
