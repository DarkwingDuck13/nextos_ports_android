...
### ⚠️ sessão 1 fim — il2cpp_init crasha no metadata-parse (muro FF9) + device ocupado
- **PROGRESSO REAL**: chamar `il2cpp_set_data_dir(bin/Data/Managed)+il2cpp_init` explicito → **global-metadata.dat CARREGA** (openat fd=3) e o runtime C# começa a inicializar. Crasha em **libil2cpp+0x77a600** (`ldr r2,[r7,#-20]`, r7 fora do mapa) durante o parsing de metadata = **muro il2cpp-init classe FF9**. CRASHSKIP cirúrgico entra em loop no mesmo pc (skip-load não avança o loop interno do il2cpp).
- ⚠️ **CUIDADO**: o `.79` estava rodando `/storage/roms/ports/sonic4/sonic4` (teste de OUTRA sessão) → capturas de fb0 mostram SONIC 4, não o cvgos (falso-positivo evitado). Validar render do cvgos SÓ com o device livre (rule #3: nunca 2 jogos juntos).
- **PRÓXIMO**: debugar o crash do il2cpp_init em libil2cpp+0x77a600 (r7=base+[r2+0xb0]+20; provável offset de metadata errado — checar se metadata precisa mmap vs read, ou se falta il2cpp_set_config/register antes). Classe FF9 (multi-sessão). il2cpp_init call em main.c:5561 (CVGOS_NOIL2CPPINIT desliga).
