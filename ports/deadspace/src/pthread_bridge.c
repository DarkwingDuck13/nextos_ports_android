...
static int b_attr_setstack(void *a, void *addr, size_t sz) {
  (void)a; (void)addr; (void)sz; return 0;
}
static int b_attr_getstack(void *a, void **addr, size_t *sz) {
  (void)a;
  if (addr) *addr = NULL;
  if (sz) *sz = 2 * 1024 * 1024;
  return 0;
}
static int b_pthread_getattr_np(pthread_t th, void *a) {
  (void)th;
  return b_attr_init(a);
}

// ---------------- pthread key (bionic pthread_key_t = int) ----------------
static int b_key_create(void *key, void (*destructor)(void *)) {
  pthread_key_t real_key;
  int r = pthread_key_create(&real_key, destructor);
  if (!r && key) *(int *)key = (int)real_key;
  return r;
}
static int b_key_delete(int key) {
  if (key < 0) return 0;
  return pthread_key_delete((pthread_key_t)key);
}
static int b_setspecific(int key, const void *value) {
  if (key < 0) return 0;
  return pthread_setspecific((pthread_key_t)key, value);
}
static void *b_getspecific(int key) {
...
// ---------------- sem_t (bionic storage -> glibc sem_t*) ----------------
static sem_t *sem_real(void *s) {
  sem_t **slot = (sem_t **)s;
  if (IS_HEAP_PTR(*slot)) return *slot;
  pthread_mutex_lock(&g_lock);
  if (!IS_HEAP_PTR(*slot)) {
    sem_t *r = (sem_t *)calloc(1, sizeof(sem_t));
    sem_init(r, 0, 0);
    *slot = r;
  }
  pthread_mutex_unlock(&g_lock);
...
static int b_sem_init(void *s, int pshared, unsigned value) {
  (void)pshared;
  sem_t **slot = (sem_t **)s;
  sem_t *r = (sem_t *)calloc(1, sizeof(sem_t));
  sem_init(r, 0, value);
...
static int b_sem_destroy(void *s) {
  sem_t **slot = (sem_t **)s;
  pthread_mutex_lock(&g_lock);
  if (IS_HEAP_PTR(*slot)) {
    sem_destroy(*slot);
...
static int b_sem_wait(void *s) { return sem_wait(sem_real(s)); }
static int b_sem_trywait(void *s) { return sem_trywait(sem_real(s)); }
static int b_sem_timedwait(void *s, const struct timespec *ts) {
  return sem_timedwait(sem_real(s), ts);
}
static int b_sem_post(void *s) { return sem_post(sem_real(s)); }
static int b_sem_getvalue(void *s, int *v) { return sem_getvalue(sem_real(s), v); }
...
DynLibFunction revc_pthread_table[] = {
    ...
    {"pthread_getattr_np", (uintptr_t)&b_pthread_getattr_np},
    {"pthread_key_create", (uintptr_t)&b_key_create},
    {"pthread_key_delete", (uintptr_t)&b_key_delete},
    {"pthread_setspecific", (uintptr_t)&b_setspecific},
    {"pthread_getspecific", (uintptr_t)&b_getspecific},
