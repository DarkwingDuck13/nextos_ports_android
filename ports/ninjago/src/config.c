/* config.c -- configuration parser (Linux port)
 *
 * Language is FORCED to English on this target (never Japanese): the port
 * always reports en-US regardless of any system locale.
 *
...
int read_config(const char *file) {
  ...
  config.language = 0;   // en-US (forced English)
  config.show_fps = 0;
...
// Always English on this target (never Japanese).
