...
/* ---- GL float-by-value (chama o gl* HARDFP do device) ----
   Cocos2d-x (softfp) passa floats em registradores inteiros; o libGLESv2/libMali
   do device é hardfp. Sem estes wrappers TODO uniform de shader / cor / depth
   vira lixo -> render quebrado/tela preta. *fv/*iv usam ponteiro (mesma ABI) -> OK. */
#define GLW1F(n) SF void sf_##n(float a){ static void(*r)(float)=0; \
  if(!r) r=(void(*)(float))dlsym(RTLD_DEFAULT,#n); if(r) r(a); }
GLW1F(glClearDepthf) GLW1F(glLineWidth)
...
SF void sf_glUniform1f(int loc,float a){
  static void(*r)(int,float)=0; if(!r) r=(void(*)(int,float))dlsym(RTLD_DEFAULT,"glUniform1f"); if(r) r(loc,a); }
SF void sf_glUniform2f(int loc,float a,float b){
  static void(*r)(int,float,float)=0; if(!r) r=(void(*)(int,float,float))dlsym(RTLD_DEFAULT,"glUniform2f"); if(r) r(loc,a,b); }
SF void sf_glUniform3f(int loc,float a,float b,float c){
  static void(*r)(int,float,float,float)=0; if(!r) r=(void(*)(int,float,float,float))dlsym(RTLD_DEFAULT,"glUniform3f"); if(r) r(loc,a,b,c); }
SF void sf_glUniform4f(int loc,float a,float b,float c,float d){
  static void(*r)(int,float,float,float,float)=0; if(!r) r=(void(*)(int,float,float,float,float))dlsym(RTLD_DEFAULT,"glUniform4f"); if(r) r(loc,a,b,c,d); }
...
static const struct sfent SFTAB[] = {
  ...
  {"glClearColor",sf_glClearColor},{"glClearDepthf",sf_glClearDepthf},{"glLineWidth",sf_glLineWidth},
  {"glUniform1f",sf_glUniform1f},{"glUniform2f",sf_glUniform2f},
  {"glUniform3f",sf_glUniform3f},{"glUniform4f",sf_glUniform4f},
};
