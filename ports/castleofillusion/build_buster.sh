#!/bin/bash
# Build RELEASE do Castle of Illusion (arm64) no Docker debian:buster — glibc ≤2.30
# (regra de TODOS os ports; piso = ArkOS glibc 2.30).
#
# Run (host):
...
gen() {
  "$NM" -D --undefined-only ./castleofillusion |
...
"$CC" -fPIE -pie -O2 -fPIC -fno-omit-frame-pointer -rdynamic -D_GNU_SOURCE \
  ...
  -o castleofillusion.release $SRCS \
  -Isrc -I"$HDR" -I"$HDR/SDL2" \
  -Wl,--export-dynamic -Wl,--as-needed \
  -L"$STUB" -lSDL2 -lEGL -lGLESv2 \
  -ldl -lm -lpthread -lgcc

echo "BUILD OK -> castleofillusion.release"
echo "  arch:      $(file castleofillusion.release 2>/dev/null | head -c 80)"
echo "  GLIBC max: $($READELF -V castleofillusion.release | grep -oE 'GLIBC_[0-9.]+' | sed 's/GLIBC_//' | sort -uV | tail -1)"
echo "  tamanho:   $(stat -c%s castleofillusion.release) bytes"
"$READELF" -d castleofillusion.release | grep NEEDED
