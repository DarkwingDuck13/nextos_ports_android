...
$CC -fPIE -pie -o gtasa-buster $OBJS \
    -L"$STUB" -lSDL2 -lEGL -lGLESv2 -ldl -lm -lpthread \
    -Wl,-rpath,'$ORIGIN'

MAXV=$($OD -T gtasa-buster 2>/dev/null | grep -oE 'GLIBC_[0-9.]+' | sort -uV | tail -1)
echo "BUSTER BUILD OK -> gtasa-buster | glibc max = $MAXV (regra: <= GLIBC_2.30)"
$OD -f gtasa-buster 2>/dev/null | grep -i "start address" >/dev/null && file gtasa-buster | cut -d, -f1-3 || true
