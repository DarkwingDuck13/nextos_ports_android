#!/bin/bash
# GTA San Andreas (Android so-loader NOSSO, GLES2-nativo) -> Mali-450 fbdev.
...
GAMEDIR="/storage/roms/ports/gtasa"
cd "$GAMEDIR" || exit 1
...
pkill -9 -x gtasa-nosso 2>/dev/null; pkill -9 -x gtasa 2>/dev/null; sleep 1
...
chmod +x "$GAMEDIR/gtasa-nosso" 2>/dev/null
