...
static int audio_log_enabled(void) {
  ...
  if (enabled < 0)
    enabled = getenv("LIMBO_AUDIOLOG") ? 1 : 0;
  return enabled;
}
...
typedef struct {
  ...
  void *effectsend_ptr;
  void *androidconfig_vtable[8];
  void *androidconfig_ptr;
} AudioPlayer;
...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  /* master_gain subido 0.30 -> 0.65 (volume estava baixo); o limiter em
     threshold=28000 segura picos quando varios sons mixam. CHRONO_GAIN override. */
  static float master_gain = 0.0f;
  if (master_gain == 0.0f) { const char *g = getenv("CHRONO_GAIN"); master_gain = g ? (float)atof(g) : 0.65f; }
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].obj_ptr == obj_ptr) {
      ...
      if (iid == sl_IID_PLAY) {
        ...
        *pInterface = &p->androidconfig_ptr;
      } else {
...
static void setup_player_vtables(AudioPlayer *p) {
  ...
  for (int i = 0; i < 8; i++)
    p->androidconfig_vtable[i] = (void *)stub_success;
  p->androidconfig_vtable[0] = (void *)androidconfig_SetConfiguration;
  p->androidconfig_vtable[1] = (void *)androidconfig_GetConfiguration;
  p->androidconfig_ptr = p->androidconfig_vtable;
}
...
static SLresult engine_CreateOutputMix(void *self, void **pMix,
  ...
  if (audio_log_enabled())
    debugPrintf("opensles_shim: CreateOutputMix interfaces=%u\n",
                numInterfaces);
  init_outmix();
  if (pMix)
    *pMix = &g_outmix_ptr;
  if (audio_log_enabled())
    debugPrintf("opensles_shim: CreateOutputMix -> %p\n",
                pMix ? *pMix : NULL);
  return SL_RESULT_SUCCESS;
}
...
static SLresult engine_CreateAudioPlayer(void *self, void **pPlayer,
  ...
  if (audio_log_enabled())
    debugPrintf("opensles_shim: CreateAudioPlayer interfaces=%u src=%p snk=%p\n",
                numInterfaces, pAudioSrc, pAudioSnk);
  ...
  if (pPlayer)
    *pPlayer = &p->obj_ptr;
  if (audio_log_enabled())
    debugPrintf("opensles_shim: CreateAudioPlayer -> %p format=%uch/%uHz/%ubit queue=%u\n",
                pPlayer ? *pPlayer : NULL, p->num_channels, p->sample_rate,
...
static SLresult engine_obj_GetInterface(void *self, SLInterfaceID iid,
  ...
  (void)self;
  if (audio_log_enabled())
    debugPrintf("opensles_shim: EngineObject GetInterface iid=%p engine=%p\n",
                iid, sl_IID_ENGINE);
...
SLresult slCreateEngine_shim(void **pEngine, SLuint32 numOptions,
  ...
  debugPrintf("opensles_shim: slCreateEngine options=%u interfaces=%u\n",
              numOptions, numInterfaces);
  init_engine();
  if (pEngine)
    *pEngine = &g_engine_obj_ptr;
  if (audio_log_enabled())
    debugPrintf("opensles_shim: slCreateEngine -> %p\n",
