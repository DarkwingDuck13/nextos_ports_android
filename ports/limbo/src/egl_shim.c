...
int limbo_screen_w = 1280, limbo_screen_h = 720;
...
static SDL_Window *egl_pbuffer_window = NULL;
...
static _egl_context egl_bootstrap_context;
static __thread _egl_context *current_context = NULL;
static _egl_context *last_context = NULL;
static __thread int has_real_gl = 0;
...
void egl_shim_create_window(void) {
  ...
  if (SDL_GetDesktopDisplayMode(0, &dm) == 0 && dm.w > 0 && dm.h > 0) {
    limbo_screen_w = dm.w; limbo_screen_h = dm.h;
    debugPrintf("egl_shim: desktop mode %dx%d\n", dm.w, dm.h);
  }
  { const char *e = getenv("LIMBO_RES"); int w, h; /* override opcional */
    if (e && sscanf(e, "%dx%d", &w, &h) == 2 && w > 0 && h > 0) {
      limbo_screen_w = w; limbo_screen_h = h;
      debugPrintf("egl_shim: LIMBO_RES override %dx%d\n", w, h);
    } }
  ...
  { const char *gv = getenv("LIMBO_GLVER");
  ...
  egl_pbuffer_window = SDL_CreateWindow(
      "limbo-pbuffer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
      16, 16, SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN);
  if (!egl_pbuffer_window)
    debugPrintf("egl_shim: hidden pbuffer window FAILED: %s\n", SDL_GetError());
  else
    debugPrintf("egl_shim: hidden pbuffer window created\n");
  ...
  memset(&egl_bootstrap_context, 0, sizeof(egl_bootstrap_context));
  egl_bootstrap_context.sdl_context = egl_share_root;
  egl_bootstrap_context.id = 0;
  /* DYSMANTLE_SWAPINT no contexto novo (a engine pode nunca chamar
   * eglSwapInterval; default SDL=vsync 1 + limiter da engine = 30fps). */
  {
    const char *f = getenv("LIMBO_SWAPINT");
    if (f) {
...
int egl_shim_make_bootstrap_current(void) {
  if (!egl_window || !egl_share_root) {
    debugPrintf("egl_shim: bootstrap current pedido antes da janela GL\n");
    return has_real_gl ? 1 : 0;
  }
  if (has_real_gl && current_context)
    return 1;
  if (gl_makecurrent(egl_window, egl_share_root) != 0) {
    debugPrintf("egl_shim: bootstrap MakeCurrent FAILED [tid=%lx]: %s\n",
                (unsigned long)pthread_self(), SDL_GetError());
    return has_real_gl ? 1 : 0;
  }
  current_context = &egl_bootstrap_context;
  last_context = &egl_bootstrap_context;
  has_real_gl = 1;
  debugPrintf("egl_shim: bootstrap current [tid=%lx]\n",
...
void egl_shim_release_current_if_pbuffer(void) {
  if (!has_real_gl || !current_context || !current_context->is_pbuffer)
    return;

  SDL_Window *target = egl_pbuffer_window ? egl_pbuffer_window : egl_window;
  if (target)
    gl_makecurrent(target, NULL);
  debugPrintf("egl_shim: released pbuffer context before thread spawn [tid=%lx] [ctx_id=%d]\n",
              (unsigned long)pthread_self(), current_context->id);
...
int egl_shim_ensure_current(void) {
  ...
  SDL_Window *target = ctx->is_pbuffer && egl_pbuffer_window
                           ? egl_pbuffer_window
                           : egl_window;
  int ret = gl_makecurrent(target, ctx->sdl_context);
  if (ret == 0) {
    has_real_gl = 1;
    current_context = ctx;
...
EGLBoolean egl_shim_Terminate(EGLDisplay dpy) {
  ...
  if (egl_pbuffer_window) {
    SDL_DestroyWindow(egl_pbuffer_window);
    egl_pbuffer_window = NULL;
  }
  return EGL_TRUE;
}
...
EGLBoolean egl_shim_MakeCurrent(EGLDisplay dpy, EGLSurface draw,
  ...
  if (context == NULL || draw == NULL) {
    SDL_Window *target = current_context && current_context->is_pbuffer &&
                                 egl_pbuffer_window
                             ? egl_pbuffer_window
                             : egl_window;
    current_context = NULL;
  ...
  SDL_Window *target = !is_window && egl_pbuffer_window
                           ? egl_pbuffer_window
                           : egl_window;
  int ret = gl_makecurrent(target, context->sdl_context);
  if (ret == 0) {
    has_real_gl = 1;
    /* DYSMANTLE_SWAPINT: intervalo é estado por-contexto; aplica 1x em cada */
    {
      static const char *si = (const char *)-1;
      if (si == (const char *)-1) si = getenv("LIMBO_SWAPINT");
...
static void limbo_maybe_screenshot(void) {
  static int chk = 0;
  if (++chk % 15) return;
  if (access("/dev/shm/limbo_shot", F_OK) != 0) return;
  unlink("/dev/shm/limbo_shot");
  GLint vp[4] = {0,0,0,0};
  ...
  FILE *o = fopen("/dev/shm/limbo_shot.raw", "wb");
  if (o) { fwrite(buf, 1, (size_t)w * h * 4, o); fclose(o); }
  FILE *t = fopen("/dev/shm/limbo_shot.txt", "w");
  if (t) { fprintf(t, "%d %d\n", w, h); fclose(t); }
  free(buf);
...
EGLBoolean egl_shim_SwapBuffers(EGLDisplay dpy, EGLSurface surface) {
  ...
  if (has_real_gl && current_context && !current_context->is_pbuffer) {
    if (getenv("LIMBO_FRAME_INPUT"))
      android_shim_pump_input_frame();
    limbo_maybe_screenshot();
...
void *egl_shim_GetProcAddress(const char *procname) {
  extern void *limbo_gl_proc_override(const char *name);
  void *ov = limbo_gl_proc_override(procname);
  if (ov)
    return ov;
