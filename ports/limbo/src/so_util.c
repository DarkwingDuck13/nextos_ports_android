...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  /* Layout-agnostico: o segmento executavel NAO e' necessariamente o 1o PT_LOAD
   * nem esta' em vaddr 0 (libc++_shared do NDK moderno = R / R+X / RW / RW).
   * Ancoramos TUDO em vaddr 0 = load_base e copiamos todos os PT_LOAD. */
  {
    Elf64_Addr max_vaddr = 0;
    int n_load = 0, exec_seg = -1;
    for (int i = 0; i < elf_hdr->e_phnum; i++) {
      if (prog_hdr[i].p_type != PT_LOAD) continue;
      n_load++;
      Elf64_Addr end = prog_hdr[i].p_vaddr + prog_hdr[i].p_memsz;
      if (end > max_vaddr) max_vaddr = end;
      if ((prog_hdr[i].p_flags & PF_X) == PF_X) { text_segno = i; exec_seg = i; }
      else if (data_segno < 0) data_segno = i;
    }
    if (n_load == 0 || exec_seg < 0)
      goto err_free_so;
    load_size = ALIGN_MEM(max_vaddr, 0x1000);
  }
  ...
  /* ancora vaddr 0 -> toda a aritmetica (sec_hdr/rela/sym usam vaddr) bate */
  text_base = load_base;          text_virtbase = load_virtbase;
  data_base = load_base;          data_virtbase = load_virtbase;
  text_size = load_size;          data_size = load_size;

  /* copiar TODOS os PT_LOAD para load_base + p_vaddr */
  for (int s = 0; s < elf_hdr->e_phnum; s++) {
    if (prog_hdr[s].p_type != PT_LOAD) continue;
    void *dst = (void *)((uintptr_t)load_base + prog_hdr[s].p_vaddr);
    ...
    debugPrintf("so_load: PT_LOAD[%d] vaddr=0x%lx filesz=0x%lx flags=%c%c%c\n", s,
                ...
                (prog_hdr[s].p_flags & PF_R) ? 'R' : '-',
                (prog_hdr[s].p_flags & PF_W) ? 'W' : '-',
                (prog_hdr[s].p_flags & PF_X) ? 'X' : '-');
  }
      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_ABS64: {
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (!found) {
              /* modulo auxiliar (libc++_shared do jogo): resolve os simbolos
                 std::__ndk1 / __cxa_* / operator new-delete que so existem nele */
              uintptr_t aux = so_aux_find_export(name);
              if (aux) { *ptr = aux + add; found = 1; }
            }
            if (!found) {
            ...
            if (getenv("LIMBO_RESOLVELOG") &&
                (!strcmp(name, "dlopen") || !strcmp(name, "dlsym") ||
                 !strcmp(name, "dlerror") || !strcmp(name, "dlclose") ||
                 !strcmp(name, "memcpy") || !strcmp(name, "__memcpy_chk"))) {
              fprintf(stderr, "[resolve] %-12s type=%d got=0x%lx slot=%p value=%p found=%d\n",
                      name, type, (unsigned long)rels[j].r_offset,
                      (void *)ptr, (void *)*ptr, found);
            }
          }
          break;
        }
...
/* ===== modulo auxiliar (libc++_shared do jogo) p/ resolver imports cross-modulo ===== */
static so_module *g_aux_module = NULL;
void so_set_aux_module(so_module *m) { g_aux_module = m; }

uintptr_t so_module_find_export(so_module *m, const char *name) {
  if (!m) return 0;
  for (int i = 0; i < m->num_syms; i++) {
    if (m->syms[i].st_shndx == SHN_UNDEF) continue;
    if (m->syms[i].st_value == 0) continue;
    if (strcmp(m->dynstrtab + m->syms[i].st_name, name) == 0)
      return (uintptr_t)m->text_base + m->syms[i].st_value;
  }
  return 0;
}

uintptr_t so_aux_find_export(const char *name) {
  return so_module_find_export(g_aux_module, name);
}

/* DEBUG: varre .rela.plt/.rela.dyn e reporta slots JUMP_SLOT/GLOB_DAT cujo valor
   ficou < text_base (= offset cru nao relocado -> aponta p/ PLT0 ou base+0). */
void so_debug_scan_got(void) {
  int bad = 0;
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
    char *sh_name = shstrtab + sec_hdr[i].sh_name;
    if (strcmp(sh_name, ".rela.plt") && strcmp(sh_name, ".rela.dyn")) continue;
    Elf64_Rela *rels = (Elf64_Rela *)((uintptr_t)text_base + sec_hdr[i].sh_addr);
    int n = sec_hdr[i].sh_size / sizeof(Elf64_Rela);
    for (int j = 0; j < n; j++) {
      int type = ELF64_R_TYPE(rels[j].r_info);
      if (type != R_AARCH64_JUMP_SLOT && type != R_AARCH64_GLOB_DAT) continue;
      uintptr_t *ptr = (uintptr_t *)((uintptr_t)text_base + rels[j].r_offset);
      Elf64_Sym *sym = &syms[ELF64_R_SYM(rels[j].r_info)];
      if (*ptr && *ptr < (uintptr_t)text_base) {
        if (bad++ < 30)
          debugPrintf("BADGOT %s off=0x%lx val=0x%lx undef=%d type=%d\n",
                      dynstrtab + sym->st_name, (unsigned long)rels[j].r_offset,
                      (unsigned long)*ptr, sym->st_shndx == SHN_UNDEF, type);
      }
    }
  }
  debugPrintf("so_debug_scan_got: %d bad slots\n", bad);
}
