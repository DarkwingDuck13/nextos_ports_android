...
static char g_writable_path[512] = "/storage/roms/ports/megaman6/userdata/";
...
static void *string_for_tag(void *methodID) {
  ...
  if (methodID == &g_method_tags[MID_PACKAGE_NAME]) return jni_make_string("com.capcom.rockmanmobile1");
  ...
  if (methodID == &g_method_tags[MID_ASSETS_PATH]) return jni_make_string("/storage/roms/ports/megaman6/assets/");
  if (methodID == &g_method_tags[MID_VERSION]) return jni_make_string("1.0.0");
...
/* JNIEnv::GetJavaVM (índice 219). Cricket Audio faz env->GetJavaVM(&vm) no init
   p/ montar o CkConfig; sem isto vm fica NULL -> SystemAndroid chama vm->GetEnv
   com this=NULL -> deref nulo. Devolve nosso JavaVM fake. */
...
/* usado por android_shim.c (path NativeActivity, não exercitado no fluxo cocos,
 * mas precisa linkar): embrulha 'data' num handle de array servido por
 * GetByteArrayRegion/GetArrayLength. */
void *jni_shim_make_array(const void *data, int len) {
  return jarr_new(len, 0, (unsigned char *)data);
}

void jni_shim_init(void **out_vm, void **out_env) {
  g_jni_log = getenv("MM_JNILOG") != NULL;
