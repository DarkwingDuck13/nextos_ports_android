 ...
 *
 * LEGO Ninjago: Shadow of Ronin (Android, arm64) on Mali-450.
 * The game lib is libLEGO_Pixel_Mobile.so (WB Games "Fusion" engine, same JNI
 * surface as LEGO Star Wars TFA), driven through the com.wbgames.LEGOgame.Fusion
 * + GameGLSurfaceView JNI surface: classic Android GLSurfaceView model where WE
 * own EGL and call nativeRender() each frame.
 *
...
// LEGO Marvel Super Heroes ships a single arm64 libLEGO_M1.so with a statically
// linked C++ runtime (no donor needed). v2.0.1.17 (com.wb.lego.marvel).
// This is an EARLIER Fusion build than Ninjago/Batman2: it lacks
// nativeControllerSetData (controls go via the fnaController_Poll GOT hook, as
// in LEGO Batman 2), nativeSetAlertDialogResponse, nativeSetAudioOutputBufferSize
// and fnOBBPackages_AddAssetDir. Game data lives inside a single ~1.9 GB OBB,
// mounted via addOBBEntriesToFusion (obbDir + filename array; engine parses the
// OBB zip TOC itself).
...
#define PACKAGE      "com.wb.lego.marvel"
#define GAMEDATA_DIR "gamedata"   // loose extracted assets (unused for Marvel; OBB path used)
// OBB registered via addOBBEntriesToFusion: obbDir/OBB_FILE opened & parsed by
// the engine (the .obb is a zip with store'd .fib/.mp3/.mp4 entries).
#define OBB_DIR      "."
#define OBB_FILE     "main.20117.com.wb.lego.marvel.obb"
...
#define SAVE_PATH   "/data/user/0/com.wb.lego.marvel/files"                       // getFilesDir
#define CACHE_PATH  "/data/user/0/com.wb.lego.marvel/cache"                       // getCacheDir
#define WRITE_PATH  "/storage/emulated/0/Android/data/com.wb.lego.marvel/files"   // getExternalFilesDir
