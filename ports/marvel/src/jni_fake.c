...
// fileName (String), offset (J), length (J). We register the single OBB.
static char g_obb_filename[256] = { 0 };
static long long g_obb_length = 0;

static void *get_object_field(const char *name) {
  if (!strcmp(name, "WINDOW_SERVICE"))
    return jni_make_string("window");
  if (!strcmp(name, "fileName")) {
    debugPrintf("JNI: GetObjectField(fileName) -> %s\n", g_obb_filename);
    return jni_make_string(g_obb_filename);
  }
