...
/* PC_INLINETASK (pixelcup): o per-object task/fence wait da libunity 2022.3 está em
...
/* CUP_SKIPRESWAIT (pixelcup): a tela de loading (CPCSLoadingState) fica presa em
...
#define ASSET_BASE_M "/storage/roms/ports/pixelcup/"
...
static int my_statfs64(const char *p, void *buf) {
  ...
  int rc = real ? real("/storage/roms/ports/pixelcup", buf) : -1;
 ...
 * em /storage/roms/ports/pixelcup/shot.ppm (verificação autônoma de IMAGEM no fbdev Mali). */
static void ter_screenshot_maybe(void) {
  ...
  FILE *f = fopen("/storage/roms/ports/pixelcup/shot.ppm","wb");
  if (f) { fprintf(f,"P6\n%d %d\n255\n", w, h);
...
static void *choreo_driver_thread(void *arg) {
  ...
  /* il2cpp_domain_get/il2cpp_thread_attach são EXPORTS — resolver por NOME (offsets
     variam por build; os hardcoded do Terraria batiam em lixo no pixelcup -> blr crash). */
...
int main(int argc, char **argv) {
  ...
  jni_shim_set_package("com.BatoviGamesStudio.PixelCupSoccer19", 0);
  ...
  /* PC_INLINETASK (pixelcup, default ON via run.sh): trampolim no topo do loop de espera
