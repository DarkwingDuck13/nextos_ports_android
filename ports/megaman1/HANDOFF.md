...
- **ESTADO ATUAL**: boot→menu(navegável por touch)→modo→save→**stage select**→**gameplay (fase CUTMAN, INTERATIVO)**, INGLÊS, 0 crash, banks de áudio carregam. **FALTA**: (1) mover Mega Man (Lib_MultiInput RE); (2) áudio output (renderBuffer→SDL); depois reusar base p/ MM2-6 (~/Downloads MEGAMAN-2..6.apk).
