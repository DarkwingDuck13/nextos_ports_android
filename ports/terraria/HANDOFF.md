...
**🔴 PENDENTE (única coisa): caminho "Criar" DIRETO (sem digitar o nome no campo antes).** Sintoma: tela "Digite o Nome" + nosso OSK abre + digita + OK aplica o nome, MAS a tela não avança. Diagnóstico da última sessão (run.lastfail no device + scratchpad): a cadeia do guard RESOLVE e liga (`[AUTONAME] guard de edicao LIGADO`), porém `GetInputText` NUNCA é chamado → no disasm de `GUIPlayerNameMenu.Draw` (0xD360E4), depois do guard há `cbz x23 → exit` onde **x23 = static@0 da classe no slot `base+0x2fd6fc0`** (segundo singleton, usado como `[x23+0x20]` no caminho do GetInputText) — null no caminho Criar-direto, existente no caminho digitado (que funciona 100%). PRÓXIMO PASSO: logar/identificar esse singleton (mesma técnica da cadeia: `*(*(void**)(*(void**)(base+0x2fd6fc0)+0xB8))`), descobrir quem o cria no fluxo do campo, e criá-lo/populá-lo no fluxo direto (como fizemos com o GUIMenuNameEdit em LocalUser.Active+0x2F0 — código já no draw hook). Falhas já descartadas: EnterName não é chamado pelo Criar; setters por RVA fora da thread do jogo crasham (thread-static); chatText/inputTextEnter de fora são zerados pelo GetInputText.

**⌨️🏆 NOME/TECLADO RESOLVIDO (commit 3256708, usuário validou "resolveu"):** OSK abre no campo (dpad+A digita, Start=OK, Select=cancela; supressão de reopen desligada em modo OSK; watchdog solta controles se sair sem OK). **Tela "Digite o Nome do Personagem" (GUIPlayerNameMenu = herança PC, GetInputText+inputTextEnter de teclado FÍSICO morto):** fix = ligar o guard de "edição ativa" pela cadeia `[[base+0x2fc2cb0]]→+0xB8→[..]→+0x2F0→byte+0x18` (com sanidade por elo) + hook `GetInputText@0xFCD98C` devolve o nome e liga `set_inputTextEnter@0xF74EF0` (setter REAL; 0xf74e9c/0xf74f4c são os GETTERS!) DEPOIS do original que zera a flag → a própria tela roda PendingPlayer→CreateAndSave→set_menuMode. ⚠️ Injeções de input SEMPRE na thread do jogo (hooks de Draw): estado é thread-static (da swap thread crasha). ⚠️ `EnterName` NÃO é chamado pelo botão Criar (hook nele nunca disparou; transição é por menuMode). ⚠️ Antes de relançar: `cp run.out run.prev` (run.sh sobrescreve o log).
