/* imports.c -- libLEGOHarry.so import resolution (ELF32/ARM, GLES1)
 *
 * armeabi-v7a (softfp) game vs armhf (hardfp) loader: every imported function
 * that takes a float/double arg is routed through a pcs("aapcs") softfp wrapper
 * (softfp_shim). Non-float GL/libc bind directly. GLES1 is fixed-function, so GL
 * goes straight to the system libGLESv1_CM (no shader hooks). OpenSL ES via the
 * SDL bridge; the rest to glibc / bionic shims.
 *
...
static int hp_fprintf(FILE *f, const char *fmt, ...) {
...
// bionic points these at &table[0]; access is table[(int)c + 1]
...
// --- armv7/bionic runtime imports missing from the base table ---------------
static int aeabi_atexit_fake(void *obj, void (*dtor)(void *), void *dso) {
  return __cxa_atexit(dtor, obj, dso);
}
static void *gnu_unwind_find_exidx_fake(void *pc, int *pcount) { (void)pc; if (pcount) *pcount = 0; return 0; }
...
static void *dlopen_fake(const char *n, int f) { (void)f; debugPrintf("dlopen(%s)\n", n?n:"?"); return (void *)0x1; }
...
DynLibFunction dynlib_functions[] = {
  ...
  { "fprintf", (uintptr_t)&hp_fprintf }, { "printf", (uintptr_t)&printf },
  ...
  // EGL (wrapper owns display/ctx/surface)
  ...
  { "eglGetConfigAttrib", (uintptr_t)&eglGetConfigAttribHook },
  ...
  // GLES1 -- non-float bind direct to libGLESv1_CM
  ...
  // GLES1 OES extensions (Mali-450 libGLESv1_CM)
  ...
  // GLES1 float args -> softfp wrappers
  { "glAlphaFunc", (uintptr_t)&sf_glAlphaFunc },
  { "glClearColor", (uintptr_t)&sf_glClearColor },
  { "glClearDepthf", (uintptr_t)&sf_glClearDepthf },
  { "glDepthRangef", (uintptr_t)&sf_glDepthRangef },
  { "glFogf", (uintptr_t)&sf_glFogf },
  { "glLightf", (uintptr_t)&sf_glLightf },
  { "glMaterialf", (uintptr_t)&sf_glMaterialf },
  { "glNormal3f", (uintptr_t)&sf_glNormal3f },
  { "glTexEnvf", (uintptr_t)&sf_glTexEnvf },
  ...
  // armv7/bionic runtime
  { "__aeabi_atexit", (uintptr_t)&aeabi_atexit_fake },
  { "__gnu_Unwind_Find_exidx", (uintptr_t)&gnu_unwind_find_exidx_fake },
  { "__isfinitef", (uintptr_t)&sf_isfinitef },
