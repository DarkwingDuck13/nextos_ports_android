...
volatile int g_alert_pending = 0;

static juint call_boolean(const char *name, va_list va) {
  // Modal alert dialogs (sign-in / consent prompts): there is no Java UI, so log
  // the text and flag main.c to feed a positive response, otherwise the frontend
  // spins forever on GetAlertDialogResponse and New Game never starts.
  if (!strcmp(name, "ShowAlertDialog")) {
    void *t = va_arg(va, void *);
    void *m = va_arg(va, void *);
    void *pos = va_arg(va, void *);
    void *neg = va_arg(va, void *);
    debugPrintf("JNI: ShowAlertDialog title=\"%s\" msg=\"%s\" pos=\"%s\" neg=\"%s\"\n",
                obj_str(t), obj_str(m), obj_str(pos), obj_str(neg));
    g_alert_pending = 1;
    return 1; // dialog shown
  }
...
// OBB registration: addOBBEntriesToFusion reads each array element's fields
// fileName (String), offset (J), length (J). We register one OBB.
char g_obb_filename[256] = { 0 };
long long g_obb_length = 0;

static void *get_object_field(const char *name) {
  if (!strcmp(name, "WINDOW_SERVICE"))
    return jni_make_string("window");
  if (!strcmp(name, "fileName")) { debugPrintf("JNI: GetObjectField(fileName) -> %s\n", g_obb_filename); return jni_make_string(g_obb_filename); }
  debugPrintf("JNI: GetObjectField(%s) -> null\n", name);
  return NULL;
}

static long long get_long_field(const char *name) {
  debugPrintf("JNI: GetLongField(%s)\n", name);
  if (!strcmp(name, "length")) return g_obb_length;
  if (!strcmp(name, "offset")) return 0;
  debugPrintf("JNI: GetLongField(%s) -> 0\n", name);
  return 0;
}

// build a 1-element object array carrying the OBB entry (fields resolved above)
void *jni_make_obb_array(const char *filename, long long length) {
  strncpy(g_obb_filename, filename, sizeof(g_obb_filename) - 1);
  g_obb_length = length;
  FakeObjArray *a = calloc(1, sizeof(*a));
  a->tag = TAG_OBJARR;
  a->len = 1;
  a->items = calloc(1, sizeof(void *));
  a->items[0] = jni_make_object("obbentry");
  return a;
}
...
static long long j_GetLongField(void *env, void *obj, FakeID *id) {
  (void)env; (void)obj; return get_long_field(id_name(id));
}
