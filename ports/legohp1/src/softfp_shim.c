...
/* ---- GLES1 float-arg entry points ---------------------------------------
   O engine (softfp) chama estas funcoes GL passando floats em registradores
   inteiros; declaramos os wrappers pcs("aapcs") e encaminhamos para a libGLESv1_CM
   hardfp do sistema (GCC traduz os registradores). Sem isto, cor/luz/material/
   depth vem como lixo. */
#include <GLES/gl.h>

SF void sf_glClearColor(GLclampf r, GLclampf g, GLclampf b, GLclampf a){ glClearColor(r,g,b,a); }
SF void sf_glClearDepthf(GLclampf d){ glClearDepthf(d); }
SF void sf_glDepthRangef(GLclampf n, GLclampf f){ glDepthRangef(n,f); }
SF void sf_glAlphaFunc(GLenum func, GLclampf ref){ glAlphaFunc(func,ref); }
SF void sf_glFogf(GLenum pname, GLfloat p){ glFogf(pname,p); }
SF void sf_glLightf(GLenum light, GLenum pname, GLfloat p){ glLightf(light,pname,p); }
SF void sf_glMaterialf(GLenum face, GLenum pname, GLfloat p){ glMaterialf(face,pname,p); }
SF void sf_glTexEnvf(GLenum target, GLenum pname, GLfloat p){ glTexEnvf(target,pname,p); }
SF void sf_glNormal3f(GLfloat nx, GLfloat ny, GLfloat nz){ glNormal3f(nx,ny,nz); }

/* sincosf: 1 float in (softfp), 2 out-pointers */
SF void sf_sincosf(float x, float *s, float *c){ *s = sinf(x); *c = cosf(x); }
SF int sf_isfinitef(float x){ return __builtin_isfinite(x); }
