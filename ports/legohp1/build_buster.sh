#!/bin/bash
# Build RELEASE do LEGO Harry Potter Years 1-4 (armhf) no Docker debian:buster
...
# binario host ja linkado (./legohp1).
set -e
...
# stubs a partir dos undefined do binario host (./legohp1 precisa existir)
[ -f ./legohp1 ] || { echo "ERRO: ./legohp1 (build host) ausente -- rode build.sh primeiro"; exit 1; }
gen() { "$NM" -D --undefined-only ./legohp1 | awk '{print $NF}' | grep -E "$1" | sort -u | sed 's/.*/void &(void){}/'; }
gen '^SDL_'   > "$STUB/sdl.c"
gen '^egl'    > "$STUB/egl.c"
...
$CC -march=armv7-a -mfpu=neon -mfloat-abi=hard \
    ...
    -o legohp1.release $SRCS \
    -Wl,--export-dynamic \
    -L"$STUB" -lSDL2 -lGLESv1_CM -lGLESv2 -lEGL -ldl -lm -lpthread -lgcc

echo "BUILD OK -> legohp1.release"
echo "  arch:      $(file legohp1.release 2>/dev/null | head -c 80)"
echo "  GLIBC max: $($READELF -V legohp1.release | grep -oE 'GLIBC_[0-9.]+' | sed 's/GLIBC_//' | sort -uV | tail -1)"
echo "  tamanho:   $(stat -c%s legohp1.release) bytes"
$READELF -d legohp1.release | grep NEEDED
