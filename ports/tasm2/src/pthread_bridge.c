...
static void *thread_noop_bridge(void *arg) {
  (void)arg;
  return NULL;
}

static void *thread_start_bridge(void *arg) {
  thread_start_t *ctx = (thread_start_t *)arg;
  void *(*fn)(void *) = ctx->fn;
  void *fn_arg = ctx->arg;
  uintptr_t f = (uintptr_t)fn;
  uintptr_t text = (uintptr_t)text_base;
  fprintf(stderr, "[b_pthread_create] thread start tid=%ld fn=%p",
          (long)syscall(SYS_gettid), (void *)fn);
  if (f >= text && f < text + text_size)
    fprintf(stderr, " (libtasm2.so+0x%lx)", (unsigned long)(f - text));
...
static int b_pthread_create(pthread_t *th, const void *attr,
  ...
  uintptr_t f = (uintptr_t)fn;
  uintptr_t text = (uintptr_t)text_base;
  uintptr_t off = (text && f >= text && f < text + text_size) ? f - text : 0;
  fprintf(stderr, "[b_pthread_create] caller_tid=%ld attr=%p fn=%p",
          (long)syscall(SYS_gettid), attr, (void *)fn);
  if (f >= text && f < text + text_size)
    fprintf(stderr, " (libtasm2.so+0x%lx)", (unsigned long)off);
  fprintf(stderr, " arg=%p\n", arg);
  pthread_attr_t ga;
  ...
  if (getenv("TASM2_BLOCK_GAIA_THREADS") && off == 0x00cbce34u) {
    int r = pthread_create(th, &ga, thread_noop_bridge, NULL);
    fprintf(stderr,
            "[b_pthread_create] blocked GAIA/GLOT worker -> noop r=%d th=%p\n",
            r, th ? (void *)(uintptr_t)*th : NULL);
    pthread_attr_destroy(&ga);
    return r;
  }
  thread_start_t *ctx = (thread_start_t *)calloc(1, sizeof(*ctx));
  if (!ctx) {
    pthread_attr_destroy(&ga);
    return 12;
  }
  ctx->fn = fn;
  ctx->arg = arg;
  int r = pthread_create(th, &ga, thread_start_bridge, ctx);
  fprintf(stderr, "[b_pthread_create] pthread_create -> %d th=%p\n", r,
          th ? (void *)(uintptr_t)*th : NULL);
  if (r != 0)
    free(ctx);
  pthread_attr_destroy(&ga);
  return r;
