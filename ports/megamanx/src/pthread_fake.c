...
int pthread_cond_wait_fake(void **cslot, void **mslot) {
  { static int mw = -1; if (mw < 0) mw = getenv("MMX_MAINWAIT") ? 1 : 0;
    static int mn = 0; if (mw && mn < 200 && (int)syscall(SYS_gettid) == getpid()) { mn++;
      unsigned long o; const char *l = ra_name((unsigned long)__builtin_return_address(0), &o);
      fprintf(stderr, "[MAINWAIT] #%d cslot=%p caller=%s+0x%lx\n", mn, (void *)cslot, l, o); fsync(2); } }
  /* MMX_SKIPWAIT=lista de "u:0xOFF" (libunity) e/ou "i:0xOFF" (libil2cpp) separados por virgula.
   * O job-system nativo do Unity trava (main em WaitForJobGroup libunity+0x34d1d0; choreographer
   * em libunity+0x35059c; thread da folha em libil2cpp+0xd1ac28) pq o dispatch p/ os workers esta
   * quebrado no so-loader. Waits NORMAIS nesses callers (sinalizados dentro de MMX_SKIPWAIT_MS,
   * default 2000ms) passam intactos; so o STUCK estoura o timeout e PROSSEGUE (return 0). Aplica
   * a QUALQUER thread (a folha 0xd1ac28 nao e a main). */
  {
    static char list[256]; static int parsed = 0;
    if (!parsed) { parsed = 1; const char *e = getenv("MMX_SKIPWAIT");
      if (e) { size_t n = strlen(e); if (n >= sizeof(list)) n = sizeof(list) - 1; memcpy(list, e, n); list[n] = 0; } }
    if (list[0]) {
      unsigned long ra = (unsigned long)__builtin_return_address(0);
      long off = -1; char mod = 0;
      if (g_pt_ub && ra >= g_pt_ub && ra < g_pt_ub + 0x2000000) { off = (long)(ra - g_pt_ub); mod = 'u'; }
      else if (g_pt_ib && ra >= g_pt_ib && ra < g_pt_ib + 0x3000000) { off = (long)(ra - g_pt_ib); mod = 'i'; }
      if (off >= 0) {
        /* casa "u:0xOFF"/"i:0xOFF" na lista */
        int hit = 0; const char *p = list;
        while (*p) {
          char m = 0; if ((p[0] == 'u' || p[0] == 'i') && p[1] == ':') { m = p[0]; p += 2; }
          long v = (long)strtoul(p, (char **)&p, 0);
          if (v == off && (m == 0 || m == mod)) { hit = 1; break; }
          while (*p && *p != ',') p++; if (*p == ',') p++;
        }
        if (hit) {
          static long ms = -2; if (ms == -2) { const char *m = getenv("MMX_SKIPWAIT_MS"); ms = m ? atol(m) : 2000; }
          struct timespec tt; clock_gettime(CLOCK_REALTIME, &tt);
          tt.tv_sec += ms / 1000; tt.tv_nsec += (ms % 1000) * 1000000L;
          if (tt.tv_nsec >= 1000000000L) { tt.tv_sec++; tt.tv_nsec -= 1000000000L; }
          sigset_t old; gc_wait_unblock(&old);
          int r = pthread_cond_timedwait(cond_get(cslot), mtx_get(mslot), &tt);
          gc_wait_restore(&old);
          if (r == ETIMEDOUT) {
            static int once[8]; int oi = (mod == 'i') ? 1 : 0;
            if (once[oi] < 3) { once[oi]++; fprintf(stderr, "[SKIPWAIT] %s+0x%lx timeout %ldms -> PROSSEGUE (tid=%d)\n", mod == 'i' ? "libil2cpp" : "libunity", off, ms, (int)syscall(SYS_gettid)); fsync(2); }
            return 0;
          }
          return r;
        }
