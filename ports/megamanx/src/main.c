...
/* MMX_INLINETASK: MESMO conceito do ter_inline_task, mas p/ o Mega Man X. O loop de
   per-object-task do MMX está em libunity+0x350580 (`ldr x8,[x19,#88]; cbz; ldr x8,[x8];
   cbnz -> sai`), estruturalmente idêntico ao 0x2f37a4 do Terraria. Fingimos a conclusão:
   node=*(obj+88); *node=1 (satisfaz o cbnz em 0x35058c → sai da espera). O contador do
   WaitForJobGroup do MMX (analogo ao 0xc10360 do Terraria) é configurável via MMX_JOBCOUNTER
   (hex offset em libunity); default 0 = não mexe (achar depois se a frame 3 travar). */
static volatile int g_mmx_inline_n = 0;
void mmx_inline_task(void *obj) {
  if (!obj) return;
  void *node = *(void **)((char *)obj + 88);
  if (node) *(void **)node = (void *)1;
  const char *jc = getenv("MMX_JOBCOUNTER");
  if (jc && g_unity_base) {
    uint32_t *cnt = (uint32_t *)(g_unity_base + strtoul(jc, 0, 0));
    __atomic_add_fetch(cnt, 1, __ATOMIC_SEQ_CST);
  }
  int n = __atomic_add_fetch(&g_mmx_inline_n, 1, __ATOMIC_RELAXED);
  if (n <= 5 || (n % 100) == 0) { fprintf(stderr, "[MMX_INLINETASK] #%d obj=%p node=%p\n", n, obj, node); fsync(2); }
}
...
/* MMX_NOINTEGRITY: o BootScene.Start() do Mega Man X chama IntegrityCheck (Google Play Integrity,
 * DRM anti-tamper) que dá NullReferenceException no nosso JNI fake -> a coroutine de boot MORRE ->
 * o jogo nunca carrega a cena real (tela = error-shader magenta). Achamos a classe aninhada da
...
static void mmx_nuke_integrity(void) {
  ...
  static int tries = 0; if (tries++ > 2000) { done = 1; return; }
  ...
  void *(*cls_from_name)(void *, const char *, const char *) = mmx_i2sym("il2cpp_class_from_name");
  ...
  if (!dom_get || !dom_asms || !asm_img || !cls_from_name || !cls_method || !cls_nested || !cls_name) {
    static int lg=0; if(!lg++){fprintf(stderr,"[NOINTEGRITY] il2cpp syms: dom=%p asms=%p img=%p cfn=%p cm=%p cn=%p cnm=%p\n",dom_get,dom_asms,asm_img,cls_from_name,cls_method,cls_nested,cls_name);fsync(2);}
    return;
  }
  void *domain = dom_get(); if (!domain) return;
  size_t na = 0; const void **asms = dom_asms(domain, &na); if (!asms || !na) return;
  /* MMX_RESOLVE: resolve offsets (bl targets do MoveNext do BootScene) -> nomes de método */
  if (getenv("MMX_RESOLVE")) { static int rdone = 0; if (!rdone) { rdone = 1;
    unsigned long want[] = {0xe3b020,0xe3b160,0xe3b2a0,0xe6cc98,0xe6ed2c,0xfef174,0x100ecc4};
    ...
    const char *(*cls_nm)(void *) = mmx_i2sym("il2cpp_class_get_name");
    if (img_ccount && img_class && cls_methods && meth_name) {
      for (size_t a = 0; a < na; a++) { void *im = asm_img(asms[a]); if (!im) continue;
        size_t nc = img_ccount(im);
        for (size_t ci = 0; ci < nc; ci++) { void *cl = img_class(im, ci); if (!cl) continue;
          void *it = NULL, *m;
          while ((m = cls_methods(cl, &it))) { unsigned long mp = (unsigned long)(*(void **)m);
            if (!mp) continue; unsigned long off = mp - g_il2cpp_base;
            for (unsigned w = 0; w < sizeof(want)/sizeof(want[0]); w++)
              if (off == want[w]) { fprintf(stderr, "[RESOLVE] 0x%lx = %s::%s\n", off, cls_nm ? cls_nm(cl) : "?", meth_name(m)); fsync(2); }
          }
        }
      }
      fprintf(stderr, "[RESOLVE] fim\n"); fsync(2);
    }
  } }
  { static int lg=0; if(!lg++){fprintf(stderr,"[NOINTEGRITY] procurando IntegrityCheck em %zu assemblies...\n",na);fsync(2);} }
  for (size_t i = 0; i < na; i++) {
    void *img = asm_img(asms[i]); if (!img) continue;
    /* MMX_BOOTDUMP: dumpa a coroutine <Start>d__ do BootScene (os campos = tasks/awaits que travam) */
    if (getenv("MMX_BOOTDUMP")) { static int bd = 0;
      void *bs = cls_from_name(img, "", "BootScene");
      if (bs && !bd) { bd = 1;
        { void *(*cls_methods3)(void *, void **) = mmx_i2sym("il2cpp_class_get_methods");
          const char *(*meth_name3)(void *) = mmx_i2sym("il2cpp_method_get_name");
          if (cls_methods3 && meth_name3) { void *it0 = NULL, *m0;
            while ((m0 = cls_methods3(bs, &it0))) fprintf(stderr, "[BOOTDUMP] BootScene.method: %s\n", meth_name3(m0)); } }
        void *(*cls_nested2)(void *, void **) = mmx_i2sym("il2cpp_class_get_nested_types");
        const char *(*cls_name2)(void *) = mmx_i2sym("il2cpp_class_get_name");
        void *(*cls_fields2)(void *, void **) = mmx_i2sym("il2cpp_class_get_fields");
        const char *(*field_name2)(void *) = mmx_i2sym("il2cpp_field_get_name");
        void *(*field_type2)(void *) = mmx_i2sym("il2cpp_field_get_type");
        const char *(*type_name2)(void *) = mmx_i2sym("il2cpp_type_get_name");
        void *it = NULL, *nc2;
        while (cls_nested2 && (nc2 = cls_nested2(bs, &it))) {
          const char *nn = cls_name2 ? cls_name2(nc2) : "?";
          if (!nn || !strstr(nn, "Start")) continue;
          fprintf(stderr, "[BOOTDUMP] coroutine %s fields:\n", nn);
          size_t (*field_off)(void *) = mmx_i2sym("il2cpp_field_get_offset");
          void *fit = NULL, *ff;
          while (cls_fields2 && (ff = cls_fields2(nc2, &fit))) {
            const char *fn = field_name2 ? field_name2(ff) : "?";
            const char *tn = (field_type2 && type_name2) ? type_name2(field_type2(ff)) : "?";
            fprintf(stderr, "[BOOTDUMP]   +0x%zx %s : %s\n", field_off ? field_off(ff) : 0, fn, tn);
          }
          /* troca o methodPointer do MoveNext -> mmx_bootmn (loga state, chama original) */
          void *bmm = cls_method(nc2, "MoveNext", 0);
          if (bmm) { void *mp = *(void **)bmm;
            fprintf(stderr, "[BOOTDUMP] MoveNext methodPtr=%p il2cpp_base=0x%lx offset=0x%lx\n",
                    mp, (unsigned long)g_il2cpp_base, (unsigned long)((uintptr_t)mp - g_il2cpp_base)); fsync(2); }
        }
        fsync(2);
      }
    }
    void *cls = cls_from_name(img, "", "IntegrityCheck"); if (!cls) continue;
    fprintf(stderr,"[NOINTEGRITY] classe IntegrityCheck achada (asm %zu)\n",i); fsync(2);
    /* MMX_INTDUMP: enumera métodos da IntegrityCheck (achar o gate de resultado) */
    if (getenv("MMX_INTDUMP")) {
      void *(*cls_methods)(void *, void **) = mmx_i2sym("il2cpp_class_get_methods");
      const char *(*meth_name)(void *) = mmx_i2sym("il2cpp_method_get_name");
      void *(*cls_fields)(void *, void **) = mmx_i2sym("il2cpp_class_get_fields");
      const char *(*field_name)(void *) = mmx_i2sym("il2cpp_field_get_name");
      if (cls_methods && meth_name) { void *it2 = NULL, *m2;
        while ((m2 = cls_methods(cls, &it2))) fprintf(stderr, "[INTDUMP] method: %s\n", meth_name(m2)); }
      if (cls_fields && field_name) { void *it3 = NULL, *f2;
        while ((f2 = cls_fields(cls, &it3))) fprintf(stderr, "[INTDUMP] field: %s\n", field_name(f2)); }
      fsync(2);
    }
    /* 🔑 patcha get_IsSuccess -> retorna TRUE (integrity "passou") senão o anti-tamper do BootScene
       nao carrega a cena. mov w0,#1; ret */
    /* MMX_NOTERMS: nuka viewTerms/viewParentCtrl (telas de Termos/Controle Parental do BootScene
       que travam esperando aceitação numa UI que não renderiza no so-loader) -> ret. */
    if (getenv("MMX_NOTERMS")) {
      void *bs2 = cls_from_name(img, "", "BootScene");
      const char *vm[] = { "viewTerms", "viewParentCtrl", "viewError", 0 };
      for (int k = 0; bs2 && vm[k]; k++) {
        for (int argc = 0; argc <= 2; argc++) {
          void *vmeth = cls_method(bs2, vm[k], argc); if (!vmeth) continue;
          uint32_t *vp = *(uint32_t **)vmeth; if (!vp) continue;
          long pg = sysconf(_SC_PAGESIZE); void *va = (void *)((uintptr_t)vp & ~((uintptr_t)pg - 1));
          mprotect(va, pg * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
          vp[0] = 0xaa1f03e0u; vp[1] = 0xaa1f03e1u; vp[2] = 0xD65F03C0u;  /* mov x0,xzr; mov x1,xzr; ret = UniTask completo */
          mprotect(va, pg * 2, PROT_READ | PROT_EXEC); __builtin___clear_cache((char *)va, (char *)va + pg * 2);
          fprintf(stderr, "[NOTERMS] BootScene.%s(argc=%d) -> UniTask completo\n", vm[k], argc); fsync(2);
        }
      }
    }
    void *gs = cls_method(cls, "get_IsSuccess", 0);
    if (gs) { uint32_t *gp = *(uint32_t **)gs;
      if (gp) { long pg = sysconf(_SC_PAGESIZE); void *ga = (void *)((uintptr_t)gp & ~((uintptr_t)pg - 1));
        mprotect(ga, pg * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
    ...
    /* 🔑 IntegrityCheck::Start retorna um IEnumerator (coroutine Unity) — o BootScene faz
       ...
       Fix CERTO: DEIXAR Start() rodar (cria+retorna o enumerator real, NÃO-null) e patchar o
       MoveNext do state-machine aninhado <Start>d__N -> `mov w0,#0; ret` (MoveNext=false =
       enumeração terminada NA HORA). Assim o await recebe um enumerator válido que completa
       imediatamente, SEM rodar a chamada do Play Integrity (que crasha no so-loader). */
    { void *nit = NULL, *nc;
      int patched = 0;
      while (cls_nested && (nc = cls_nested(cls, &nit))) {
        const char *ncn = cls_name ? cls_name(nc) : NULL;
        if (!ncn || !strstr(ncn, "Start")) continue;
        void *mn = cls_method(nc, "MoveNext", 0); if (!mn) continue;
        uint32_t *mp = *(uint32_t **)mn; if (!mp) continue;
        long pgsz = sysconf(_SC_PAGESIZE); void *pa = (void *)((uintptr_t)mp & ~((uintptr_t)pgsz - 1));
        mprotect(pa, pgsz * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
        mp[0] = 0x52800000u;  /* mov w0, #0 (MoveNext -> false = coroutine done) */
        mp[1] = 0xD65F03C0u;  /* ret */
        mprotect(pa, pgsz * 2, PROT_READ | PROT_EXEC); __builtin___clear_cache((char *)pa, (char *)pa + pgsz * 2);
...
static long my_syscall(long n, long a1, long a2, long a3, long a4, long a5, long a6) {
  ...
  if (n == SYS_futex) {
    ...
    if (op == 0 || op == 9) {   /* FUTEX_WAIT / FUTEX_WAIT_BITSET: thread vai BLOQUEAR */
      long t4 = a4;
      struct timespec ts;
      /* poll do futex: g_futexpoll_ms (TER_FUTEXPOLL, todas as threads) OU MMX_WORKERPOLL (SÓ as
       * threads NÃO-main: Job.Worker/Loading.* ficam presas em futex esperando trabalho que o
       * dispatch quebrado do so-loader nunca sinaliza; acordando-as elas re-checam a fila e
       * PROCESSAM os jobs/load da cena. A MAIN fica intacta senão o boot quebra). */
      long wpoll = g_futexpoll_ms;
      if (!wpoll) { const char *wp = getenv("MMX_WORKERPOLL");
        if (wp && (int)syscall(178 /*gettid arm64*/) != getpid()) wpoll = atoi(wp); }
      if (wpoll && a4 == 0) {  /* injeta timeout (poll anti-lost-wakeup) */
        if (op == 0) { ts.tv_sec = wpoll / 1000; ts.tv_nsec = (wpoll % 1000) * 1000000L; }
        else { clock_gettime(((int)a2 & 256) ? CLOCK_REALTIME : CLOCK_MONOTONIC, &ts);
               ts.tv_sec += wpoll / 1000; ts.tv_nsec += (wpoll % 1000) * 1000000L;
...
static int my_statfs64(const char *p, void *buf) {
  ...
  int rc = real ? real("/storage/roms/megamanx", buf) : -1;
...
static const unsigned char *my_glGetString(unsigned n){
  ...
  /* MMX_GLLOG: loga o que o Mali REAL reporta (VENDOR/RENDERER/VERSION/GLSL) — decide o
     platform de shader que o Unity deseja (2=GLES2/platform4, 3=GLES3/platform5). */
  if (getenv("MMX_GLLOG") && (n==0x1F00||n==0x1F01||n==0x1F02||n==0x8B8C)) {
    static int seen[4]; int i=(n==0x1F00)?0:(n==0x1F01)?1:(n==0x1F02)?2:3;
    if(!seen[i]){seen[i]=1; fprintf(stderr,"[GLLOG] glGetString(0x%X)=\"%s\"\n", n, s?(const char*)s:"(null)"); fsync(2);}
  }
  int i = glstr_idx(n);
  ...
  /* MMX_FAKEGLES3: reporta ES 3.0 -> Unity cria GfxDevice GLES3 -> deseja platform 9 (os shaders
     GLES3 #version 300 es que o JOGO TEM). O my_glShaderSource traduz p/ ES2 (#version 100) pro
     Mali-450 aceitar. Sem isto o Unity fica GLES2 (platform 5) e os shaders do jogo (só 9/18) somem. */
  if (getenv("MMX_FAKEGLES3")) {
 ...
 * em /storage/roms/megamanx/shot.ppm (verificação autônoma de IMAGEM no fbdev Mali). */
static void ter_screenshot_maybe(void) {
  ...
  FILE *f = fopen("/storage/roms/megamanx/shot.ppm","wb");
  if (f) { fprintf(f,"P6\n%d %d\n255\n", w, h);
...
/* MMX_FORCEGLES2VER: força o Unity a VER GL major=2 (GLES2) via glGetIntegerv, pra ele criar
   o GfxDevice GLES2 e desejar shaders platform 4. O Mali-450 finge GLES3 (reporta major=3). */
static void (*r_glGetIntegerv)(unsigned, int *) = 0;
static void my_glGetIntegerv(unsigned pname, int *params) {
  if (!r_glGetIntegerv) r_glGetIntegerv = (void (*)(unsigned, int *))dlsym(RTLD_DEFAULT, "glGetIntegerv");
  if (r_glGetIntegerv) r_glGetIntegerv(pname, params);
  if (params) {
    if (pname == 0x821B) *params = 2;   /* GL_MAJOR_VERSION -> 2 */
    else if (pname == 0x821C) *params = 0;  /* GL_MINOR_VERSION -> 0 */
  }
}
...
static void *my_eglGetProcAddress(const char *nm) {
  if (nm && getenv("MMX_FORCEGLES2VER") && !strcmp(nm, "glGetIntegerv")) {
    if (!r_glGetIntegerv) r_glGetIntegerv = (void (*)(unsigned, int *))(r_eglGetProcAddress ? r_eglGetProcAddress(nm) : dlsym(RTLD_DEFAULT, nm));
    return (void *)my_glGetIntegerv;
  }
  /* MMX_XLATE: wira glShaderSource SEMPRE (não só sob drawspy) p/ traduzir GLES3 GLSL->GLES2. */
  if (nm && getenv("MMX_XLATE") && !strcmp(nm, "glShaderSource")) {
    if (!r_glShaderSource) r_glShaderSource = (void (*)(unsigned, int, const char **, const int *))(r_eglGetProcAddress ? r_eglGetProcAddress(nm) : dlsym(RTLD_DEFAULT, nm));
  ...
  /* MMX_NOGLES3: o Mali-450 (Utgard, GLES2 real) EXPORTA símbolos GLES3 falsos -> o Unity acha
   * que GLES3 funciona e cria o GfxDevice GLES3, desejando shaders "platform 5" (GLES3) que o
   * blob não tem -> tela uniforme. Retornando NULL nas funções GLES3-only, o init do device GLES3
   * do Unity FALHA e ele cai p/ o GfxDevice GLES2 (deseja platform 4). */
  if (nm && p && getenv("MMX_NOGLES3")) {
    static const char *g3[] = {
      "glMapBufferRange", "glUnmapBuffer", "glFlushMappedBufferRange", "glVertexAttribDivisor",
      "glDrawArraysInstanced", "glDrawElementsInstanced", "glGenVertexArrays", "glBindVertexArray",
      "glDeleteVertexArrays", "glGenSamplers", "glBindSampler", "glDeleteSamplers", "glSamplerParameteri",
      "glTexStorage2D", "glTexStorage3D", "glGetStringi", "glInvalidateFramebuffer",
      "glRenderbufferStorageMultisample", "glBlitFramebuffer", "glGenQueries", "glBeginQuery",
      "glDrawBuffers", "glReadBuffer", "glTexImage3D", "glTexSubImage3D", "glCompressedTexImage3D",
      "glGenTransformFeedbacks", "glBindTransformFeedback", "glVertexAttribIPointer",
      "glUniformMatrix3x4fv", "glUniformMatrix4x3fv", "glGetUniformBlockIndex", "glUniformBlockBinding",
      "glBindBufferRange", "glBindBufferBase", "glFenceSync", "glClientWaitSync", "glDeleteSync",
      "glProgramBinary", "glGetProgramBinary", "glProgramParameteri", 0 };
    for (int i = 0; g3[i]; i++) if (!strcmp(nm, g3[i])) { p = NULL; break; }
  }
  if (g_egp_n++ < 400)
...
static void *my_dlsym(void *h, const char *nm) {
  ...
  /* MMX_XLATE: o Unity resolve GL via dlsym(RTLD_DEFAULT) — ESTE é o caminho (não eglGetProcAddress).
     Wira glShaderSource p/ traduzir GLES3 GLSL->GLES2, e glCompile/glLink p/ logar erros. */
  if (getenv("MMX_XLATE")) {
...
int main(int argc, char **argv) {
  ...
  /* MMX_PATCH="0xOFF=0xWORD,0xOFF=0xWORD": escreve words de 32 bits crus no .text da libunity.
   * Usado p/ destravar os loops de espera do so-loader que nunca recebem sinal:
   *   Swappy thread-ready: 0x34d1bc cbnz->b(saida 0x34d1d8) = 0x14000007
   *   Choreographer msg-queue: 0x35059c b(loop)->b(saida 0x3505a0) = 0x14000001 */
  if (getenv("MMX_PATCH")) {
    const char *p = getenv("MMX_PATCH");
    so_make_text_writable();
    while (*p) {
      unsigned long off = strtoul(p, (char **)&p, 0);
      if (*p == '=') { p++;
        unsigned long word = strtoul(p, (char **)&p, 0);
        *(uint32_t *)((uintptr_t)text_base + off) = (uint32_t)word;
        fprintf(stderr, "[MMX_PATCH] libunity+0x%lx = 0x%08lx\n", off, word);
      }
      while (*p && *p != ',') p++; if (*p == ',') p++;
    }
    so_make_text_executable(); so_flush_caches();
  ...
  jni_shim_set_package("jp.co.capcom.rockmanx", 0);
  ...
  /* MMX_INLINETASK: trampolim no per-object-task loop do MMX (libunity+0x350580, default;
     override via MMX_INLINETASK_OFF). Chama mmx_inline_task(obj) que finge a conclusão
     (node->next=1 → sai da espera 0x35059c) + executa a instrução original (ldr x8,[x19,#88])
     e volta. Análogo ao TER_INLINETASK. */
  if (getenv("MMX_INLINETASK") && g_unity_base) {
    extern void mmx_inline_task(void *);
    long pgsz = sysconf(_SC_PAGESIZE);
    unsigned long off = getenv("MMX_INLINETASK_OFF") ? strtoul(getenv("MMX_INLINETASK_OFF"), 0, 0) : 0x350580;
    uintptr_t patch = g_unity_base + off;
    uintptr_t hint = (g_unity_base + 0x2200000) & ~((uintptr_t)pgsz - 1);
    void *tp = mmap((void *)hint, pgsz, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (tp == MAP_FAILED) tp = mmap(NULL, pgsz, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    long d = (long)((uintptr_t)tp) - (long)patch;
    if (tp != MAP_FAILED && d > -0x7000000L && d < 0x7000000L) {
      uint32_t *t = (uint32_t *)tp;
      t[0] = 0xF81F0FFEu;          /* str x30,[sp,#-16]! */
      t[1] = 0xAA1303E0u;          /* mov x0, x19 (obj)  */
      t[2] = 0x580000D0u;          /* ldr x16,[pc+0x18] -> fn */
      t[3] = 0xD63F0200u;          /* blr x16 */
      t[4] = 0xF84107FEu;          /* ldr x30,[sp],#16 */
      t[5] = 0xF9402E68u;          /* ldr x8,[x19,#88] (instr original de 0x350580) */
      t[6] = 0x58000090u;          /* ldr x16,[pc+0x10] -> dst */
      t[7] = 0xD61F0200u;          /* br x16 */
      *(uint64_t *)((char *)tp + 0x20) = (uint64_t)(uintptr_t)mmx_inline_task;
      *(uint64_t *)((char *)tp + 0x28) = (uint64_t)(patch + 4);   /* volta p/ 0x350584 */
      __builtin___clear_cache((char *)tp, (char *)tp + pgsz);
      void *pp = (void *)(patch & ~((uintptr_t)pgsz - 1));
      mprotect(pp, pgsz * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
      *(uint32_t *)patch = 0x14000000u | (uint32_t)(((d) >> 2) & 0x03FFFFFF);
      mprotect(pp, pgsz * 2, PROT_READ | PROT_EXEC);
      __builtin___clear_cache((char *)pp, (char *)pp + pgsz * 2);
      fprintf(stderr, "[MMX_INLINETASK] trampolim @%p (d=0x%lx) patch libunity+0x%lx->b\n", tp, d, off);
    } else {
      fprintf(stderr, "[MMX_INLINETASK] FALHOU mmap/alcance (tp=%p d=0x%lx)\n", tp, d);
    }
  }
