#!/bin/bash
# Build RELEASE do LEGO Batman 2 (arm64) no Docker debian:buster — glibc ≤2.30
...
gen() {
  "$NM" -D --undefined-only ./legobatman2 |
...
"$CC" -fPIE -pie -O2 -fPIC -fno-omit-frame-pointer -rdynamic -D_GNU_SOURCE \
  ...
  -o legobatman2.release $SRCS \
  -Isrc -Isrc/hooks -I"$HDR" -I"$HDR/SDL2" \
  -Wl,--export-dynamic -Wl,--as-needed \
  -L"$STUB" -lSDL2 -lEGL -lGLESv2 \
  -ldl -lm -lpthread -lgcc

echo "BUILD OK -> legobatman2.release"
echo "  arch:      $(file legobatman2.release 2>/dev/null | head -c 80)"
echo "  GLIBC max: $($READELF -V legobatman2.release | grep -oE 'GLIBC_[0-9.]+' | sed 's/GLIBC_//' | sort -uV | tail -1)"
echo "  tamanho:   $(stat -c%s legobatman2.release) bytes"
"$READELF" -d legobatman2.release | grep NEEDED