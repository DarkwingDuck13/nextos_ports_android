...
/* GL single-context handoff (LB2/Fusion): antes de BLOQUEAR num lock/cond, a
 * render-thread solta o contexto GL p/ o loader Fusion poder pega-lo. Sem isto
 * o render segura o ctx a frame inteiro e o loader trava -> deadlock (0 swaps).
 * trylock rapido primeiro evita o park no caso comum sem contencao. */
extern void egl_shim_gl_park(void);
int b_mutex_lock(void *m) {
  pthread_mutex_t *r = mtx_real(m);
  if (pthread_mutex_trylock(r) == 0) return 0;
  egl_shim_gl_park();
...
int b_cond_wait(void *c, void *m) {
  egl_shim_gl_park(); /* vai bloquear -> solta o ctx GL p/ o loader */
