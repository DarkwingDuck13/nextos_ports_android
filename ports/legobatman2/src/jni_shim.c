...
enum {
  ...
  MID_IS_FIRETV,
  MID_IS_HEADSET,
  /* LB2 (engine Fusion, com.wbgames.LEGOgame): callbacks C++ -> Java */
  MID_START_MOVIE,   /* Renderer.startMoviePlayback(Str,Z,Str)Z -> false=skip */
  MID_STOP_MOVIE,
  MID_MOVIE_ATTRS,   /* setMoviePlaybackAttributes(F) */
  MID_SHOW_TITLE,    /* setShowTitleScreen(Z) */
  MID_SHUTDOWN_APP,
  MID_LANG_ISO3,     /* FusionCallback.GetCurrentLanguageAsISO3Letter -> "eng" */
  MID_DEVICE_REGION, /* FusionCallback.GetDeviceRegion -> "USA" */
  MID_OPEN_BROWSER,
  MID_ASSET_FD,      /* LEGOgame.a.a(String) -> AssetFileDescriptor (NULL) */
  MID_GENERIC,
  FID_OBB_VERSIONCODE,
  FID_GENERIC,
};
...
/* caminho do OBB no device (a engine fopen()a exatamente esta string).
 * override por COI_OBB. */
static const char *coi_obb_path(void) {
  extern char *getenv(const char *);
  const char *e = getenv("COI_OBB");
  return e ? e : "/storage/roms/ports/castleofillusion/obb/main.154.obb";
}
...
static void *jni_GetStaticMethodID(void *env, void *clazz, const char *name,
  ...
  /* LB2 Fusion: superfice Java chamada pelo C++ */
  if (strcmp(name, "startMoviePlayback") == 0)
    return &g_method_tags[MID_START_MOVIE];
  if (strcmp(name, "stopMoviePlayback") == 0)
    return &g_method_tags[MID_STOP_MOVIE];
  if (strcmp(name, "setMoviePlaybackAttributes") == 0)
    return &g_method_tags[MID_MOVIE_ATTRS];
  if (strcmp(name, "setShowTitleScreen") == 0)
    return &g_method_tags[MID_SHOW_TITLE];
  if (strcmp(name, "shutdownApp") == 0)
    return &g_method_tags[MID_SHUTDOWN_APP];
  if (strcmp(name, "GetCurrentLanguageAsISO3Letter") == 0)
    return &g_method_tags[MID_LANG_ISO3];
  if (strcmp(name, "GetDeviceRegion") == 0)
    return &g_method_tags[MID_DEVICE_REGION];
  if (strcmp(name, "OpenWebBrowser") == 0)
    return &g_method_tags[MID_OPEN_BROWSER];
  if (strcmp(name, "a") == 0 && sig && strstr(sig, "AssetFileDescriptor"))
    return &g_method_tags[MID_ASSET_FD];
  return &g_method_tags[MID_GENERIC];
}
...
static void *jni_CallStaticObjectMethod(void *env, void *clazz,
  ...
  if (methodID == &g_method_tags[MID_LANG_ISO3]) {
    debugPrintf("jni_shim: GetCurrentLanguageAsISO3Letter -> \"eng\"\n");
    return make_jstring("eng");
  }
  if (methodID == &g_method_tags[MID_DEVICE_REGION]) {
    debugPrintf("jni_shim: GetDeviceRegion -> \"USA\"\n");
    return make_jstring("USA");
  }
  if (methodID == &g_method_tags[MID_ASSET_FD]) {
    debugPrintf("jni_shim: LEGOgame.a(AssetFileDescriptor) -> NULL\n");
    return NULL;
  }
...
static unsigned char jni_CallStaticBooleanMethod(void *env, void *clazz,
  ...
  if (methodID == &g_method_tags[MID_IS_HEADSET])
    return 0;
  /* LB2: startMoviePlayback -> TRUE = "filme iniciou" (receita lswtfa: reportar
   * sucesso e depois terminar na hora). false fazia a engine ABORTAR. O main
   * loop chama nativeSkippedMovie no proximo frame (g_movie_skip_pending) p/ a
   * engine avancar sem o player Java. */
  if (methodID == &g_method_tags[MID_START_MOVIE]) {
    extern volatile int g_movie_skip_pending;
    g_movie_skip_pending = 1;
    debugPrintf("jni_shim: startMoviePlayback -> true (FMV vai ser pulado)\n");
    return 1;
  }
...
static void jni_CallStaticVoidMethod(void *env, void *clazz, void *methodID,
  ...
  (void)env;
  (void)clazz;
  if (methodID == &g_method_tags[MID_SHUTDOWN_APP]) {
    debugPrintf("jni_shim: shutdownApp() -> exit\n");
    extern void _exit(int);
    _exit(0);
  }
  if (methodID == &g_method_tags[MID_STOP_MOVIE]) {
    debugPrintf("jni_shim: stopMoviePlayback (noop)\n");
    return;
  }
  if (methodID == &g_method_tags[MID_SHOW_TITLE]) {
    debugPrintf("jni_shim: setShowTitleScreen (noop)\n");
    return;
  }
  if (methodID == &g_method_tags[MID_MOVIE_ATTRS]) {
    debugPrintf("jni_shim: setMoviePlaybackAttributes (noop)\n");
    return;
  }
...
/* LB2: engine armazena o JavaVM pego via env->GetJavaVM (indice 219) e usa
 * em toda thread (fnaThread_ThreadProc: vm->AttachCurrentThread). */
static jint jni_GetJavaVM(void *env, void **out_vm) {
  (void)env;
  extern void *jni_shim_vm_ptr(void);
  *out_vm = jni_shim_vm_ptr();
...
void *jni_shim_vm_ptr(void) { return java_vm_ptr ? (void *)&java_vm_ptr : NULL; }
...
void jni_shim_init(void **out_vm, void **out_env) {
  ...
  jni_env_vtable[219] = (uintptr_t)jni_GetJavaVM;               /* LB2: engine pega o VM por aqui */
