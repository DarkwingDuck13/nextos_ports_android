...
// single-context handoff (Fusion loader-thread): render-thread adquire antes de
// emitir GL; release_if_wanted cede se outra thread esta esperando.
void egl_shim_gl_acquire(void);
void egl_shim_gl_release_if_wanted(void);
void egl_shim_gl_park(void); /* solta o ctx antes de bloquear (mutex/cond wait) */
