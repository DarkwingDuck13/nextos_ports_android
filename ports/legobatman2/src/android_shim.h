...
/* Layout CLASSICO do NDK android_native_app_glue (bionic LP64): o COI (engine oz)
 * traz o glue estatico proprio (android_app_pre_exec_cmd/post_exec_cmd) que le/
 * escreve nestes offsets EXATOS. bionic LP64 pthread_mutex_t=40B, cond_t=48B
 * (== glibc), entao os offsets batem. window@72, activityState@96, pendingWindow@280. */
struct android_app {
  void *userData;                                        /* 0   */
  void (*onAppCmd)(struct android_app *app, int32_t cmd);/* 8   */
  int32_t (*onInputEvent)(struct android_app *app, AInputEvent *event); /* 16 */
  ANativeActivity *activity;                             /* 24  */
  AConfiguration *config;                                /* 32  */
  void *savedState;                                      /* 40  */
  size_t savedStateSize;                                 /* 48  */
  ALooper *looper;                                       /* 56  */
  AInputQueue *inputQueue;                               /* 64  */
  ANativeWindow *window;                                 /* 72  */
  int32_t contentRect[4];                                /* 80..95  */
  int activityState;                                     /* 96  */
  int destroyRequested;                                  /* 100 */
  char mutex[40];                                        /* 104 (bionic pthread_mutex_t) */
  char cond[48];                                         /* 144 (bionic pthread_cond_t)  */
  int msgread;                                           /* 192 */
  int msgwrite;                                          /* 196 */
  unsigned long thread;                                  /* 200 (pthread_t) */
  struct android_poll_source cmdPollSource;              /* 208 */
  struct android_poll_source inputPollSource;            /* 232 */
  int running;                                           /* 256 */
  int stateSaved;                                        /* 260 */
  int destroyed;                                         /* 264 */
  int redrawNeeded;                                      /* 268 */
  AInputQueue *pendingInputQueue;                        /* 272 */
  ANativeWindow *pendingWindow;                          /* 280 */
  int32_t pendingContentRect[4];                         /* 288..303 */
};
