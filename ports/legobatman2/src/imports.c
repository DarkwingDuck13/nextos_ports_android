/* imports.c -- libProject_Douglas_HH.so import resolution (Linux/SDL port)
 *
 * Binds every undefined dynamic symbol of TFA's libProject_Douglas_HH.so. GL
 * goes through the serialized hooks (hooks/egl.c); audio to the SDL-backed
 * OpenSL ES bridge (opensles_shim.c); threads/fs/locale through the bionic
 * shims; the rest to glibc.
 *
...
// glibc-provided; bound by address
extern const char *lb2_bionic_ctype;
...
static void stack_chk_fail_fake(void) {
  // the false-positive canary branches are NOP'd in main.c; this should never
  // run. Log rather than abort if it somehow does.
  debugPrintf("!!! __stack_chk_fail reached (ignored)\n");
}
...
/* gettimeofday com CLAMP de picos de delta.
 *
 ...
 * "completa" no spawn. Servimos um relógio VIRTUAL monotônico que acompanha o
 * real em ritmo normal, mas engole qualquer salto > 100ms (load/stall),
 * limitando-o a um frame de 30fps. Assim o 1º frame pós-load tem dt normal. */
static uint64_t g_vt_us = 0, g_vt_last_real = 0;
static volatile int g_vt_lock = 0;
static int gettimeofday_clamp(struct timeval *tv, void *tz) {
  (void)tz;
  struct timeval r;
  gettimeofday(&r, NULL);
  uint64_t now = (uint64_t)r.tv_sec * 1000000ULL + (uint64_t)r.tv_usec;
  while (__sync_lock_test_and_set(&g_vt_lock, 1)) { /* spin */ }
  if (g_vt_last_real == 0) { g_vt_last_real = now; g_vt_us = now; }
  uint64_t d = (now > g_vt_last_real) ? (now - g_vt_last_real) : 0;
  g_vt_last_real = now;
  if (d > 100000ULL) d = 33333ULL; /* salto de load -> 1 frame */
  g_vt_us += d;
  uint64_t out = g_vt_us;
  __sync_lock_release(&g_vt_lock);
...
DynLibFunction dynlib_functions[] = {
  ...
  { "atan2f", (uintptr_t)&atan2f }, { "atanf", (uintptr_t)&atanf },
  { "ceill", (uintptr_t)&ceill }, { "cosf", (uintptr_t)&cosf },
  { "expf", (uintptr_t)&expf }, { "floorl", (uintptr_t)&floorl },
  { "fmodl", (uintptr_t)&fmodl }, { "log10f", (uintptr_t)&log10f },
  { "log10l", (uintptr_t)&log10l }, { "logf", (uintptr_t)&logf },
  { "powf", (uintptr_t)&powf }, { "powl", (uintptr_t)&powl },
  { "sincosf", (uintptr_t)&sincosf_fake }, { "sinf", (uintptr_t)&sinf },

  // memory / stdlib
  { "abort", (uintptr_t)&abort_log }, { "atof", (uintptr_t)&atof },
  { "atoi", (uintptr_t)&atoi }, { "bsearch", (uintptr_t)&bsearch },
  ...
  // extras LB2 (imports que o TFA nao usava)
  { "_ctype_", (uintptr_t)&lb2_bionic_ctype },
  { "time", (uintptr_t)&time }, { "localtime", (uintptr_t)&localtime },
  { "memchr", (uintptr_t)&memchr }, { "fprintf", (uintptr_t)&fprintf_fake },
