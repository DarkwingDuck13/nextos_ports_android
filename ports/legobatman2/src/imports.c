...
/* LB2: IIDs extras — identidades REAIS do opensles_shim do lswtfa (mesma engine
 * Fusion): o GetInterface do shim compara por ponteiro-identidade. */
extern const void *sl_IID_PLAYBACKRATE, *sl_IID_SEEK;
static const void *SL_IID_VOLUME_v, *SL_IID_PLAYBACKRATE_v, *SL_IID_SEEK_v;
static char *lb2_strrchr_chk(const char *s, int c, unsigned long sz) {
  (void)sz;
  return strrchr(s, c);
}
__attribute__((constructor)) static void sl_iid_init(void) {
  ...
  SL_IID_VOLUME_v = sl_IID_VOLUME; /* LB2: volume real do shim */
  SL_IID_PLAYBACKRATE_v = sl_IID_PLAYBACKRATE;
  SL_IID_SEEK_v = sl_IID_SEEK;
}
...
DynLibFunction coi_overrides[] = {
  ...
  /* LB2 (Fusion): IIDs extras importados DIRETO pelo .so (identidade) +
   * fortify que faltou */
  {"SL_IID_VOLUME", (uintptr_t)&SL_IID_VOLUME_v},
  {"SL_IID_PLAYBACKRATE", (uintptr_t)&SL_IID_PLAYBACKRATE_v},
  {"SL_IID_SEEK", (uintptr_t)&SL_IID_SEEK_v},
  {"__strrchr_chk", (uintptr_t)lb2_strrchr_chk},
};
