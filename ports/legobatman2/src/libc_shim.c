...
int fprintf_fake(FILE *f, const char *fmt, ...) {
  ...
  int r = vfprintf_fake(f, fmt, va);
  va_end(va);
  return r;
}
/* rwlock bionic -> glibc direto: inicializador estatico e all-zeros nos dois */
int pthread_rwlock_rdlock_fake(void **rw) { return pthread_rwlock_rdlock((pthread_rwlock_t *)rw); }
int pthread_rwlock_wrlock_fake(void **rw) { return pthread_rwlock_wrlock((pthread_rwlock_t *)rw); }
int pthread_rwlock_unlock_fake(void **rw) { return pthread_rwlock_unlock((pthread_rwlock_t *)rw); }
/* bionic _ctype_: ponteiro p/ tabela [(c)+1] (c em -1..255) */
static char lb2_ctype_table[1 + 256];
const char *lb2_bionic_ctype = lb2_ctype_table;
__attribute__((constructor)) static void lb2_ctype_init(void) {
  lb2_ctype_table[0] = 0;
  for (int c = 0; c < 256; c++) {
    unsigned v = 0;
    if (isupper(c)) v |= 0x01;
