 ...
 *
 * LEGO Harry Potter 5-7 (libLEGO_HP2.so) is a classic GLSurfaceView client using
 ...
 * single-context handover stubs the pthread bridge references (no-ops here: a
 * fixed-function pipeline has no async shader-preload deadlock like the ES2 TFA).
 *
...
int egl_bringup(void) {
  ...
  g_window = SDL_CreateWindow("LEGO Harry Potter 5-7",
...
void egl_present(void) {
  // Amlogic fbdev OSD blends fb0 by PIXEL ALPHA; force the alpha plane opaque
  // pre-swap so the composited frame scans out instead of "transparent" black.
  {
