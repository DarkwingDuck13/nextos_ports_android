...
static void re4_gp_poll(void){
  ...
  if(g_re4_gp_ctrl && !getenv("RE4_GP_VIRTONLY")){
    ...
    /* DIAG GUIADO: loga QUALQUER mudanca de botao/hat/eixo cru (RE4_RAWALL).
       Permite descobrir exatamente o que o adaptador do NextOS envia por input. */
...
int main(void){
  ...
  { size_t msz=24*1024*1024; void *mh=mmap(NULL,msz,PROT_READ|PROT_WRITE|PROT_EXEC,MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
    if(mh!=MAP_FAILED && so_load("libmonobdwgc-2.0.so",mh,msz)>=0){ so_relocate(); so_resolve(dynlib_functions,dynlib_numfunctions,0);
      { uintptr_t a;
  ...
  jni_shim_set_package((pkg&&pkg[0])?pkg:"com.jam.l4d2", obb_version);
  fprintf(stderr,"[JNI] package=%s obb=%d gamedir=%s\n",(pkg&&pkg[0])?pkg:"com.jam.l4d2",obb_version,re4_gamedir());
