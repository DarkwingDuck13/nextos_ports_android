#!/bin/bash
# Build aarch64 so-loader for Summertime Saga (Ren'Py/SDL2 Android).
set -e
...
for s in SDL_CreateWindow SDL_DestroyWindow SDL_GetDesktopDisplayMode SDL_GetError \
         ...
         SDL_PauseAudioDevice SDL_LockAudioDevice SDL_UnlockAudioDevice; do echo "void $s(void){}"; done > "$STUB/sdl.c"
...
$CC --sysroot="$SR" -D_GNU_SOURCE -I src -I "$SR/usr/include" -I "$SR/usr/include/SDL2" \
    ...
    -o summertimesaga $SRCS \
    -L "$STUB" -lSDL2 -lGLESv2 -lGLESv1_CM -ldl -lm -lpthread -lstdc++ -lgcc_s
rm -rf "$STUB"

echo "BUILD OK -> $(file summertimesaga | cut -d, -f1-3)"
