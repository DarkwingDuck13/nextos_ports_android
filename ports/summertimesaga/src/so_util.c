...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  size_t so_size = 0;
  int text_segno = -1;
  uintptr_t nonexec_start = UINTPTR_MAX;
  uintptr_t nonexec_end = 0;
  ...
  debugPrintf(
      "so_load: ELF64 AArch64, %d program headers, %d section headers\n",
  ...
  for (int i = 0; i < elf_hdr->e_phnum; i++) {
    if (prog_hdr[i].p_type == PT_LOAD) {
      const uintptr_t seg_start = prog_hdr[i].p_vaddr;
      const uintptr_t seg_end =
          prog_hdr[i].p_vaddr + ALIGN_MEM(prog_hdr[i].p_memsz,
                                          prog_hdr[i].p_align);
      ...
      if ((prog_hdr[i].p_flags & PF_X) == PF_X) {
        ...
        if (text_segno < 0)
          goto err_free_so;
        if (seg_start < nonexec_start)
          nonexec_start = seg_start;
        if (seg_end > nonexec_end)
          nonexec_end = seg_end;
      }
    }
  }
  if (text_segno < 0 || nonexec_start == UINTPTR_MAX || nonexec_end <= nonexec_start)
    goto err_free_so;
  ...
  data_size = nonexec_end - nonexec_start;
  data_virtbase = (void *)(nonexec_start + (Elf64_Addr)load_virtbase);
  data_base = (void *)(nonexec_start + (Elf64_Addr)load_base);
    ...
    if (strcmp(sh_name, ".rela.dyn") == 0 ||
      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_ABS64:
          // Undefined imports (e.g. malloc) carry st_value 0; leave them for
          // so_resolve, otherwise they'd resolve to text_base+0 (ELF header).
      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_JUMP_SLOT: {
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (!found) {
              /* Fallback: resolve from the host's already-linked libraries
                 (libc/libm/libpthread/libz/libGLESv2/libGLESv1_CM/libEGL...).
                 The explicit table above always takes priority for shims. */
            ...
            if (!found)
              fprintf(stderr,
                      "*** UNRESOLVED import: \"%s\" (GOT offset 0x%lx) ***\n",
