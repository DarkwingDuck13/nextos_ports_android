...
static FILE *my_fopen(const char *pathname, const char *mode) {
  ...
  if (pathname) {
    ...
    /* BGM: a musica de fundo (music_2 .akb) abre aqui -> toca o PCM em loop
     * (bypass do streaming SQEX flaky). */
    if (n > 4 && strcasecmp(pathname + n - 4, ".akb") == 0 && strstr(pathname, "music_2")) {
      extern void ff7_bgm_set_akb(const char *);
...
void *AAssetManager_open_fake(void *mgr, const char *filename, int mode) {
  ...
  if (f && filename) {
    ...
    if (fn > 4 && strcasecmp(filename + fn - 4, ".akb") == 0 && strstr(filename, "music_2")) {
      extern void ff7_bgm_set_akb(const char *);
      ff7_bgm_set_akb(path);
    }
  }
...
static void my_glTexImage2D(unsigned t, int lvl, int ifmt, int w, int h, int b,
  ...
  if (getenv("FF7_TEXDIMS")) { debugPrintf("TEXIMG %dx%d ifmt=0x%x fmt=0x%x ty=0x%x\n", w, h, ifmt, fmt, ty); fflush(stdout); }
...
static void my_glTexSubImage2D(unsigned t, int lvl, int xo, int yo, int w, int h,
  ...
  /* TEXDIMS: loga ANTES do upload -> se o Mali emperrar aqui, o ULTIMO log = a
   * textura culpada (dimensoes). flush p/ garantir que sai antes do travamento. */
  if (getenv("FF7_TEXDIMS")) { debugPrintf("TEXSUB %dx%d fmt=0x%x ty=0x%x\n", w, h, fmt, ty); fflush(stdout); }
