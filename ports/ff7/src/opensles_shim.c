...
#define MUSIC_SLOT (MAX_PLAYERS - 1)  /* slot reservado p/ ff7_music_feed (bypass BGM) */
#define FMVAUDIO_SLOT (MAX_PLAYERS - 2) /* slot reservado p/ audio do FMV (44100 stereo) */
...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  /* Use static buffers to save thread stack space */
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    /* MUSIC_SLOT (bypass de BGM): pre-buffer p/ ride-out do jitter do loop de video.
     * Espera acumular ~150ms antes de tocar; se drenar a 0, re-buffera. Evita o
     * chiado/crackle de underrun continuo (producao do RenderMix e' por-frame). */
    if (i == MUSIC_SLOT) {
      static int mus_ready = 0;
      uint32_t rd = ring_readable(p);
      if (!mus_ready) {
        if (rd >= 44100u * 2u * 2u * 25u / 100u) mus_ready = 1; /* ~250ms cushion (BGM PCM 44100) */
        else continue;
      } else if (rd == 0) {
        mus_ready = 0;
        continue;
    ...
    if (i == FMVAUDIO_SLOT) {
      static int fa_ready = 0;
      uint32_t rd = ring_readable(p);
      if (!fa_ready) {
        if (rd >= 44100u * 2u * 2u * 20u / 100u) fa_ready = 1; /* ~200ms cushion */
        else continue;
      } else if (rd == 0) {
        fa_ready = 0;
        continue;
    ...
    /* Determine if the game is feeding us 8-bit or 16-bit audio */
    uint32_t bps = p->bits_per_sample;
    if (bps != 8 && bps != 16)
      bps = 16; /* Fallback safety */
    uint32_t bytes_per_sample = bps / 8;
    uint32_t frame_size = src_channels * bytes_per_sample;
    ...
    if (vol < 0.0f || vol > 2.0f || vol != vol /* NaN */) {
      ...
      if (vol_warn < 20) {
        debugPrintf("opensles_shim: CORRUPT vol player %d: volume=%f (raw "
                    "bits=0x%08x)\n",
                    i, vol, *(uint32_t *)&vol);
        vol_warn++;
      }
    ...
    if (i != MUSIC_SLOT) { /* nao penaliza a BGM (estava baixa) */
      vol *= 0.8f;
    }
    player_vol[i] = vol;
    ...
    if (src_rate == SDL_OUTPUT_RATE) {
      ...
      step = 65536;  /* FIX: 1.0 em 16.16 — sem isso o mix mono mesma-taxa fica
                      * preso na amostra 0 (step=0) = ~silencio (SFX mudos). */
    } else {
      step = (uint32_t)((uint64_t)src_rate * 65536 / SDL_OUTPUT_RATE);
      uint32_t end_pos = p->resample_pos + out_frames * step;
      /* We need exactly the frames we'll touch +1 for linear interpolation
       * target */
    ...
    /* PEEK raw bytes instead of READ so we don't discard unused fractional
     * frames */
    ...
    if (src_rate == SDL_OUTPUT_RATE && src_channels == 2) {
      ...
      uint32_t fade_start_out =
          underrun
              ...
              : out_frames + 1;
      uint32_t fade_out_len_out =
          (uint32_t)((uint64_t)fade_out_len * SDL_OUTPUT_RATE / src_rate);
      if (fade_out_len_out == 0)
        fade_out_len_out = 1;
      uint32_t mixed = 0;
      for (uint32_t f = 0; f < out_frames; f++) {
        ...
        if (f >= fade_start_out && fade_out_len_out > 0) {
          float fo =
              1.0f - (float)(f - fade_start_out) / (float)fade_out_len_out;
          if (fo < env)
            env = fo;
        }
        if (env < 0.0f)
      ...
      /* Update our fractional position tracker and accurately flag consumed
       * frames */
      frames_consumed = pos >> 16;
      p->resample_pos = pos & 0xFFFF;
    }

    /* Now safely ADVANCE the ring buffer and queued byte tracker by precisely
     * what was used */
  ...
  if (getenv("FF7_AUDDIAG") && (callback_count % 40 == 1)) {
    ...
    debugPrintf("AUDDIAG cb#%u active=%d mix_peak=%.0f master_gain=%.2f\n",
                callback_count, num_active, mixpk, master_gain);
    for (int i = 0; i < MAX_PLAYERS; i++) {
      AudioPlayer *p = &g_players[i];
      if (!p->active && !p->ever_enqueued) continue;
      debugPrintf("  p%d act=%d state=%u vol=%.3f mixpeak=%.0f rate=%u ch=%u readable=%u under=%u\n",
                  i, p->active, p->play_state, p->volume, player_peak[i],
                  p->sample_rate, p->num_channels, ring_readable(p), p->underrun_count); }
  }
  ...
  if (jump_l > 8000 || jump_r > 8000) {
    click_count++;
    debugPrintf("opensles_shim: CLICK #%u cb#%u jump L=%d R=%d prev=%d/%d "
                "new=%d/%d mixpeak=%.0f active=%d\n",
                click_count, callback_count, jump_l, jump_r, (int)prev_left,
                (int)prev_right, (int)out[0], (int)out[1], mix_peak,
                num_active);
    for (int a = 0; a < num_active; a++) {
      int pi = player_active_list[a];
      AudioPlayer *pp = &g_players[pi];
      debugPrintf("  p%d: vol=%.3f peak=%.0f ch=%u rate=%u readable=%u\n", pi,
                  player_vol[pi], player_peak[pi], pp->num_channels,
                  pp->sample_rate, ring_readable(pp));
    }
  }
  if (out_samples >= 2) {
...
static AudioPlayer *alloc_player(void) {
  ...
  for (int i = 0; i < FMVAUDIO_SLOT; i++) {
    AudioPlayer *p = &g_players[i];
    if (p->play_state == SL_PLAYSTATE_STOPPED && ring_readable(p) == 0 &&
        p->queued_count == 0) {
      player_reset_meta(p);
      pthread_mutex_unlock(&g_players_lock);
  ...
  }
  {
    ...
    if (oldest >= 0) {
      ...
      debugPrintf("opensles_shim: WARNING: force-killing player %d (state=%u "
                  "played=%llu)\n",
...
static SLresult play_RegisterCallback(void *self, void *callback, void *ctx) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].play_ptr == itf_ptr) {
       ...
       *       debugPrintf("opensles_shim: player %d play callback registered=%p
    ctx=%p
       *     caller=libTTapp.so+0x%lx\n", i, callback, ctx, (unsigned long)(ra -
       *     (uintptr_t)text_base)); } else { debugPrintf("opensles_shim: player
    %d
       *     play callback registered=%p ctx=%p caller=%p\n", i, callback, ctx,
    (void
       *)ra);
    } */
      return SL_RESULT_SUCCESS;
...
static SLresult volume_SetVolumeLevel(void *self, SLmillibel level) {
  ...
  /* Clamp insane values */
  if (linear > 2.0f) {
    debugPrintf("opensles_shim: WARNING: SetVolumeLevel level=%d -> linear=%f, "
                "clamping to 1.0\n",
                (int)level, linear);
    linear = 1.0f;
  }
  ...
  debugPrintf("opensles_shim: WARNING: SetVolumeLevel - no matching player for "
              "itf=%p\n",
              self);
  return SL_RESULT_SUCCESS;
}
...
static SLresult bq_Enqueue(void *self, const void *pBuffer, SLuint32 size) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (written != size) {
        debugPrintf("opensles_shim: WARNING: truncated enqueue for player %d "
                    "(%u/%u bytes)\n",
                    i, written, size);
      }
      queue_push(p, written);
      p->enqueued_since_cb += written;
      /* DIAG FF7_MUSCALLER: descobre QUEM alimenta o player de musica (streaming
       * stereo, rate != 44100). Loga o return address resolvido contra text_base
       * 1x por player p/ achar a funcao de feed da BGM (que enfileira silencio). */
      if (getenv("FF7_MUSCALLER") && p->num_channels == 2 && !p->ever_enqueued) {
        uintptr_t ra = (uintptr_t)__builtin_return_address(0);
        uintptr_t tb = (uintptr_t)text_base;
        if (tb && ra >= tb && ra < tb + text_size)
          debugPrintf("MUSCALLER player %d (rate=%u ch=2) fed by libjni+0x%lx size=%u\n",
                      i, p->sample_rate, (unsigned long)(ra - tb), size);
        else
          debugPrintf("MUSCALLER player %d (rate=%u ch=2) fed by %p size=%u\n",
                      i, p->sample_rate, (void *)ra, size);
      }
      /* DIAG FF7_AUDDIAG: pico do dado NA ENTRADA (16-bit). Se >0 aqui mas mix=0 na
       * saida -> som chega e some na reamostragem (conserto). Se 0 aqui -> vem mudo. */
      if (getenv("FF7_AUDDIAG") && size >= 2) {
        const int16_t *s16 = (const int16_t *)pBuffer; int n = size / 2; int pk = 0;
        for (int j = 0; j < n; j++) { int a = s16[j] < 0 ? -s16[j] : s16[j]; if (a > pk) pk = a; }
        if (p->enqueue_counter % 200 == 0)
          debugPrintf("ENQDIAG player %d enq_peak16=%d size=%u rate=%u ch=%u bps=%u\n",
                      i, pk, size, p->sample_rate, p->num_channels, p->bits_per_sample);
      }
      if (written > 0) {
        p->last_enqueue_size = written;
        p->enqueue_counter++;
        if (!p->ever_enqueued) debugPrintf("AUDIOCHK player %d 1o ENQUEUE size=%u (audio FLUINDO)\n", i, size);
        p->ever_enqueued = 1;
        /* if (p->debug_enqueue_logs < 16 || p->enqueue_counter % 64 == 0) {
         *         debugPrintf("opensles_shim: player %d enqueue size=%u
      written=%u
         *       readable=%u counter=%u\n", i, size, written, ring_readable(p),
         *       p->enqueue_counter); p->debug_enqueue_logs++;
      } */
      }
      return SL_RESULT_SUCCESS;
    }
...
static SLresult bq_RegisterCallback(void *self, slBufferQueueCallback callback,
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      AudioPlayer *p = &g_players[i];
      if (getenv("FF7_CBCALLER")) {
        uintptr_t ra = (uintptr_t)__builtin_return_address(0);
        uintptr_t tb = (uintptr_t)text_base;
        debugPrintf("CBCALLER player %d bq_cb=%p (libjni+0x%lx) ctx=%p reg_by=libjni+0x%lx\n",
                    i, callback,
                    (tb && (uintptr_t)callback >= tb) ? (unsigned long)((uintptr_t)callback - tb) : 0UL,
                    pContext,
                    (tb && ra >= tb) ? (unsigned long)(ra - tb) : 0UL);
      }
      /* uintptr_t ra = (uintptr_t)__builtin_return_address(0); */
      p->callback = callback;
      ...
      /* if (text_base && ra >= (uintptr_t)text_base &&
       *         ra < (uintptr_t)text_base + text_size) {
       *       debugPrintf("opensles_shim: player %d buffer callback
    registered=%p
       *     ctx=%p caller=libTTapp.so+0x%lx\n", i, callback, pContext,
    (unsigned
       *     long)(ra - (uintptr_t)text_base)); } else {
    debugPrintf("opensles_shim:
       *     player %d buffer callback registered=%p ctx=%p caller=%p\n", i,
    callback,
       *     pContext, (void *)ra);
    } */
      return SL_RESULT_SUCCESS;
...
static SLresult engine_CreateAudioPlayer(void *self, void **pPlayer,
  ...
  { static int once = 0; if (!once++) debugPrintf("AUDIOCHK engine_CreateAudioPlayer CHAMADO (FF7 usa OpenSLES)\n"); }
  ...
  if (pAudioSrc) {
    ...
    if (src->pLocator) {
      SLDataLocator_BufferQueue *loc =
          (SLDataLocator_BufferQueue *)src->pLocator;
...
void opensles_shim_pump_callbacks(void) {
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (callback_threshold == 0 ||
        callback_threshold > (RING_BUFFER_SIZE / 2)) {
      callback_threshold = RING_BUFFER_SIZE / 4;
    }

    uint32_t refill_threshold = callback_threshold * 2;
    /* Piso de ~0.5s: faz o player 0 (driver da BGM) BUFFERIZAR A FRENTE, entao o
     * RenderMix roda adiantado e enche o colchao do MUSIC_SLOT -> BGM suave, sem
     * underrun/chiado. Sem isso o piso era ~4KB (so' realtime, na beira do underrun).
     * So' afeta players de streaming (com callback); SFX one-shot nao tem callback. */
    if (refill_threshold < 96 * 1024)
      refill_threshold = 96 * 1024;
    if (refill_threshold > RING_BUFFER_SIZE / 2)
      refill_threshold = RING_BUFFER_SIZE / 2;

    /* max_calls maior = a producao de BGM (via player 0 -> RenderMix -> music_feed)
     * consegue se recuperar de stalls do loop de video, mantendo o colchao do
     * MUSIC_SLOT cheio = sem chiado/underrun. */
    int max_calls = 16;
...
/* BYPASS BGM: o SQEX CoreSource::RenderMix produz a BGM perfeita em 44100 stereo
 * s16, mas o resample/enqueue interno p/ a saida (player 0 @32000) le um buffer
 * zerado = mudo. main.c hooka RenderMix e nos manda o PCM aqui; tocamos num player
 * dedicado (slot MUSIC_SLOT, fora do alloc do jogo) na taxa nativa = SEM resample. */
void ff7_music_feed(const void *pcm, uint32_t bytes) {
  ...
  ring_write(p, pcm, bytes);
  if (getenv("FF7_AUDDIAG")) {
    static unsigned fc = 0; static uint64_t tot = 0;
    fc++; tot += bytes;
    if (fc % 200 == 1)
      debugPrintf("MUSICFEED calls=%u total_bytes=%llu last=%u readable=%u\n",
                  fc, (unsigned long long)tot, bytes, ring_readable(p));
  }
}

/* Audio do FMV: o .webm tem Vorbis, mas extraimos p/ PCM s16 44100 stereo
 * (movies_ivf/<nome>.pcm). fmv.c alimenta aqui em sync com o decode de video.
 * Player dedicado (FMVAUDIO_SLOT) na taxa de saida = mix direto. */
void ff7_fmv_audio_feed(const void *pcm, uint32_t bytes) {
  if (!pcm || bytes == 0)
    return;
  ensure_audio_initialized();
  AudioPlayer *p = &g_players[FMVAUDIO_SLOT];
  if (!p->active || p->sample_rate != SDL_OUTPUT_RATE ||
      p->play_state != SL_PLAYSTATE_PLAYING) {
    if (g_audio_dev) SDL_LockAudioDevice(g_audio_dev);
    player_reset_meta(p);
    p->num_channels = 2;
    p->sample_rate = SDL_OUTPUT_RATE; /* 44100 = saida -> sem resample */
    p->bits_per_sample = 16;
    p->volume = 1.4f;
    p->play_state = SL_PLAYSTATE_PLAYING;
    p->active = 1;
  ...
  uint32_t max_buf = 44100u * 2u * 2u; /* ~1s cap */
  if (ring_readable(p) > max_buf && ring_readable(p) >= bytes) {
    __sync_synchronize();
    p->ring_tail += bytes;
  }
...
/* BGM via PCM (bypass do streaming SQEX flaky): a BGM (music_2 .akb = Ogg) e'
 * pre-decodificada p/ PCM s16 44100 stereo; main.c faz o stream em loop p/ ca'.
 * Reusa o MUSIC_SLOT (agora 44100), com prebuffer. */
/* configura o MUSIC_SLOT p/ BGM PCM (chamado 1x no start, fora do hot path). */
void ff7_bgm_pcm_start(void) {
  ensure_audio_initialized();
  AudioPlayer *p = &g_players[MUSIC_SLOT];
  /* lock-free: o slot e' dedicado/nao usado pelo jogo; um read torn pela callback
   * por 1 frame e' inofensivo. Evita SDL_LockAudioDevice (SDL3 no device) que
   * perturba o threading fragil do Mali (deadlock no _mali_frame_builder). */
  p->play_state = SL_PLAYSTATE_STOPPED; __sync_synchronize();
  p->ring_head = 0; p->ring_tail = 0;
  p->num_channels = 2; p->sample_rate = SDL_OUTPUT_RATE; p->bits_per_sample = 16;
  p->volume = 1.6f; p->resample_pos = 0; p->frames_played = 0; p->underrun_count = 0;
  p->active = 1; p->callback = NULL; p->ever_enqueued = 0;
  __sync_synchronize(); p->play_state = SL_PLAYSTATE_PLAYING;
}
void ff7_bgm_pcm_feed(const void *pcm, uint32_t bytes) {
  if (!pcm || bytes == 0) return;
  AudioPlayer *p = &g_players[MUSIC_SLOT];
  if (p->play_state != SL_PLAYSTATE_PLAYING) return; /* nao iniciado */
  ring_write(p, pcm, bytes); /* SPSC: sem lock no hot path */
}
uint32_t ff7_bgm_readable(void) { return ring_readable(&g_players[MUSIC_SLOT]); }
void ff7_bgm_pcm_stop(void) {
  AudioPlayer *p = &g_players[MUSIC_SLOT];
  if (g_audio_dev) SDL_LockAudioDevice(g_audio_dev);
  p->ring_head = 0; p->ring_tail = 0; p->play_state = SL_PLAYSTATE_STOPPED;
  if (g_audio_dev) SDL_UnlockAudioDevice(g_audio_dev);
}

/* Para/limpa o audio do FMV (fim do filme). */
