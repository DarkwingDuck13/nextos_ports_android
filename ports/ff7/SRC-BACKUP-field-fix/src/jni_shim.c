...
static jint jni_CallStaticIntMethod(void *env, void *clazz, void *mid, ...) {
  ...
  if (mid == &g_method_tags[MID_DEC_GETPOS]) {
    /* NO EOF retorna 999999 (fim HARD, igual ao pulo antigo que FUNCIONAVA) p/ o
     * engine encerrar o filme DE VEZ e o script do campo spawnar o Cloud. Retornar
     * so' 'total' nao bastava -> o engine achava o filme rolando -> Cloud nao
     * aparecia + congelava (relato do NextOS). */
    int v = g_fmv_skip ? 999999 : (fmv_eof() ? 999999 : fmv_cur_frame());
    if (g_fmv_log) debugPrintf("FMV GET_POSITION -> %d\n", v);
    return v;   /* >= total -> movie terminou */
  }
  if (mid == &g_method_tags[MID_DEC_FRAME]) {
    g_fmv_calls++;
    int v = g_fmv_skip ? 999999 : (fmv_eof() ? -1 : fmv_cur_frame());
    if (g_fmv_log && g_fmv_calls < 8) debugPrintf("FMV FRAME#%ld -> %d\n", g_fmv_calls, v);
    return v;
  }
