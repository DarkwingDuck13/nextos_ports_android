...
typedef struct {
  ...
  int eof;
  int cur_frame;
  int fps;                /* frames/s do video (timebase IVF) */
...
void fmv_reset(void) {
  ...
  g_open_ivf[0] = 0;
  extern void fmv_clear_force_eof(void); fmv_clear_force_eof();
}
...
static int fmv_open_path(const char *path) {
  ...
  /* timebase IVF: rate(16..19)/scale(20..23) = fps do video (opening=15) */
  { uint32_t rate = u32le(hdr + 16), scale = u32le(hdr + 20);
    g.fps = (scale > 0 && rate > 0) ? (int)(rate / scale) : 15;
    if (g.fps < 1 || g.fps > 120) g.fps = 15; }
...
int fmv_fps(void) { return g.fps > 0 ? g.fps : 15; }
static int g_force_eof = 0;
void fmv_force_eof(void) { g_force_eof = 1; }
void fmv_clear_force_eof(void) { g_force_eof = 0; }
int fmv_eof(void) {
  if (g_force_eof) return 1;
  const char *m = getenv("FF7_FMVMAXF"); /* DIAG: encerra o FMV cedo p/ testar transicao */
  if (m && g.cur_frame >= atoi(m)) return 1;
  return g.eof || (g.num_frames && g.cur_frame >= (int)g.num_frames);
}
