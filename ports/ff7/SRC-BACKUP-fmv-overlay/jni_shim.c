...
/* FF7 nao usa texto cocos (engine proprio) -> stub (caminho morto, so linka). */
static unsigned char *chrono_render_text(const char *utf8, int px, int r, int g, int b,
                                         int align, int reqW, int reqH, int *outW, int *outH) {
  (void)utf8;(void)px;(void)r;(void)g;(void)b;(void)align;(void)reqW;(void)reqH;
  if (outW) *outW = 0; if (outH) *outH = 0; return NULL;
}
...
enum {
  ...
  /* MyDecoder (FMV/video VP8): START/FRAME/GET_POSITION/GET_TOTALTIME ()I,
   * GET_YUV_PATCH ()Z, AFTER_FRAME/RESET ()V, SET_TEXTURE/SET_VOLUME/PLAY. */
  MID_DEC_START, MID_DEC_FRAME, MID_DEC_GETPOS, MID_DEC_GETTOTAL,
  MID_DEC_YUVPATCH, MID_DEC_AFTERFRAME, MID_DEC_RESET, MID_DEC_SETTEX,
  MID_DEC_SETVOL, MID_DEC_PLAY, MID_DEC_SETPOS,
  MID_TAG_COUNT
};
...
static int tag_for_method(const char *name) {
  ...
  /* MyDecoder (nomes UPPERCASE exatos do .so) */
  if (strcmp(name, "START") == 0)         return MID_DEC_START;
  if (strcmp(name, "FRAME") == 0)         return MID_DEC_FRAME;
  if (strcmp(name, "GET_POSITION") == 0)  return MID_DEC_GETPOS;
  if (strcmp(name, "GET_TOTALTIME") == 0) return MID_DEC_GETTOTAL;
  if (strcmp(name, "GET_YUV_PATCH") == 0) return MID_DEC_YUVPATCH;
  if (strcmp(name, "AFTER_FRAME") == 0)   return MID_DEC_AFTERFRAME;
  if (strcmp(name, "RESET") == 0)         return MID_DEC_RESET;
  if (strcmp(name, "SET_TEXTURE") == 0)   return MID_DEC_SETTEX;
  if (strcmp(name, "SET_VOLUME") == 0 || strcmp(name, "SETVOLUME") == 0) return MID_DEC_SETVOL;
  if (strcmp(name, "PLAY") == 0)          return MID_DEC_PLAY;
  if (strcmp(name, "SET_POSITION") == 0)  return MID_DEC_SETPOS;
  return MID_GENERIC;
}
...
/* ==== FMV / MyDecoder (video VP8) ====
 * O decoder de video do FF7 mobile e' uma classe Java (MyDecoder) que usa
 * MediaCodec + textura OES — inexistente no so-loader. Sem ele o engine fica
 * PRESO tocando o FMV de abertura (campo nunca carrega). Aqui instrumentamos e,
 * em modo skip (FF7_FMVSKIP, default ON), reportamos o movie como TERMINADO
 * (position >= total) p/ o jogo seguir pro campo. FF7_FMVLOG loga a sequencia. */
static int g_fmv_log = 0, g_fmv_skip = 1, g_fmv_started = 0;
static long g_fmv_calls = 0;
int g_ff7_movie_active = 0;        /* lido pelo overlay de video em main.c */
static int g_fmv_overlay = 0;
static int fmv_env_init(void) {
  static int done = 0;
  if (!done) { g_fmv_log = getenv("FF7_FMVLOG") != NULL;
               g_fmv_overlay = 0;
               g_fmv_skip = getenv("FF7_FMVSKIP_ON") != NULL;  /* ORIGINAL = off */ done = 1; }
  return 0;
}
static jint jni_CallStaticIntMethod(void *env, void *clazz, void *mid, ...) {
  ...
  fmv_env_init();
  /* MyDecoder agora reflete o NOSSO decoder VP8 (fmv.c): o engine ve um filme
   * TOCANDO (total/posicao em frames) -> desenha o quad do filme cada frame ->
   * VIDEO_enableOES(1) dispara -> main.c injeta o frame decodificado. Quando nosso
   * video acaba (eof), posicao>=total -> o engine encerra e segue o jogo.
   * FF7_FMVSKIP_ON forca o comportamento antigo (pular o FMV). */
  extern int fmv_total_frames(void);
  extern int fmv_cur_frame(void);
  extern int fmv_eof(void);
  extern int fmv_has_movie(void);
  if (mid == &g_method_tags[MID_DEC_START]) {
    g_fmv_started = 1; g_fmv_calls = 0; g_ff7_movie_active = 1;
    if (getenv("FF7_FMVSTART")) return atoi(getenv("FF7_FMVSTART"));
    int t = fmv_has_movie() ? fmv_total_frames() : 0;
    int v = t > 0 ? t : 1;     /* total de frames (nonzero = filme OK) */
    if (g_fmv_log) debugPrintf("FMV START -> %d\n", v);
    return v;
  }
  if (mid == &g_method_tags[MID_DEC_GETTOTAL]) {
    int t = fmv_has_movie() ? fmv_total_frames() : 0;
    int v = g_fmv_skip ? 1 : (t > 0 ? t : 1);
    if (g_fmv_log) debugPrintf("FMV GET_TOTALTIME -> %d\n", v);
    return v;
  }
  /* modo overlay: o engine BLOQUEIA em fw_update_movie_sample (usleep-loop na
   * main thread) esperando o proximo frame — e quem avancaria o decode (present
   * cb) nunca roda = deadlock. O wait-loop consulta FRAME/GET_POSITION daqui;
   * avancamos o decode AQUI (so' CPU, paced por relogio) p/ destravar o loop. */
  extern void ff7_fmv_clock_tick(void);
  if (mid == &g_method_tags[MID_DEC_GETPOS] || mid == &g_method_tags[MID_DEC_FRAME])
    ff7_fmv_clock_tick();
  if (mid == &g_method_tags[MID_DEC_GETPOS]) {
    int t = fmv_has_movie() ? fmv_total_frames() : 0;
    int v = g_fmv_skip ? 999999 : (fmv_eof() ? (t > 0 ? t : 999999) : fmv_cur_frame());
    if (g_fmv_log) debugPrintf("FMV GET_POSITION -> %d\n", v);
    return v;
  }
  if (mid == &g_method_tags[MID_DEC_FRAME]) {
    g_fmv_calls++;
    /* FIM DO FILME = FRAME retornar 0: fw_update_movie_sample faz `bl AVI_frame;
     * cbnz w0, paced-path` — SO' o retorno 0 cai no caminho de encerramento
     * (AVI_draw final + done=1). -1 e' nonzero -> o engine "tocava" o filme p/
     * SEMPRE (counter passava do total) = a tela preta pos-trem. E' exatamente
     * por isso que NOFMV transicionava limpo (sem filme, cur_frame=0 -> FRAME=0). */
    int v = g_fmv_skip ? 0 : (fmv_eof() ? 0 : fmv_cur_frame());
    if (g_fmv_log && (g_fmv_calls < 8 || v == 0))
      debugPrintf("FMV FRAME#%ld -> %d%s\n", g_fmv_calls, v, v == 0 ? " (FIM)" : "");
    return v;
  }
  if (mid == &g_method_tags[MID_DEC_YUVPATCH]) {
    if (g_fmv_log) debugPrintf("FMV GET_YUV_PATCH -> 0\n");
    return 0;   /* sem patch YUV nativo: usamos o caminho OES (override em main.c) */
  }
  return 0;
}
...
static const char *dec_tag_name(void *mid) {
  if (mid == &g_method_tags[MID_DEC_START]) return "START";
  if (mid == &g_method_tags[MID_DEC_FRAME]) return "FRAME";
  if (mid == &g_method_tags[MID_DEC_GETPOS]) return "GET_POSITION";
  if (mid == &g_method_tags[MID_DEC_GETTOTAL]) return "GET_TOTALTIME";
  if (mid == &g_method_tags[MID_DEC_YUVPATCH]) return "GET_YUV_PATCH";
  if (mid == &g_method_tags[MID_DEC_AFTERFRAME]) return "AFTER_FRAME";
  if (mid == &g_method_tags[MID_DEC_RESET]) return "RESET";
  if (mid == &g_method_tags[MID_DEC_SETTEX]) return "SET_TEXTURE";
  if (mid == &g_method_tags[MID_DEC_SETVOL]) return "SET_VOLUME";
  if (mid == &g_method_tags[MID_DEC_PLAY]) return "PLAY";
  if (mid == &g_method_tags[MID_DEC_SETPOS]) return "SET_POSITION";
  return NULL;
}
static void jni_CallStaticVoidMethod(void *env, void *clazz, void *mid, ...) {
  ...
  fmv_env_init();
  if (mid == &g_method_tags[MID_DEC_RESET]) g_ff7_movie_active = 0;  /* movie acabou */
  if (g_fmv_log) { const char *n = dec_tag_name(mid);
    static long vc = 0; if (n && vc++ < 30) debugPrintf("FMV(void) %s\n", n); }
}
static void jni_CallStaticVoidMethodV(void *env, void *clazz, void *mid, va_list a) {
  if (g_fmv_log) { const char *n = dec_tag_name(mid);
    static long vc = 0; if (n && vc++ < 30) debugPrintf("FMV(voidV) %s\n", n); }
}
