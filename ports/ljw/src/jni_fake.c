...
volatile int g_alert_is_quit = 0;   // 1 = the pending dialog is a "Quit Game?" prompt
...
static juint call_boolean(const char *name, va_list va) {
  ...
  if (!strcmp(name, "ShowAlertDialog")) {
    ...
    const char *ts = obj_str(t), *ms = obj_str(m);
    debugPrintf("JNI: ShowAlertDialog title=\"%s\" msg=\"%s\" pos=\"%s\" neg=\"%s\"\n",
                ts, ms, obj_str(pos), obj_str(neg));
    g_alert_is_quit = (strstr(ts, "Quit") || strstr(ms, "Quit")) ? 1 : 0;
    g_alert_pending = 1;
