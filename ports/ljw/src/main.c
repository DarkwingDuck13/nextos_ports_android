...
int main(int argc, char *argv[]) {
  ...
  debugPrintf("=== LEGO Jurassic World -> Mali-450 (Linux/SDL) ===\n");
  ...
  if (stat(GAMEDATA_DIR "/project_amber_mobile.fib", &st) < 0)
    debugPrintf("WARN: %s/project_amber_mobile.fib not found (assets may be missing)\n", GAMEDATA_DIR);
  ...
  while (running) {
    ...
    if (g_alert_pending && g.setAlertResponse) {
      // "Quit Game?" -> No (keep playing). Everything else (the Google Play
      // "create a new save?" gate that blocks New Game) -> Yes/create-new so
      // gameplay proceeds. Conditioning on a local save file was unreliable
      // (the engine writes savegame.dat during boot), so we always create.
      int resp = g_alert_is_quit ? ALERT_RESP_KEEP : ALERT_RESP_NEW;
      g.setAlertResponse(fake_env, FUSION_OBJ, resp);
      debugPrintf("alert: %s -> fed response %d (%s)\n",
                  g_alert_is_quit ? "quit" : "google-play/newsave", resp,
                  g_alert_is_quit ? "No/keep-playing" : "Yes/create-new");
      g_alert_pending = 0;
    }
