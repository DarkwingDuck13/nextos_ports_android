#!/bin/bash
# Build RELEASE do LEGO Jurassic World (arm64) no Docker debian:buster — glibc ≤2.30
...
echo "BUILD OK -> ljw.release"
echo "  arch:      $(file ljw.release 2>/dev/null | head -c 80)"
echo "  GLIBC max: $($READELF -V ljw.release | grep -oE 'GLIBC_[0-9.]+' | sed 's/GLIBC_//' | sort -uV | tail -1)"
echo "  tamanho:   $(stat -c%s ljw.release) bytes"
"$READELF" -d ljw.release | grep NEEDED