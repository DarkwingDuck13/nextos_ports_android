#!/bin/bash
# PORTMASTER: megaman4, Mega Man 4.sh
# Mega Man 4 Mobile (Cocos2d-x 3.9, Capcom) -> Mali-450 armhf so-loader.
...
GAMEDIR="/$directory/ports/megaman4"
cd "$GAMEDIR" || exit 1

> "$GAMEDIR/log.txt" && exec > >(tee "$GAMEDIR/log.txt") 2>&1

$ESUDO chmod +x "$GAMEDIR/megaman4" 2>/dev/null
...
$GPTOKEYB "megaman4" &
pm_platform_helper "$GAMEDIR/megaman4" >/dev/null
