/* main.c -- LEGO Ninjago Tournament of Elements (Android armeabi-v7a) on Linux/Mali
 *
...
static int ninjago_in_level(void) { return g_goplayer_active && *g_goplayer_active != 0; }
static int ninjago_paused(void)   { return g_paused_flag && *g_paused_flag != 0; }
...
static void controls_route_fe_to_pad(void) {
  if (!ctl_current || !ctl_joypad) return;
  void *joy = *ctl_joypad;
  if (!joy) return;
...
int main(int argc, char *argv[]) {
  ...
  debugPrintf("=== LEGO Ninjago Tournament -> Mali-450 (Linux/SDL) ===\n");
  ...
  while (running) {
    ...
    // Touch injection: the title/front-end is touch-driven, while gameplay uses
    // the native joypad poll above. Same pattern as Marvel, with this title's
    // Play arrow position at the lower-right of 1280x720.
    {
      static int touch_hold = 0;
      static float tx = 0.0f, ty = 0.0f;
      static int press_prev = 0;
      static float cx = -1.0f, cy = -1.0f;
      static int cursor_down = 0, cursor_used = 0;
      int want_tap = 0;
      float wx = 0.0f, wy = 0.0f;
      int in_level = ninjago_in_level();
      int frontend_or_pause = !in_level || ninjago_paused();

      if (cx < 0.0f) {
        cx = screen_width  * (1190.0f / 1280.0f);
      ...
      if (touch_hold > 0) {
        ...
        FILE *tf = fopen("/dev/shm/ninjagot_touch", "r");
        if (!tf) tf = fopen("/dev/shm/ninjagot_tap", "r");
        if (tf) {
          float fx, fy;
          if (fscanf(tf, "%f %f", &fx, &fy) == 2) { want_tap = 1; wx = fx; wy = fy; }
          fclose(tf);
          remove("/dev/shm/ninjagot_touch");
          remove("/dev/shm/ninjagot_tap");
        }
        if (want_tap) {
          tx = wx;
          ty = wy;
    ...
    if (frame == 120) {
      debugPrintf("pad bindings: LX=%d LY=%d RX=%d RY=%d START=%d SELECT=%d "
                  ...
                  ctl_val(ctl_south), ctl_val(ctl_east), ctl_val(ctl_west), ctl_val(ctl_north),
                  ctl_val(ctl_confirm), ctl_val(ctl_cancel),
                  ninjago_in_level(), ninjago_paused());
    }
