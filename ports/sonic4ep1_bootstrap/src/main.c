/*
 * main.c — loader ARMHF (gerado por new-port-arm.sh) p/ sonic4ep1_bootstrap.
 *
 * Multi-módulo bionic→glibc: carrega as dependências (libs3eAPKExpansion.so libs3eDialog.so libs3eFlurry.so) cada uma
 * no seu heap + so_snapshot_symbols, acumula numa tabela combinada, e carrega o
 * módulo principal (libs3e_android.so) resolvendo contra tudo + fallback dlsym. Acha
...
int main(int argc, char *argv[]) {
  ...
  debugPrintf("=== sonic4ep1_bootstrap — loader ARMHF (Mali-450) ===\n");
  ...
  debugPrintf("=== F1 OK: multi-módulo carregado+resolvido. (F2: boot do engine) ===\n");
  return 0;
}
