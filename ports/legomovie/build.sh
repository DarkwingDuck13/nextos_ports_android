#!/bin/bash
# build ARMHF do LEGO Movie Video Game (so-loader, engine Fusion, GLES2).
# libLEGO_Emmet.so = armeabi-v7a (softfp) -> loader armhf (hardfp) + softfp thunks.
...
echo "BUILD OK -> $(file legomovie | cut -d, -f1-3)"
