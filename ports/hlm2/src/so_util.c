...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  /* libyoyo (GameMaker) tem MAIS DE UM segmento PT_LOAD de dados (text + 2x RW:
   * .data.rel.ro e .data/.bss). O loop acima so guardava o ULTIMO em data_segno;
   * os do meio (ex: .data.rel.ro com as module-classes do FreeType) ficavam
   * ZERADOS -> module_size=0 -> crash. Aqui copiamos TODO PT_LOAD restante. */
  for (int i = 0; i < elf_hdr->e_phnum; i++) {
    if (prog_hdr[i].p_type != PT_LOAD) continue;
    if (i == text_segno || i == data_segno) continue;
    void *seg = (void *)(prog_hdr[i].p_vaddr + (Elf64_Addr)load_base);
    memcpy(seg, (void *)((uintptr_t)so_base + prog_hdr[i].p_offset),
           prog_hdr[i].p_filesz);
    prog_hdr[i].p_vaddr = (Elf64_Addr)(prog_hdr[i].p_vaddr + (Elf64_Addr)load_virtbase);
    debugPrintf("so_load: extra PT_LOAD seg %d -> %p (filesz 0x%zx)\n", i, seg,
                (size_t)prog_hdr[i].p_filesz);
  }
      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_JUMP_SLOT: {
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (!found) {
              void *da = NULL;
              if (g_libcpp) da = dlsym(g_libcpp, name);
              if (!da) da = dlsym(RTLD_DEFAULT, name);
              if (da) { *ptr = (uintptr_t)da; found = 1; }
            }
            if (!found)
...
/* so_snapshot_symbols — copiado do reVC (ports/revc, port FUNCIONANDO).
 * Snapshot dos símbolos exportados do módulo atual (p/ resolver __ndk1 do jogo). */
DynLibFunction *so_snapshot_symbols(int *out_count) {
  int n = 0;
  for (int i = 0; i < num_syms; i++) {
    if (syms[i].st_shndx == SHN_UNDEF || syms[i].st_value == 0) continue;
    int bind = ELF64_ST_BIND(syms[i].st_info);
    if ((bind != STB_GLOBAL && bind != STB_WEAK) || syms[i].st_name == 0) continue;
    n++;
  }
