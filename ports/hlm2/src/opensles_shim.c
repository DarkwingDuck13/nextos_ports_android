...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  { static int dbgc=0; if(getenv("KZ_SNDLOG") && dbgc++%80==0){ int act=0; for(int k=0;k<MAX_PLAYERS;k++) if(g_players[k].active && g_players[k].play_state==SL_PLAYSTATE_PLAYING) act++; fprintf(stderr,"[sl] sdl_cb len=%d active_players=%d\n", len, act); } }
  ...
  { static int pkc=0; if(getenv("KZ_SNDLOG") && pkc++%80==0){ AudioPlayer *p0=&g_players[0];
      fprintf(stderr,"[sl] mixpeak=%.0f active=%d | p0 state=%d vol=%.2f readable=%u ch=%u rate=%u\n",
        mix_peak, num_active, p0->play_state, p0->volume, ring_readable(p0), p0->num_channels, p0->sample_rate); } }
...
static SLresult play_SetPlayState(void *self, SLuint32 state) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].play_ptr == itf_ptr) {
      AudioPlayer *p = &g_players[i];
      if(getenv("KZ_SNDLOG")) fprintf(stderr, "[sl] SetPlayState p%d %u->%u\n", i, p->play_state, state);
      if (g_audio_dev) SDL_LockAudioDevice(g_audio_dev);
...
void opensles_shim_pump_callbacks(void) {
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    while (p->callback && readable <= refill_threshold && max_calls > 0) {
      ...
      { static int fc=0; if(getenv("KZ_SNDLOG") && fc++%150==0) fprintf(stderr,"[sl] bq_cb#%d enq %u->%u\n", fc, counter_before, p->enqueue_counter); }
