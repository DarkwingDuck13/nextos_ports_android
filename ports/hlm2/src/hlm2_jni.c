...
static const char *g_datapath = "/storage/roms/ports/hlm2/";
...
void jni_run(void) {
  ...
  g_win = SDL_CreateWindow("Hotline Miami 2", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  *(uintptr_t *)(fake_vm + 0x38) = (uintptr_t)j_AttachCurrentThread; /* idx 7 */
  ...
  snprintf(apk_path, sizeof(apk_path), "%s", apk_env ? apk_env : "/storage/roms/ports/hlm2/hlm2.apk");
  ...
  if (getenv("HM_CLOUD")) cloud_resolve();
  ...
  for (long f = 0;; f++) {
    ...
    if (g_showui_called && getenv("HM_NFX")) {
