...
int pthread_cond_wait_fake(void **cslot, void **mslot) {
  ...
  unsigned long ra0 = (unsigned long)__builtin_return_address(0);
  unsigned long off0 = 0;
  const char *lib0 = ra_name(ra0, &off0);
  if (g_pt_ub && strcmp(lib0, "libunity") == 0 && off0 == 0x271404 &&
      getenv("GK_SKIP_CHOREO_WAIT")) {
    static uintptr_t ready = 1;
    void **pred = (void **)((uintptr_t)cslot - 0x30);
    *pred = &ready;
    static int n;
    if (n++ < 8) {
      fprintf(stderr, "[CHOREO_WAIT] libunity+0x271404 cslot=%p pred=%p -> ready\n",
              (void *)cslot, (void *)pred);
      fsync(2);
    }
    return 0;
  }
