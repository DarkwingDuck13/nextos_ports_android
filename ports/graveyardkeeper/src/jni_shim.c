...
static const char *g_package_name = "com.tinybuild.graveyardkeeper";
...
static const char *resolve_jstring(void *jstr) {
  ...
  for (int i = 0; i < g_jstring_count; i++)
    if (g_jstrings[i].handle == jstr)
      return g_jstrings[i].value ? g_jstrings[i].value : "";
  return "";
}
...
#define ASSET_PACK_PATH GK_PORT_BASE "assets"
#define ASSET_BASE ASSET_PACK_PATH "/"
#define GK_LIB_BASE GK_PORT_BASE "lib/"
...
static void *jni_CallObjectMethodV(void *env, void *obj, void *methodID,
  ...
  if (nm) {
    ...
    if (strcmp(nm, "getAssetPackPath") == 0)
      return make_jstring(getenv("GK_PACKPATH") ? getenv("GK_PACKPATH")
                          : ASSET_PACK_PATH);
    ...
    if (strcmp(nm, "findLibrary") == 0) {
      ...
      if (ln && strstr(ln, "il2cpp"))
        return make_jstring(GK_LIB_BASE "libil2cpp.so");
      if (ln && strstr(ln, "main"))
        return make_jstring(GK_LIB_BASE "libmain.so");
      if (ln && strstr(ln, "unity"))
        return make_jstring(GK_LIB_BASE "libunity.so");
      return make_jstring("");
    }
    if (strcmp(nm, "getPackageCodePath") == 0 ||
        strcmp(nm, "getPackageResourcePath") == 0)
      return make_jstring(GK_PORT_BASE "base.apk");
    ...
    if (strcmp(nm, "getFilesDir") == 0 || strcmp(nm, "getExternalFilesDir") == 0 ||
        ...
        strcmp(nm, "getObbDir") == 0 || strcmp(nm, "getObbDirs") == 0 ||
      ...
      return make_jstring(GK_PORT_BASE "userdata");
    /* Bundle.getString(key) e SharedPreferences.getString(key, default) compartilham
       o mesmo nome. Distinguir pela assinatura evita consumir um va_arg inexistente
       e transformar lixo de registrador em jstring. */
    if (strcmp(nm, "getString") == 0) {
      ...
      if (sig && strcmp(sig, "(Ljava/lang/String;)Ljava/lang/String;") == 0) {
        if (key && strcmp(key, "unity.build-id") == 0)
          return make_jstring("807d62e2-be5f-459a-b71a-e821fe1205b4");
        return make_jstring("");
      }
...
static void *jni_CallObjectMethodA(void *env, void *obj, void *methodID, const jvalue *args) {
  (void)env;
  const char *nm = mid_name(methodID);
  debugPrintf("jni_shim: CallObjectMethodA(%s)\n", nm ? nm : "?");
  if (nm && strcmp(nm, "getLooper") == 0) {
    debugPrintf("jni_shim: getLooper(A) -> Looper fake\n");
    return &g_looper_obj;
  }
...
static void jni_CallVoidMethodV(void *env, void *obj, void *methodID, va_list ap) {
  ...
  if (nm && strcmp(nm, "sendToTarget") == 0) {
    if (obj != (void *)&g_message_sentinel)
      debugPrintf("jni_shim: sendToTarget(obj=%p) sem Message sentinel; dirigindo handleMessage mesmo assim\n", obj);
    if (!getenv("TER_NOHANDLEMSG")) jni_handlemessage(env);
    return;
  }
  if (nm && strcmp(nm, "postFrameCallback") == 0) {
    void *cb = va_arg(ap, void *);
    if (cb && proxy_handle(cb)) g_framecb_proxy = cb;
    struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts);
    long nanos = (long)ts.tv_sec * 1000000000L + ts.tv_nsec;
    debugPrintf("jni_shim: postFrameCallback(cb=%p) -> doFrame imediato\n", cb);
    jni_choreo_doframe(env, nanos);
    return;
  }
  if (nm && strcmp(nm, "removeFrameCallback") == 0) {
    (void)va_arg(ap, void *);
    debugPrintf("jni_shim: removeFrameCallback\n");
    return;
  }
...
static void jni_CallVoidMethodA(void *env, void *obj, void *methodID, const jvalue *args) {
  ...
  if (nm && strcmp(nm, "sendToTarget") == 0) {
    if (obj != (void *)&g_message_sentinel)
      debugPrintf("jni_shim: sendToTargetA(obj=%p) sem Message sentinel; dirigindo handleMessage mesmo assim\n", obj);
    if (!getenv("TER_NOHANDLEMSG")) jni_handlemessage(env);
    return;
  }
  if (nm && strcmp(nm, "postFrameCallback") == 0) {
    void *cb = args ? args[0].l : NULL;
    if (cb && proxy_handle(cb)) g_framecb_proxy = cb;
    struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts);
    long nanos = (long)ts.tv_sec * 1000000000L + ts.tv_nsec;
    debugPrintf("jni_shim: postFrameCallbackA(cb=%p) -> doFrame imediato\n", cb);
    jni_choreo_doframe(env, nanos);
    return;
  }
  if (nm && strcmp(nm, "removeFrameCallback") == 0) {
    debugPrintf("jni_shim: removeFrameCallbackA\n");
    return;
  }
  if (nm && (strcmp(nm, "runOnUiThread") == 0 || strcmp(nm, "post") == 0 ||
             strcmp(nm, "postAtFrontOfQueue") == 0)) {
...
static void run_runnable(void *env, void *runnable) {
  ...
  void *iface = class_for("java/lang/Runnable");
...
static void *jni_NewObject(void *env, void *clazz, void *mid, ...) {
  ...
  if (cn && !strcmp(cn, "android/os/HandlerThread")) {
    debugPrintf("jni_shim: NewObject(HandlerThread) -> HandlerThread fake\n");
    return &g_handler_thread_obj;
  }
  if (cn && !strcmp(cn, "android/os/Handler")) {
    debugPrintf("jni_shim: NewObject(Handler) -> Handler fake\n");
