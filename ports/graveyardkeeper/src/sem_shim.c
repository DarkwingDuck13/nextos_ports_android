...
/* cond com CLOCK_MONOTONIC: a libunity (Android) computa o `abs` do sem_timedwait
 * em CLOCK_MONOTONIC (uptime ~10^4s). Com cond default (REALTIME), esse abs vira
 * 1970+10^4s = passado -> ETIMEDOUT imediato -> worker (Loading.Preload) gira a 100%
 * CPU e o loading nunca progride. Inicializar o cond em MONOTONIC casa os relógios. */
static void init_cond_monotonic(pthread_cond_t *c) {
...
/* converte um abs de relógio DESCONHECIDO (a libunity pode passar MONOTONIC ou
 * REALTIME) p/ um abs MONOTONIC válido p/ o nosso cond. Heurística pela ordem de
 * grandeza: se abs->tv_sec está perto do realtime atual (±1 dia) é REALTIME -> rebase
 * p/ monotonic; senão trata como já-monotonic. */
static void to_monotonic_abs(const struct timespec *in, struct timespec *out) {
  struct timespec rt, mo;
  clock_gettime(CLOCK_REALTIME, &rt);
  clock_gettime(CLOCK_MONOTONIC, &mo);
  long drt = (long)in->tv_sec - (long)rt.tv_sec;
  if (drt > -86400 && drt < 86400) {
    /* abs é REALTIME -> delta p/ agora + agora-monotonic */
    long ns = (long)(in->tv_nsec - rt.tv_nsec);
    long s  = drt;
    out->tv_sec = mo.tv_sec + s;
    out->tv_nsec = mo.tv_nsec + ns;
  } else {
    *out = *in;  /* já é monotonic */
  }
  if (out->tv_nsec >= 1000000000L) { out->tv_sec += out->tv_nsec / 1000000000L; out->tv_nsec %= 1000000000L; }
...
int sh_sem_init(void *s, int pshared, unsigned value) {
  ...
  if (!r && freeslot) {
    ...
    pthread_mutex_init(&r->m, NULL); init_cond_monotonic(&r->c);
  }
...
static void semwho_resolve(unsigned long ra, const char **lib, unsigned long *off) {
  *lib = "?";
  *off = ra;
  if (g_ubase && ra >= g_ubase && ra < g_ubase + g_usize) {
    *lib = "libunity";
    *off = ra - g_ubase;
  } else if (g_ibase && ra >= g_ibase && ra < g_ibase + g_isize) {
    *lib = "libil2cpp";
    *off = ra - g_ibase;
  }
}

static unsigned long semwho_parent_ra(void) {
  uintptr_t fp = (uintptr_t)__builtin_frame_address(0);
  if (!fp) return 0;
  uintptr_t caller_fp = *(uintptr_t *)fp;
  if (!caller_fp || (caller_fp & 7) || caller_fp <= fp || caller_fp > fp + 0x100000)
    return 0;
  return *((uintptr_t *)caller_fp + 1);
}

static void semwho_log(void *s, int count, unsigned long ra, unsigned long parent) {
  if (!getenv("GK_SEMWHO")) return;
  static struct { unsigned long ra, parent; void *sem; int tid; } seen[128]; static int ns;
  int tid = stid();
  for (int i = 0; i < ns; i++)
    if (seen[i].ra == ra && seen[i].parent == parent && seen[i].sem == s && seen[i].tid == tid) return;
  if (ns < 128) { seen[ns].ra = ra; seen[ns].parent = parent; seen[ns].sem = s; seen[ns].tid = tid; ns++; }
  const char *lib, *plib; unsigned long off, poff;
  semwho_resolve(ra, &lib, &off);
  semwho_resolve(parent, &plib, &poff);
  fprintf(stderr, "[GK_SEMWHO] sem_wait caller=%s+0x%lx parent=%s+0x%lx sem=%p tid=%d%s count=%d\n",
          lib, off, plib, poff, s, tid, (g_main_tid && tid == g_main_tid) ? " MAIN" : "", count);
  fsync(2);
}
int sh_sem_wait(void *s) {
  ...
  semwho_log(s, m->count, (unsigned long)__builtin_return_address(0), semwho_parent_ra());
  ...
  while (m->count <= 0) {
    ...
    if (poll) {
      struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts);  /* cond é MONOTONIC */
...
int sh_sem_timedwait(void *s, const struct timespec *abs) {
  ...
  int rc = 0;
  struct timespec mabs;
  if (abs) to_monotonic_abs(abs, &mabs);   /* casa o relógio do abs (Android) com o cond MONOTONIC */
  pthread_mutex_lock(&m->m);
  /* registra o sem p/ o tick (CUP_PRELOAD_TICK): threads de loading (Preload/AsyncReader)
     bloqueiam aqui esperando trabalho; se o wakeup-post se perdeu, o tick as re-acorda. */
  if (m->count <= 0 && g_main_tid && stid() != g_main_tid) register_tick_sem(s);
  while (m->count <= 0) {
    sigset_t o; gc_wait_unblock(&o);
    if (abs) {
      rc = pthread_cond_timedwait(&m->c, &m->m, &mabs);
      gc_wait_restore(&o);
