/*
 * main.c - Graveyard Keeper (Unity 2018.2.2f1 IL2CPP) so-loader -> NextOS/Mali-450.
 *
...
#define PT_LIST(X) \
  ...
  X("pthread_attr_init", pthread_attr_init_fake) X("pthread_attr_destroy", pthread_attr_destroy_fake) \
  X("pthread_attr_setdetachstate", pthread_attr_setdetachstate_fake) \
  X("pthread_attr_setstacksize", pthread_attr_setstacksize_fake) \
  X("pthread_attr_getstack", pthread_attr_getstack_fake) X("pthread_getattr_np", pthread_getattr_np_fake) \
...
#define PT_SET(n, f) set_import(n, (void *)f);
  PT_LIST(PT_SET)
  /* GK: pthread_create recebe pthread_attr_t bionic. Roteia sempre pelo fake para
     ignorar o attr incompatível e instalar o trampoline de thread. */
  if (!getenv("TER_NOPTCREATE")) set_import("pthread_create", (void *)pthread_create_fake);
}
static void patch_pthread_shim(void) {
  if (getenv("TER_NOPTSHIM")) return;
#define PT_PATCH(n, f) patch_got(n, (void *)f);
  PT_LIST(PT_PATCH)
  if (!getenv("TER_NOPTCREATE")) patch_got("pthread_create", (void *)pthread_create_fake);
}
...
/* PC_INLINETASK (legado de port Unity 2022 incompleto): o per-object task/fence wait está em
 * 0x65140c; o loop de espera (0x651530) checa `*(obj[0x58]) != 0` (flag de conclusão
 * que o worker setaria). Workers ociosos no so-loader -> flag fica 0 -> deadlock.
 * Fingimos a conclusão: setamos node[0]=1 -> predicado verdadeiro -> sai sem esperar. */
static int g_pc_inlinetask_n;
static void try_register_main(void);   /* fwd: PC_MAINREG registra a main como worker */
void pc_inline_task(void *obj) {
  try_register_main();                  /* 1ª oportunidade na main: registra p/ workers roubarem o async */
  if (!obj) return;
  void *node = *(void **)((char *)obj + 0x58);   /* obj[0x58] = result/fence node */
  if (node) *(volatile uintptr_t *)node = 1;     /* node[0]=1 -> satisfaz cbnz em 0x65153c */
  int n = __atomic_add_fetch(&g_pc_inlinetask_n, 1, __ATOMIC_RELAXED);
  if (n <= 5 || (n % 200) == 0) { fprintf(stderr, "[PC_INLINETASK] #%d obj=%p node=%p\n", n, obj, node); fsync(2); }
}
...
/* CUP_SKIPRESWAIT (legado de outro port): a tela de loading fica presa em
 * resourceRequestsEnded() esperando os AudioClips (Resources.LoadAsync) chegarem a
 * AudioDataLoadState.Loaded (==2). No so-loader esse load async/áudio não conclui ->
 * a tela do malabarista nunca transiciona p/ o menu (que carrega o datapack.unity3d).
 * Patchamos resourceRequestsEnded (il2cpp+0x16550A0) p/ `return true` -> transiciona. */
static void cup_skip_reswait(void) {
  static int done = 0;
  if (done || !g_il2cpp_base || !getenv("CUP_SKIPRESWAIT")) { if (!getenv("CUP_SKIPRESWAIT")) done = 1; return; }
  uint32_t *mp = (uint32_t *)(g_il2cpp_base + 0x16550A0);
  long pgsz = sysconf(_SC_PAGESIZE);
  void *pa = (void *)((uintptr_t)mp & ~((uintptr_t)pgsz - 1));
  mprotect(pa, pgsz * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
  mp[0] = 0x52800020u;   /* movz w0,#1 */
  mp[1] = 0xD65F03C0u;   /* ret */
  mprotect(pa, pgsz * 2, PROT_READ | PROT_EXEC);
  __builtin___clear_cache((char *)pa, (char *)pa + 8);
  fprintf(stderr, "[CUP_SKIPRESWAIT] resourceRequestsEnded -> return 1 @%p\n", (void *)mp); fsync(2);
  done = 1;
}
...
static FILE *my_fopen(const char *p, const char *m) {
  /* 🔑 Unity 2022 lê /proc/<pid>/cmdline via FOPEN (não open) p/ pegar argv (force-gfx-direct,
     job-worker-count, etc). Sem isto ele lê o argv real "./graveyardkeeper" -> ZERO args -> MT
     rendering ligado -> GfxDeviceWorker faz GL concorrente -> DEADLOCK no Mali Utgard. */
  if (p && strstr(p, "cmdline") && !getenv("CUP_NOGFXARGS")) {
    int fd = cmdline_fd();
...
#define GK_PORT_BASE "/storage/roms/ports/graveyardkeeper/"
#define ASSET_BASE_M GK_PORT_BASE "assets/"
static int asset_strip_res_suffix(char *path) {
  char *dot = strrchr(path, '.');
  if (dot && (!strcmp(dot, ".res") || !strcmp(dot, ".resS") || !strcmp(dot, ".resG"))) {
    *dot = 0;
    return 1;
  }
  return 0;
}
static int asset_leaf_is_hash32(const char *path) {
...
static const char *asset_use_existing_or_split0(char *buf, const char *path) {
  if (access(path, F_OK) == 0) {
    if (buf != path) snprintf(buf, 512, "%s", path);
    return buf;
  }
  if (!strstr(path, ".split")) {
    char split0[512];
    snprintf(split0, sizeof split0, "%s.split0", path);
    if (access(split0, F_OK) == 0) {
      snprintf(buf, 512, "%s", split0);
      return buf;
    }
  }
  return NULL;
}
static const char *asset_use_resource_variant(char *buf, const char *path) {
  char res[512];
  snprintf(res, sizeof res, "%s", path);
  char *assets = strstr(res, ".assets");
  if (assets && assets[7] == 0) {
    snprintf(assets, (size_t)(res + sizeof res - assets), ".resource");
    if (access(res, F_OK) == 0) {
      snprintf(buf, 512, "%s", res);
      return buf;
    }
  }
  snprintf(res, sizeof res, "%s.resource", path);
  if (access(res, F_OK) == 0) {
    snprintf(buf, 512, "%s", res);
    return buf;
  }
...
static const char *asset_existing_variant(char *buf) {
  char tmp[512];
  snprintf(tmp, sizeof tmp, "%s", buf);
  if (asset_strip_res_suffix(tmp)) {
    if (asset_leaf_is_hash32(tmp)) {
      snprintf(buf, 512, "%s", tmp);
      return buf;
    }
    if (asset_use_resource_variant(buf, tmp) || asset_use_existing_or_split0(buf, tmp)) return buf;
  }

  if (asset_use_existing_or_split0(buf, buf)) return buf;

  snprintf(tmp, sizeof tmp, "%s", buf);
  char *split = strstr(tmp, ".split");
  if (split) {
    char *res = NULL;
    char *p;
    if ((p = strstr(tmp, ".resG")) && p < split) res = p;
    if ((p = strstr(tmp, ".resS")) && p < split) res = p;
    if ((p = strstr(tmp, ".res")) && p < split) res = p;
    if (res) {
      memmove(res, split, strlen(split) + 1);
      if (asset_use_existing_or_split0(buf, tmp)) return buf;
      split = strstr(tmp, ".split");
    }
    if (split && !strcmp(split, ".split0")) {
      *split = 0;
      if (asset_use_existing_or_split0(buf, tmp)) return buf;
    }
  }
...
static const char *asset_redirect(const char *p, char *buf, size_t bufsz) {
  ...
  if (ab) {
    const char *sap = getenv("GK_SAPATH"); if (!sap) sap = getenv("CUP_SAPATH");
    if (!sap) sap = ASSET_BASE_M;
  ...
  if (!strncmp(p, ASSET_BASE_M "bin/Data/", sizeof(ASSET_BASE_M "bin/Data/") - 1)) {
    snprintf(buf, bufsz, "%s", p);
    return asset_existing_variant(buf);
  }
  const char *sub = strstr(p, "bin/Data/");
  if (sub) {
    snprintf(buf, bufsz, ASSET_BASE_M "bin/Data/%s", sub + 9);
    if (asset_existing_variant(buf)) return buf;
  }
  ...
  if (!strncmp(base, "level", 5) || !strncmp(base, "sharedassets", 12) ||
      ...
      strstr(base, ".res") || strstr(base, ".resS") || strstr(base, ".resG") ||
      strstr(base, ".resource") ||
...
static int cmdline_fd(void) {
  ...
  n += sprintf(buf + n, "graveyardkeeper") + 1;
  if (extra && *extra) {
    ...
    /* PC: 0 job-workers -> Unity roda TODOS os jobs INLINE na main thread (modo suportado).
       Contorna o muro do dispatch do job-system no so-loader (workers existem mas nunca
       recebem trabalho -> WaitForJobGroup trava no async load). PC_WORKERS sobrescreve. */
    if (!getenv("PC_NOJOBARG")) {
      const char *wc = getenv("PC_WORKERS"); if (!wc || !*wc) wc = "0";
      n += sprintf(buf + n, "-job-worker-count") + 1;
      n += sprintf(buf + n, "%s", wc) + 1;
    }
  }
  FILE *t = tmpfile();
  if (!t) return -1;
...
static void gk_statspy_log(const char *kind, const char *p, const char *r, int rc, uintptr_t ra) {
  if (!getenv("GK_STATSPY")) return;
  static unsigned long total, miss, asset_total;
  total++;
  if (rc < 0) miss++;
  if (p && strstr(p, "/assets/bin/Data/")) asset_total++;
  if (total <= 160 || (total % 1000) == 0) {
    const char *base = p ? strrchr(p, '/') : NULL;
    base = base ? base + 1 : (p ? p : "?");
    const char *rbase = r ? strrchr(r, '/') : NULL;
    rbase = rbase ? rbase + 1 : (r ? r : "-");
    char caller[64];
    if (g_unity_base && ra >= g_unity_base && ra < g_unity_base + text_size)
      snprintf(caller, sizeof caller, "libunity+0x%lx", (unsigned long)(ra - g_unity_base));
    else if (g_il2cpp_base && ra >= g_il2cpp_base && ra < g_il2cpp_base + 0x5000000)
      snprintf(caller, sizeof caller, "libil2cpp+0x%lx", (unsigned long)(ra - g_il2cpp_base));
    else
      snprintf(caller, sizeof caller, "0x%lx", (unsigned long)ra);
    fprintf(stderr, "[GK_STATSPY] %s #%lu asset=%lu miss=%lu rc=%d caller=%s %s -> %s\n",
            kind, total, asset_total, miss, rc, caller, base, rbase);
    fsync(2);
  }
}
static int gk_is_res_suffix(const char *p) {
  if (!p) return 0;
  const char *dot = strrchr(p, '.');
  return dot && (!strcmp(dot, ".res") || !strcmp(dot, ".resS") || !strcmp(dot, ".resG"));
}
static int gk_fast_dir_stat(const char *p, uintptr_t ra) {
  if (!p || getenv("GK_NOFASTDIRSTAT") || !g_unity_base) return 0;
  if (ra < g_unity_base || ra >= g_unity_base + text_size) return 0;
  if (ra - g_unity_base != 0x6c916c) return 0;  /* Unity DirectoryExists stat() caller */
  if (!strstr(p, "/assets/bin/Data/") || !gk_is_res_suffix(p)) return 0;
  errno = ENOENT;
  return 1;
}
static int gk_res_path_is_hash(const char *p) {
  if (!p || !strstr(p, "/assets/bin/Data/")) return 0;
  char tmp[512];
  snprintf(tmp, sizeof tmp, "%s", p);
  asset_strip_res_suffix(tmp);
  return asset_leaf_is_hash32(tmp);
}
static int gk_fast_hash_file_stat(const char *p, void *st, uintptr_t ra) {
  if (!p || !st || getenv("GK_NOFASTHASHSTAT") || !g_unity_base) return 0;
  if (ra < g_unity_base || ra >= g_unity_base + text_size) return 0;
  uintptr_t off = ra - g_unity_base;
  if (off != 0x25c9e0 && off != 0x6c923c) return 0;  /* Unity FileExists-style stat callers */
  if (!gk_res_path_is_hash(p)) return 0;
...
static int my_stat(const char *p, void *st) {
  uintptr_t ra = (uintptr_t)__builtin_return_address(0);
  if (gk_fast_dir_stat(p, ra)) {
    gk_statspy_log("stat-fastdir", p, NULL, -1, ra);
    return -1;
  }
  if (gk_fast_hash_file_stat(p, st, ra)) {
    gk_statspy_log("stat-fasthash", p, NULL, 0, ra);
    return 0;
  }
  char rb[512]; const char *r = asset_redirect(p, rb, sizeof rb);
  ...
  gk_statspy_log("stat", p, r, rc, ra);
  return rc;
}
static int my_lstat(const char *p, void *st) {
  char rb[512]; const char *r = asset_redirect(p, rb, sizeof rb);
  int rc = lstat(r ? r : p, (struct stat *)st);
  gk_statspy_log("lstat", p, r, rc, (uintptr_t)__builtin_return_address(0));
  return rc;
}
...
static int my_stat64(const char *p, void *st) {
  ...
  gk_statspy_log("stat64", p, r, rc, (uintptr_t)__builtin_return_address(0));
  return rc;
}
static int my_lstat64(const char *p, void *st) {
  char rb[512]; const char *r = asset_redirect(p, rb, sizeof rb);
  int rc = lstat64(r ? r : p, (struct stat64 *)st);
  gk_statspy_log("lstat64", p, r, rc, (uintptr_t)__builtin_return_address(0));
  return rc;
}
...
static int g_pc_skipjobwait = 0;
#ifndef SYS_futex
#define SYS_futex 98
#endif

/* ===== PC_KICKWORKERS: dispatch manual dos semáforos de worker do job-system =====
 * Dado da análise de futex (TER_FUTEXLOG): os Job.Worker/Background Job./Loading.* ficam
 * parkados em Semaphore::Acquire (futex) e seus semáforos NUNCA são Released pelo scheduler
 * no so-loader -> o job de async-load nunca roda -> a main trava em WaitForJobGroup.
 * Aqui registramos o uaddr (=base do objeto Semaphore, count em [x19]) de cada thread-worker
 * quando ela faz FUTEX_WAIT, e periodicamente chamamos o Semaphore::Release REAL do engine
 * (libunity+0x400a08) sobre cada um -> os workers acordam, drenam a fila, rodam o job. */
static struct { int tid; int isw; volatile long uaddr; } g_wsem[96];
static volatile int g_wsem_n = 0;
static pthread_mutex_t g_wsem_lk = PTHREAD_MUTEX_INITIALIZER;
static int g_kickworkers = 0;   /* 1=engine Release, 2=raw futex post */
/* classifica a thread tid pelo comm: é worker/loader do job-system? (cache via g_wsem.isw) */
static int wsem_is_worker_comm(int tid) {
...
static void wsem_record(int tid, long uaddr) {
  pthread_mutex_lock(&g_wsem_lk);
  int i; for (i = 0; i < g_wsem_n; i++) if (g_wsem[i].tid == tid) break;
  if (i == g_wsem_n && g_wsem_n < 96) { g_wsem[i].tid = tid; g_wsem[i].isw = wsem_is_worker_comm(tid); g_wsem_n++; }
  if (i < 96) g_wsem[i].uaddr = uaddr;
  pthread_mutex_unlock(&g_wsem_lk);
}
/* dispara o dispatch: Release (ou raw post) em cada semáforo de worker registrado */
static int addr_readable(uintptr_t a);  /* fwd (def @ ~377) */
#define addr_readable_q(a) addr_readable((uintptr_t)(a))
extern volatile int g_render_frame;
static void kick_workers(void) {
  void (*rel)(void *) = (void (*)(void *))(g_unity_base + 0x400a08); /* Semaphore::Release */
  static int beat = 0; int kicked = 0, total = 0, isw = 0;
  if ((beat % 50) == 0) { fprintf(stderr, "[KICK] enter beat=%d nrec=%d f=%d\n", beat, g_wsem_n, g_render_frame); fsync(2); }
  pthread_mutex_lock(&g_wsem_lk);
  long snap[96]; int ns = 0;
  for (int i = 0; i < g_wsem_n; i++) {
    total++;
    if (!g_wsem[i].isw) continue;
    isw++;
    long ua = g_wsem[i].uaddr; if (!ua) continue;
    if (ns < 96) snap[ns++] = ua;
  }
  pthread_mutex_unlock(&g_wsem_lk);
  for (int i = 0; i < ns; i++) {
    long ua = snap[i];
    /* valida o endereço SEM crashar: FUTEX_WAKE count=0 retorna -EFAULT se não-mapeado */
    if (syscall(SYS_futex, ua, 1 /*FUTEX_WAKE*/, 0, 0, 0, 0) < 0) continue;
    kicked++;
    if (g_kickworkers == 3)      { syscall(SYS_futex, ua, 1, 0x7fffffff, 0, 0, 0); }            /* só acorda todos */
    else if (g_kickworkers == 2) { (*(volatile int *)ua)++; syscall(SYS_futex, ua, 1, 1, 0, 0, 0); } /* post+wake */
    else                         { rel((void *)ua); }                                          /* engine Release */
  }
  if ((beat % 50) == 0) { fprintf(stderr, "[KICK] done beat=%d threads=%d isw=%d kicked=%d f=%d\n", beat, total, isw, kicked, g_render_frame); fsync(2); }
  beat++;
}
/* thread independente: kicka os workers ~50Hz mesmo enquanto a main está presa
 * DENTRO de nativeRender (WaitForJobGroup). PC_KICKMS sobrescreve o intervalo. */
static void *kick_worker_thread(void *arg) {
  (void)arg;
  long ms = getenv("PC_KICKMS") ? atol(getenv("PC_KICKMS")) : 20;
  int warm = getenv("PC_KICKWARM") ? atoi(getenv("PC_KICKWARM")) : 200; /* espera N ciclos p/ libunity carregar */
  for (;;) {
    if (warm > 0) { warm--; }
    else if (g_unity_base) kick_workers();
...
static long my_syscall(long n, long a1, long a2, long a3, long a4, long a5, long a6) {
  ...
  if (n == SYS_futex) {
    ...
    /* PC_SKIPJOBWAIT: a MAIN thread bloqueia em WaitForJobGroup (futex RAW de um semáforo
       Baselib) esperando jobs que o dispatch quebrado do so-loader nunca executa. Igual ao
       TER_SKIPJOBWAIT do Terraria: NÃO deixa a main bloquear — escreve o valor "satisfeito"
       no uaddr (count++ -> o acquire decrementa e segue) e retorna EAGAIN (sem dormir).
       Só a MAIN (workers de verdade não são tocados). Jobs ficam FINGIDOS (igual Terraria). */
    if (g_pc_skipjobwait && (op == 0 || op == 9) && g_main_tid && (int)syscall(178) == g_main_tid) {
      /* CIRÚRGICO: só pula quando a main repete o MESMO uaddr N× consecutivas no MESMO frame
         (= presa no loop do WaitForJobGroup). Waits legítimos 1-shot (mutex/cond) passam. */
      static long last_ua = 0; static int streak = 0; static int last_frame = -1;
      extern volatile int g_render_frame;
      if (a1 == last_ua && g_render_frame == last_frame) streak++;
      else { last_ua = a1; last_frame = g_render_frame; streak = 0; }
      int thr = getenv("PC_SKIPJW_N") ? atoi(getenv("PC_SKIPJW_N")) : 30;
      if (streak > thr) {
        static int logged = 0; if (logged++ < 8) { fprintf(stderr, "[SKIPJOBWAIT] uaddr=%p streak=%d frame=%d -> bump+skip\n", (void *)a1, streak, g_render_frame); fsync(2); }
        if (a1) *(volatile int *)a1 = (int)a3 + 1;   /* satisfaz o acquire */
        errno = EAGAIN; return -1;
      }
    }
    if (op == 0 || op == 9) {   /* FUTEX_WAIT / FUTEX_WAIT_BITSET: thread vai BLOQUEAR */
      if (g_kickworkers) wsem_record((int)syscall(178 /*gettid*/), a1);
      long t4 = a4;
      struct timespec ts;
...
static int my_statfs64(const char *p, void *buf) {
  ...
  int rc = real ? real(GK_PORT_BASE, buf) : -1;
  /* 🖤 DISK-FULL FIX: o cartão do .164 é pequeno (~936MB livres). Unity/Mono checa o espaço
     livre via statfs64 ANTES de instalar resources/escrever → vê <1GB → lança
     "IOException: Disk full" (FileStream.WriteInternal). No Terraria o SD era grande (93GB) e o
     real bastava; aqui INFLAMOS p/ 50GB livres (igual os métodos StatFs JNI). Layout statfs64
     (LP64, igual glibc/bionic): f_bsize@+8, f_blocks@+16, f_bfree@+24, f_bavail@+32 (todos u64). */
  if (rc == 0 && buf) {
    uint64_t *u = (uint64_t *)buf;
    uint64_t bsize = u[1] ? u[1] : 4096;
    uint64_t freeb = (50ULL << 30) / bsize;     /* 50 GB livres */
    uint64_t totb  = (100ULL << 30) / bsize;    /* 100 GB total */
    if (u[2] < totb) u[2] = totb;               /* f_blocks  */
    u[3] = freeb;                               /* f_bfree   */
    u[4] = freeb;                               /* f_bavail  */
  }
  if (g_dllog) fprintf(stderr, "[statfs64] path=%s -> GAMEDIR rc=%d (free inflado p/ 50GB)\n", p ? p : "?", rc);
  return rc;
}
...
/* ===== fbdev EGL multiplexer (Mali-450): Unity 2022 MT/native-gfx-jobs cria GL em
   MÚLTIPLAS threads; o Mali Utgard (single-context p/ window) trava (render thread no
   _gles_clear espera o frame-builder lock que a present thread segura no swap). Igual ao
   egl_shim do kmsdrm: SÓ a 1ª thread (render) usa a window REAL; as demais recebem um
   pbuffer e o swap delas é no-op. Gated CUP_EGLMUX (default ON no fbdev). */
static void *(*r_eglCreateWindowSurface)(void*,void*,void*,const int*);
...
static void *g_mux_dpy, *g_mux_cfg, *g_mux_winsurf, *g_mux_pbuf, *g_mux_pbcfg;
static volatile long g_mux_owner_tid;
static int g_egllog;
static _Thread_local int tls_mux_window;
static void *mux_CreateWindowSurface(void *dpy, void *cfg, void *win, const int *attr) {
  if (!r_eglCreateWindowSurface) r_eglCreateWindowSurface = dlsym(RTLD_DEFAULT, "eglCreateWindowSurface");
  void *s = r_eglCreateWindowSurface(dpy, cfg, win, attr);
  g_mux_dpy = dpy; g_mux_cfg = cfg; g_mux_winsurf = s;   /* a window REAL (única) */
  if (g_egllog) { fprintf(stderr, "[EGLMUX] CreateWindowSurface -> %p (dpy=%p cfg=%p tid=%d)\n", s, dpy, cfg, (int)syscall(178)); fsync(2); }
  return s;
}
static void *mux_get_pbuf(void) {   /* pbuffer compartilhado p/ threads não-donas */
  if (g_mux_pbuf) return g_mux_pbuf;
  if (!r_eglCreatePbufferSurface) r_eglCreatePbufferSurface = dlsym(RTLD_DEFAULT, "eglCreatePbufferSurface");
  if (!r_eglChooseConfig) r_eglChooseConfig = dlsym(RTLD_DEFAULT, "eglChooseConfig");
  void *cfg = g_mux_cfg;
  /* config própria com EGL_PBUFFER_BIT (a config da window pode não suportar pbuffer) */
  if (r_eglChooseConfig && g_mux_dpy) {
    int a[] = { 0x3033/*SURFACE_TYPE*/, 0x0001/*PBUFFER_BIT*/, 0x3040/*RENDERABLE_TYPE*/, 0x0004/*ES2*/, 0x3038 };
    void *cfgs[4]; int n = 0;
    if (r_eglChooseConfig(g_mux_dpy, a, cfgs, 4, &n) && n > 0) { cfg = cfgs[0]; g_mux_pbcfg = cfg; }
  }
  int pb[] = { 0x3057/*WIDTH*/, 16, 0x3056/*HEIGHT*/, 16, 0x3038/*NONE*/ };
  g_mux_pbuf = r_eglCreatePbufferSurface(g_mux_dpy, cfg, pb);
  if (g_egllog) { fprintf(stderr, "[EGLMUX] pbuffer criado %p (cfg=%p)\n", g_mux_pbuf, cfg); fsync(2); }
  return g_mux_pbuf;
}
/* mux MakeCurrent: SÓ a 1ª thread que torna a window current é a DONA (usa fb0 real);
   as demais recebem o pbuffer -> não tocam o frame-builder do Mali concorrentemente. */
static unsigned mux_MakeCurrent(void *dpy, void *draw, void *read, void *ctx) {
  if (!r_eglMakeCurrent) r_eglMakeCurrent = dlsym(RTLD_DEFAULT, "eglMakeCurrent");
  long me = (long)syscall(178);
  if (draw && draw == g_mux_winsurf && ctx) {
    long owner = g_mux_owner_tid;
    if (owner == 0) { long z = 0; if (__atomic_compare_exchange_n(&g_mux_owner_tid, &z, me, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) owner = me; else owner = g_mux_owner_tid; }
    if (owner == me) { tls_mux_window = 1; if (g_egllog){fprintf(stderr,"[EGLMUX] MakeCurrent WINDOW (owner tid=%ld)\n",me);fsync(2);} return r_eglMakeCurrent(dpy, draw, read, ctx); }
    void *pb = mux_get_pbuf(); tls_mux_window = 0;
    if (g_egllog){fprintf(stderr,"[EGLMUX] MakeCurrent PBUFFER (tid=%ld -> %p)\n",me,pb);fsync(2);}
    return r_eglMakeCurrent(dpy, pb ? pb : draw, pb ? pb : read, ctx);
  }
  if (!draw && !ctx) { tls_mux_window = 0; }
  return r_eglMakeCurrent(dpy, draw, read, ctx);
}
static unsigned mux_SwapBuffers(void *dpy, void *surf) {
  if (!r_eglSwapBuffers) r_eglSwapBuffers = dlsym(RTLD_DEFAULT, "eglSwapBuffers");
  if (tls_mux_window && surf == g_mux_winsurf) return r_eglSwapBuffers(dpy, surf);
  return 1;   /* swap de pbuffer/não-dona = no-op */
}
/* força eglSwapInterval(0): a present thread do Mali trava no ioctl de vsync do swap
   SEGURANDO o frame-builder lock -> render thread (glClear) deadlocka. Sem vsync o swap
   solta o lock rápido. Sempre 0 no fbdev (CUP_VSYNC=N força outro valor). */
static unsigned (*r_eglSwapInterval)(void*,int);
static unsigned mux_SwapInterval(void *dpy, int iv) {
  if (!r_eglSwapInterval) r_eglSwapInterval = dlsym(RTLD_DEFAULT, "eglSwapInterval");
  int want = getenv("CUP_VSYNC") ? atoi(getenv("CUP_VSYNC")) : 0;
  if (g_egllog) { fprintf(stderr, "[EGLMUX] SwapInterval(%d) -> %d\n", iv, want); fsync(2); }
  return r_eglSwapInterval ? r_eglSwapInterval(dpy, want) : 1;
}
 ...
 * em /storage/roms/ports/graveyardkeeper/shot.ppm (verificacao autonoma de IMAGEM no fbdev Mali). */
static void ter_screenshot_maybe(void) {
  ...
  FILE *f = fopen(GK_PORT_BASE "shot.ppm","wb");
  if (f) { fprintf(f,"P6\n%d %d\n255\n", w, h);
...
/* idêntico mas em libunity (base diferente) — relocaliza o prólogo (trata adrp). */
static int unity_install_hook4(unsigned long off, void* fn, void** orig_out) {
  uintptr_t target=g_unity_base+off; uint32_t*o=(uint32_t*)target; long pgsz=sysconf(_SC_PAGESIZE);
  uint32_t*tr=mmap(NULL,4096,PROT_READ|PROT_WRITE|PROT_EXEC,MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
  if(tr==MAP_FAILED) return 0;
  for(int i=0;i<4;i++) tr[i]=ter_reloc_insn(o[i], target+i*4, (uintptr_t)tr+i*4);
  tr[4]=0x58000050u; tr[5]=0xD61F0200u; *(uint64_t*)(tr+6)=(uint64_t)(target+16);
...
/* PC_QREDIR: a MAIN (render thread) não está registrada no JobManager → d6e410
   ("GetCurrentThreadJobQueue") devolve uma fila thread-local fallback que NINGUÉM drena
   (workers drenam as suas; a main não roda loop de drain) → o ScheduleAndWait @0x65140c
   empurra o trabalho nessa fila órfã e espera p/ sempre. FIX: redireciona d6e410 da main
   p/ a fila de UM worker (capturada no 1º d6e410 de thread não-main); o push posta o
   semáforo do worker, que acorda, roda o trabalho e sinaliza a conclusão. */
static void *(*g_orig_d6e410)(void);
static void *g_worker_q;
/* core::String (Unity) extractor: flag@+32 (==1 short), short=inline@+0 len=24-[+24],
   long=ptr@+0 len@+16. Usado p/ logar a haystack do mount do datapack. */
static int corestr_get(const void *s, char *out, int max) {
  if (!s) { out[0] = 0; return 0; }
  unsigned char flag = ((const unsigned char *)s)[32];
  const char *data; long len;
  if (flag == 1) { data = (const char *)s; len = 24 - ((const unsigned char *)s)[24]; }
  else { data = *(const char *const *)s; len = *(const long *)((const char *)s + 16); }
  if (len < 0) len = 0; if (len > max - 1) len = max - 1;
  if (data) memcpy(out, data, len); out[len] = 0; return (int)len;
}
...
static void (*g_orig_wreg)(void *);
static unsigned long g_wdesc_template[32];   /* snapshot ESTÁVEL dos bytes do 1º descritor */
static int g_wdesc_have;
void my_wreg(void *desc) {
  static int n = 0;
  if (!g_wdesc_have) { memcpy(g_wdesc_template, desc, sizeof g_wdesc_template); g_wdesc_have = 1; }
  if (n++ < 3) {
    unsigned long *u = (unsigned long *)desc;
    char line[400]; int o = 0;
    o += snprintf(line + o, sizeof line - o, "[WREG] desc=%p:", desc);
    for (int i = 0; i < 12; i++) o += snprintf(line + o, sizeof line - o, " [%d]=%lx", i * 8, u[i]);
    fprintf(stderr, "%s\n", line); fsync(2);
  }
  g_orig_wreg(desc);
}
/* PC_MAINREG: registra a MAIN na lista global de workers (0x63a260) p/ que os workers ROUBEM
   os jobs async que a main agenda (singleton 0x65140c, áudio, datapack) — hoje vão p/ fila órfã.
   Forja um descritor copiando o template de um worker real (g_wdesc_template) e aponta [+80] p/
   o worker-context da main (threadinfo[+16], de GetOrCreateThreadInfo 0x593880). */
static int g_mainreg_on = -1, g_main_registered = 0;
static int mainreg_on(void) { if (g_mainreg_on < 0) g_mainreg_on = getenv("PC_MAINREG") ? 1 : 0; return g_mainreg_on; }
static void try_register_main(void) {
  if (!mainreg_on() || g_main_registered || !g_wdesc_have || !g_orig_wreg || !g_unity_base) return;
  if ((int)syscall(SYS_gettid) != getpid()) return;
  g_main_registered = 1;
  void *(*getti)(void) = (void *)(g_unity_base + 0x593880);
  void *ti = getti();
  void *wctx = ti ? ((void **)ti)[2] : NULL;   /* threadinfo[+16] = worker-context */
  void *(*get_q)(void) = (void *)(g_unity_base + 0xd6e410);  /* fila thread-local da main */
  void *mq = get_q();
  unsigned long *desc = calloc(1, 0x100);
  memcpy(desc, g_wdesc_template, 0x90);
  desc[0] = 0; desc[1] = 0;                     /* [+0]/[+8] list links */
  desc[2] = (unsigned long)calloc(1, 0x2000);   /* [+16] buffer próprio */
  desc[4] = 0;                                  /* [+32] tid (preenchido pelo reg) */
  unsigned long qfield = getenv("PC_QFIELD") ? strtoul(getenv("PC_QFIELD"), 0, 0) : 80;
  void *qv = getenv("PC_USEWCTX") ? wctx : mq;  /* default = fila d6e410 da main */
  if (qv) desc[qfield / 8] = (unsigned long)qv; /* [+qfield] = fila da main p/ steal */
  fprintf(stderr, "[MAINREG] registrando main: ti=%p wctx=%p mq=%p qfield=%lu desc=%p\n",
          ti, wctx, mq, qfield, (void *)desc); fsync(2);
  g_orig_wreg(desc);
  fprintf(stderr, "[MAINREG] registrada OK\n"); fsync(2);
}
void *g_main_q;
void *my_d6e410(void) {
  if (mainreg_on() && !g_main_registered) try_register_main();
  void *q = g_orig_d6e410();
  int ismain = ((int)syscall(SYS_gettid) == getpid());
  static int nm = 0, nw = 0;
  if (ismain) {
    if (q && !g_main_q) g_main_q = q;
    if (q && nm++ < 4) {
      void **vt = *(void ***)q;                 /* q[0] = vtable */
      fprintf(stderr, "[QREDIR] MAIN q=%p vtbl=%p push[496]=%p state[q+256]=%d\n",
              q, (void *)vt, vt ? vt[62] : NULL, *(int *)((char *)q + 256)); fsync(2);
    }
  } else {
    if (q && nw++ < 4) { fprintf(stderr, "[QREDIR] WORKER tid=%d q=%p\n", (int)syscall(SYS_gettid), q); fsync(2); }
    if (!g_worker_q && q) g_worker_q = q;
  }
  return q;
}
...
static unsigned my_eglSwapBuffers(void *dpy, void *surf) {
  ...
  cup_skip_reswait();   /* CUP_SKIPRESWAIT: pula a espera de áudio da tela de loading */
...
BS_WRAP(0, "Preloader.Awake")
BS_WRAP(1, "Preloader.Awake.m0")
BS_WRAP(2, "Preloader.Awake.m1")
BS_WRAP(3, "Preloader.OnSceneLoaded")
BS_WRAP(4, "Preloader.OnMainGameLoaded")
BS_WRAP(5, "Preloader.Hide")
BS_WRAP(6, "Preloader.SetProgressBar")
BS_WRAP(7, "MainGame.StartGameLoading")
BS_WRAP(8, "SceneManager.LoadSceneAsyncInternal")
BS_WRAP(9, "SceneManager.LoadSceneAsync(string,mode)")
BS_WRAP(10, "SceneManager.add_sceneLoaded")
BS_WRAP(11, "AsyncOperation.get_isDone")
BS_WRAP(12, "AsyncOperation.set_allowSceneActivation")
BS_WRAP(13, "LoadingGUI.LinkAsyncProcess")
BS_WRAP(14, "LoadingGUI.Update")
BS_WRAP(15, "GameLoader.StartLoadingScene")
...
static void bootspy_install(uintptr_t base) {
  struct { uintptr_t rva; void *hook; void **orig; const char *nm; } T[] = {
    {0x32E830C, (void *)bs_hook_0, (void **)&bs_orig_0, "Preloader.Awake"},
    {0x31F5BB4, (void *)bs_hook_1, (void **)&bs_orig_1, "Preloader.Awake.m0"},
    {0x31F5D34, (void *)bs_hook_2, (void **)&bs_orig_2, "Preloader.Awake.m1"},
    {0x32E91A8, (void *)bs_hook_3, (void **)&bs_orig_3, "Preloader.OnSceneLoaded"},
    {0x32E94B8, (void *)bs_hook_4, (void **)&bs_orig_4, "Preloader.OnMainGameLoaded"},
    {0x32E9550, (void *)bs_hook_5, (void **)&bs_orig_5, "Preloader.Hide"},
    {0x32E8CD4, (void *)bs_hook_6, (void **)&bs_orig_6, "Preloader.SetProgressBar"},
    {0x31B64A0, (void *)bs_hook_7, (void **)&bs_orig_7, "MainGame.StartGameLoading"},
    {0x284ED7C, (void *)bs_hook_8,  (void **)&bs_orig_8,  "SceneManager.LoadSceneAsyncInternal"},
    {0x284F1C0, (void *)bs_hook_9,  (void **)&bs_orig_9,  "SceneManager.LoadSceneAsync(string,mode)"},
    {0x284EF44, (void *)bs_hook_10, (void **)&bs_orig_10, "SceneManager.add_sceneLoaded"},
    {0x258D578, (void *)bs_hook_11, (void **)&bs_orig_11, "AsyncOperation.get_isDone"},
    {0x258D650, (void *)bs_hook_12, (void **)&bs_orig_12, "AsyncOperation.set_allowSceneActivation"},
    {0x31B36AC, (void *)bs_hook_13, (void **)&bs_orig_13, "LoadingGUI.LinkAsyncProcess"},
    {0x31B3710, (void *)bs_hook_14, (void **)&bs_orig_14, "LoadingGUI.Update"},
    {0x0F9B188, (void *)bs_hook_15, (void **)&bs_orig_15, "GameLoader.StartLoadingScene"},
  };
...
static const char *il2cpp_str_ascii(void *s, char *buf, size_t cap) {
  if (!s || cap == 0) return "";
...
static void *(*ls_scene_async_orig)(void *, void *, int, void *);
static void *volatile g_gk_async_op;

#define GK_RVA_ASYNC_ISDONE   0x258D578
#define GK_RVA_ASYNC_PROGRESS 0x258D5E4
#define GK_RVA_ASYNC_ALLOW    0x258D650
#define GK_RVA_ASYNC_INVOKE   0x258D744

static int (*gk_async_isdone_direct)(void *, void *);
static float (*gk_async_progress_direct)(void *, void *);
static void (*gk_async_allow_direct)(void *, int, void *);

static void gk_async_tools_init(uintptr_t base) {
  if (!base) return;
  if (!gk_async_isdone_direct) gk_async_isdone_direct = (int (*)(void *, void *))(base + GK_RVA_ASYNC_ISDONE);
  if (!gk_async_progress_direct) gk_async_progress_direct = (float (*)(void *, void *))(base + GK_RVA_ASYNC_PROGRESS);
  if (!gk_async_allow_direct) gk_async_allow_direct = (void (*)(void *, int, void *))(base + GK_RVA_ASYNC_ALLOW);
}

static const char *gk_ptr_label(uintptr_t p, char *buf, size_t cap) {
  if (!p) { snprintf(buf, cap, "0"); return buf; }
  if (g_unity_base && p >= g_unity_base && p < g_unity_base + text_size)
    snprintf(buf, cap, "libunity+0x%lx", (unsigned long)(p - g_unity_base));
  else if (g_il2cpp_base && p >= g_il2cpp_base && p < g_il2cpp_base + 0x3000000)
    snprintf(buf, cap, "libil2cpp+0x%lx", (unsigned long)(p - g_il2cpp_base));
  else
    snprintf(buf, cap, "0x%lx", (unsigned long)p);
  return buf;
}

static void gk_async_dump_vtable(const char *tag, uintptr_t vt) {
  if (!vt || !addr_readable(vt) || !addr_readable(vt + 15 * sizeof(uintptr_t))) return;
  uintptr_t e[16];
  for (int i = 0; i < 16; i++) e[i] = *(uintptr_t *)(vt + i * sizeof(uintptr_t));
  char b0[48], b1[48], b2[48], b3[48];
  fprintf(stderr, "[GK_ASYNC:%s] native.vt=%lx\n", tag, (unsigned long)vt);
  for (int i = 0; i < 16; i += 4) {
    fprintf(stderr, "[GK_ASYNC:%s]  vt[%02d..%02d]=%s %s %s %s\n", tag, i, i + 3,
            gk_ptr_label(e[i + 0], b0, sizeof b0),
            gk_ptr_label(e[i + 1], b1, sizeof b1),
            gk_ptr_label(e[i + 2], b2, sizeof b2),
            gk_ptr_label(e[i + 3], b3, sizeof b3));
  }
}

static void *gk_async_native_current(void) {
  void *op = (void *)g_gk_async_op;
  if (!op) return NULL;
  if (g_maps_len == 0) maps_snapshot();
  if (!addr_readable((uintptr_t)op + 0x18)) return NULL;
  return *(void **)((char *)op + 0x10);
}

static void gk_async_native_fields(const char *tag, void *self) {
  if (!self) self = gk_async_native_current();
  if (!self) return;
  if (g_maps_len == 0) maps_snapshot();
  if (!addr_readable((uintptr_t)self + 0x348)) return;
  uint32_t state48 = *(uint32_t *)((char *)self + 0x48);
  float progress = *(float *)((char *)self + 0x6c);
  uintptr_t sub208 = *(uintptr_t *)((char *)self + 0xd0);
  uint32_t word832 = *(uint32_t *)((char *)self + 0x340);
  uint16_t half836 = *(uint16_t *)((char *)self + 0x344);
  uint8_t flag837 = *(uint8_t *)((char *)self + 0x345);
  uint8_t flag838 = *(uint8_t *)((char *)self + 0x346);
  fprintf(stderr,
          "[GK_ASYNC:%s] fields native=%p state48=%u progress=%.3f sub208=%lx w832=%x h836=%x b837=%u b838=%u f=%d\n",
          tag, self, state48, progress, (unsigned long)sub208, word832,
          half836, flag837, flag838, g_render_frame);
}

static void gk_async_dump(const char *tag) {
  maps_snapshot();
  void *op = (void *)g_gk_async_op;
  if (!op || !addr_readable((uintptr_t)op + 0x60)) return;
  uintptr_t q[12];
  for (int i = 0; i < 12; i++) q[i] = *(uintptr_t *)((char *)op + i * 8);
  fprintf(stderr,
          "[GK_ASYNC:%s] op=%p q=%lx %lx %lx %lx %lx %lx %lx %lx %lx %lx %lx %lx f=%d\n",
          tag, op,
          (unsigned long)q[0], (unsigned long)q[1], (unsigned long)q[2], (unsigned long)q[3],
          (unsigned long)q[4], (unsigned long)q[5], (unsigned long)q[6], (unsigned long)q[7],
          (unsigned long)q[8], (unsigned long)q[9], (unsigned long)q[10], (unsigned long)q[11],
          g_render_frame);
  uintptr_t np = q[2];
  if (np && addr_readable(np + 0x80)) {
    uintptr_t n[16];
    for (int i = 0; i < 16; i++) n[i] = *(uintptr_t *)(np + i * 8);
    fprintf(stderr,
            "[GK_ASYNC:%s] native@%lx q=%lx %lx %lx %lx %lx %lx %lx %lx\n",
            tag, (unsigned long)np,
            (unsigned long)n[0], (unsigned long)n[1], (unsigned long)n[2], (unsigned long)n[3],
            (unsigned long)n[4], (unsigned long)n[5], (unsigned long)n[6], (unsigned long)n[7]);
    fprintf(stderr,
            "[GK_ASYNC:%s] native@%lx q8=%lx %lx %lx %lx %lx %lx %lx %lx\n",
            tag, (unsigned long)np,
            (unsigned long)n[8], (unsigned long)n[9], (unsigned long)n[10], (unsigned long)n[11],
            (unsigned long)n[12], (unsigned long)n[13], (unsigned long)n[14], (unsigned long)n[15]);
    gk_async_native_fields(tag, (void *)np);
    gk_async_dump_vtable(tag, n[0]);
  }
  fsync(2);
}

static void gk_async_poll(int force_allow) {
  static int maps_frame = -999999;
  if (g_maps_len == 0 || g_render_frame - maps_frame > 120) {
    maps_snapshot();
    maps_frame = g_render_frame;
  }
  void *op = (void *)g_gk_async_op;
  if (!op || !addr_readable((uintptr_t)op + 0x20) ||
      !gk_async_isdone_direct || !gk_async_progress_direct) return;
  static void *last_op;
  static int last_done = -2, last_milli = -100000;
  int done = gk_async_isdone_direct(op, NULL);
  float progress = gk_async_progress_direct(op, NULL);
  int milli = (int)(progress * 1000.0f);
  if (op != last_op || done != last_done || milli != last_milli || (g_render_frame % 60) == 0) {
    uintptr_t native = *(uintptr_t *)((char *)op + 0x10);
    uintptr_t n0 = 0, n1 = 0, n6 = 0, n7 = 0;
    if (native && addr_readable(native + 0x40)) {
      n0 = *(uintptr_t *)(native + 0x00);
      n1 = *(uintptr_t *)(native + 0x08);
      n6 = *(uintptr_t *)(native + 0x30);
      n7 = *(uintptr_t *)(native + 0x38);
    }
    fprintf(stderr,
            "[GK_ASYNCPOLL] f=%d op=%p native=%lx done=%d progress=%.3f n0=%lx n1=%lx n6=%lx n7=%lx\n",
            g_render_frame, op, (unsigned long)native, done, progress,
            (unsigned long)n0, (unsigned long)n1, (unsigned long)n6, (unsigned long)n7);
    fsync(2);
    last_op = op;
    last_done = done;
    last_milli = milli;
  }
  if (force_allow && gk_async_allow_direct && (g_render_frame % 120) == 0) {
    gk_async_allow_direct(op, 1, NULL);
    fprintf(stderr, "[GK_FORCE_ALLOW] set_allowSceneActivation(%p,1) f=%d\n", op, g_render_frame);
    fsync(2);
  }
}

static int gk_async_is_current_native(void *self) {
  return self && self == gk_async_native_current();
}
...
static int gk_vt11_active(void) {
  ...
  return gk_vt11_depth > 0 && gk_vt11_tid == tid;
}

static float gk_async_progress_raw(void) {
  void *self = gk_async_native_current();
  if (!self || !addr_readable((uintptr_t)self + 0x70)) return -1.0f;
  return *(float *)((char *)self + 0x6c);
}

static long (*gk_async_vt11_orig)(void *, long, long, long, long, long, long, long);
...
static long (*gk_async_vt12_orig)(void *, long, long, long, long, long, long, long);
...
static long (*gk_async_vt13_orig)(void *, long, long, long, long, long, long, long);
...
static void *ls_scene_async_hook(void *self, void *scene, int mode, void *method) {
  char name[96];
  void *r = ls_scene_async_orig(self, scene, mode, method);
  g_gk_async_op = r;
  fprintf(stderr, "[LOADSPY] LoadSceneAsync(\"%s\", mode=%d) -> %p f=%d\n",
          il2cpp_str_ascii(scene, name, sizeof name), mode, r, g_render_frame);
  fsync(2);
  return r;
}
static void *(*ls_scene_internal_orig)(void *, void *, int, int, int, void *);
static void *ls_scene_internal_hook(void *self, void *scene, int index, int additive, int must, void *method) {
  char name[96];
  int orig_must = must;
  if (getenv("GK_SCENE_MUST")) must = 1;
  void *r = ls_scene_internal_orig(self, scene, index, additive, must, method);
  g_gk_async_op = r;
  fprintf(stderr, "[LOADSPY] LoadSceneAsyncInternal(\"%s\", idx=%d add=%d must=%d%s) -> %p f=%d\n",
          il2cpp_str_ascii(scene, name, sizeof name), index, additive, must,
          orig_must != must ? " forced" : "", r, g_render_frame);
  fsync(2);
  return r;
}
static int (*ls_async_done_orig)(void *, void *);
static int ls_async_done_hook(void *self, void *method) {
  int r = ls_async_done_orig(self, method);
  static unsigned n;
  if (n++ < 80)
    fprintf(stderr, "[LOADSPY] AsyncOperation.get_isDone(%p) -> %d f=%d\n", self, r, g_render_frame);
  return r;
}
static void (*ls_async_allow_orig)(void *, int, void *);
static void ls_async_allow_hook(void *self, int value, void *method) {
  fprintf(stderr, "[LOADSPY] AsyncOperation.set_allowSceneActivation(%p,%d) f=%d\n",
          self, value, g_render_frame);
  fsync(2);
  ls_async_allow_orig(self, value, method);
}
static void (*ls_async_invoke_orig)(void *, void *);
static void ls_async_invoke_hook(void *self, void *method) {
  fprintf(stderr, "[LOADSPY] AsyncOperation.InvokeCompletionEvent(%p) f=%d\n", self, g_render_frame);
  fsync(2);
  ls_async_invoke_orig(self, method);
}
static void (*ls_link_orig)(void *, void *, void *);
static void ls_link_hook(void *self, void *async, void *method) {
  fprintf(stderr, "[LOADSPY] LoadingGUI.LinkAsyncProcess(async=%p) f=%d\n", async, g_render_frame);
  fsync(2);
  ls_link_orig(self, async, method);
}
static void (*ls_lupdate_orig)(void *, void *);
static void ls_lupdate_hook(void *self, void *method) {
  static unsigned n;
  if (n++ < 80) {
    void *async = self ? *(void **)((char *)self + 0x38) : NULL;
    fprintf(stderr, "[LOADSPY] LoadingGUI.Update(self=%p async=%p) f=%d\n", self, async, g_render_frame);
    fsync(2);
  }
  ls_lupdate_orig(self, method);
}
static void loadspy_install(uintptr_t base) {
  struct { uintptr_t rva; void *hook; void **orig; const char *nm; } T[] = {
    {0x284F1C0, (void *)ls_scene_async_hook,   (void **)&ls_scene_async_orig,   "SceneManager.LoadSceneAsync"},
    {0x284ED7C, (void *)ls_scene_internal_hook,(void **)&ls_scene_internal_orig,"SceneManager.LoadSceneAsyncInternal"},
    {0x258D578, (void *)ls_async_done_hook,    (void **)&ls_async_done_orig,    "AsyncOperation.get_isDone"},
    {0x258D650, (void *)ls_async_allow_hook,   (void **)&ls_async_allow_orig,   "AsyncOperation.set_allowSceneActivation"},
    {0x258D744, (void *)ls_async_invoke_hook,  (void **)&ls_async_invoke_orig,  "AsyncOperation.InvokeCompletionEvent"},
    {0x31B36AC, (void *)ls_link_hook,          (void **)&ls_link_orig,          "LoadingGUI.LinkAsyncProcess"},
    {0x31B3710, (void *)ls_lupdate_hook,       (void **)&ls_lupdate_orig,       "LoadingGUI.Update"},
  };
  for (unsigned i = 0; i < sizeof T / sizeof T[0]; i++) {
    void *tr = mk_tramp(base + T[i].rva, T[i].nm);
  ...
  gk_async_tools_init(base);
  if (ls_async_done_orig) gk_async_isdone_direct = ls_async_done_orig;
  if (ls_async_allow_orig) gk_async_allow_direct = ls_async_allow_orig;
  fprintf(stderr, "[LOADSPY] %u hooks instalados\n", (unsigned)(sizeof T / sizeof T[0]));
}

/* GK_EVENTSPY: caminho managed do evento SceneManager.sceneLoaded.
 * Se a SceneManager nativa ja tem cenas carregadas mas Preloader.OnSceneLoaded
 * nao aparece, este spy separa falha de dispatch nativo de falha no delegate C#. */
#define GK_RVA_SCENEMGR_ADD_SCENELOADED      0x284EF44
#define GK_RVA_SCENEMGR_INTERNAL_SCENELOADED 0x284F288
#define GK_RVA_PRELOADER_ONSCENELOADED       0x32E91A8

static void gk_log_delegate(const char *tag, void *d) {
  maps_snapshot();
  if (!d || !addr_readable((uintptr_t)d + 0x48)) {
    fprintf(stderr, "[GK_EVENTSPY] %s delegate=%p unreadable\n", tag, d);
    return;
  }
  uintptr_t method_ptr = *(uintptr_t *)((char *)d + 0x10);
  uintptr_t invoke_impl = *(uintptr_t *)((char *)d + 0x18);
  void *target = *(void **)((char *)d + 0x20);
  uintptr_t method = *(uintptr_t *)((char *)d + 0x28);
  uintptr_t method_code = *(uintptr_t *)((char *)d + 0x40);
  fprintf(stderr,
          "[GK_EVENTSPY] %s delegate=%p method_ptr=%lx invoke=%lx target=%p method=%lx code=%lx\n",
          tag, d, (unsigned long)method_ptr, (unsigned long)invoke_impl, target,
          (unsigned long)method, (unsigned long)method_code);
}

static void (*gk_add_scene_loaded_orig)(void *, void *, void *);
static void gk_add_scene_loaded_hook(void *self, void *value, void *method) {
  gk_log_delegate("add_sceneLoaded.before", value);
  gk_add_scene_loaded_orig(self, value, method);
  fprintf(stderr, "[GK_EVENTSPY] add_sceneLoaded.after f=%d\n", g_render_frame);
  fsync(2);
}

static void (*gk_internal_scene_loaded_orig)(void *, int, int, void *);
static void gk_internal_scene_loaded_hook(void *self, int scene, int mode, void *method) {
  fprintf(stderr, "[GK_EVENTSPY] Internal_SceneLoaded(scene=%d mode=%d) f=%d\n",
          scene, mode, g_render_frame);
  fsync(2);
  gk_internal_scene_loaded_orig(self, scene, mode, method);
}

static void (*gk_preloader_scene_loaded_orig)(void *, int, int, void *);
static void gk_preloader_scene_loaded_hook(void *self, int scene, int mode, void *method) {
  fprintf(stderr, "[GK_EVENTSPY] Preloader.OnSceneLoaded(self=%p scene=%d mode=%d) f=%d\n",
          self, scene, mode, g_render_frame);
  fsync(2);
  gk_preloader_scene_loaded_orig(self, scene, mode, method);
  fprintf(stderr, "[GK_EVENTSPY] Preloader.OnSceneLoaded.done f=%d\n", g_render_frame);
  fsync(2);
}

static void gk_eventspy_install(uintptr_t base) {
  struct { uintptr_t rva; void *hook; void **orig; const char *nm; } T[] = {
    {GK_RVA_SCENEMGR_ADD_SCENELOADED,      (void *)gk_add_scene_loaded_hook,
      (void **)&gk_add_scene_loaded_orig,      "SceneManager.add_sceneLoaded"},
    {GK_RVA_SCENEMGR_INTERNAL_SCENELOADED, (void *)gk_internal_scene_loaded_hook,
      (void **)&gk_internal_scene_loaded_orig, "SceneManager.Internal_SceneLoaded"},
    {GK_RVA_PRELOADER_ONSCENELOADED,       (void *)gk_preloader_scene_loaded_hook,
      (void **)&gk_preloader_scene_loaded_orig, "Preloader.OnSceneLoaded"},
  };
  unsigned ok = 0;
  for (unsigned i = 0; i < sizeof T / sizeof T[0]; i++) {
    void *tr = mk_tramp(base + T[i].rva, T[i].nm);
    if (!tr) {
      fprintf(stderr, "[GK_EVENTSPY] tramp %s falhou\n", T[i].nm);
      continue;
    }
    *T[i].orig = tr;
    hook_arm64(base + T[i].rva, (uintptr_t)T[i].hook);
  ...
  fprintf(stderr, "[GK_EVENTSPY] %u/%u hooks instalados\n", ok, (unsigned)(sizeof T / sizeof T[0]));
}

/* GK_JOBSPY: offsets reais Unity 2018.2.2f1 do Graveyard Keeper.
 * Equivalente funcional ao WaitForJobGroup do Terraria, mas recalculado para GK. */
...
static int gk_unity_read_u32(uintptr_t rva) {
  ...
  return *(volatile uint32_t *)(g_unity_base + rva);
}

static long (*gk_waitjob_orig)(int);
static long gk_waitjob_hook(int target) {
  static unsigned n;
  int before = gk_unity_read_u32(GK_UN_WAITFORJOBGROUP_COUNT);
  if (n++ < 64 || before < target) {
    fprintf(stderr, "[GK_JOBSPY] WaitForJobGroup(target=%d count=%d) f=%d\n",
            target, before, g_render_frame);
    fsync(2);
  }
  long r = gk_waitjob_orig(target);
  int after = gk_unity_read_u32(GK_UN_WAITFORJOBGROUP_COUNT);
  if (n < 64 || after < target) {
    fprintf(stderr, "[GK_JOBSPY] WaitForJobGroup.done(target=%d count=%d ret=%ld) f=%d\n",
            target, after, r, g_render_frame);
    fsync(2);
  }
  return r;
}

static void gk_patch_unity_insn(uintptr_t rva, uint32_t insn, const char *tag) {
  if (!g_unity_base) return;
  uintptr_t a = g_unity_base + rva;
  long pgsz = sysconf(_SC_PAGESIZE);
  ...
  *(uint32_t *)a = insn;
  mprotect(pa, pgsz * 2, PROT_READ | PROT_EXEC);
  __builtin___clear_cache((char *)pa, (char *)pa + pgsz * 2);
  fprintf(stderr, "[%s] libunity+0x%lx -> %08x\n", tag, (unsigned long)rva, insn);
  fsync(2);
}

static void gk_job_tools_install(uintptr_t base) {
  if (getenv("GK_JOBSPY")) {
    uintptr_t target = base + GK_UN_WAITFORJOBGROUP;
    void *tr = mk_tramp(target, "GK.WaitForJobGroup");
    if (!tr) {
      fprintf(stderr, "[GK_JOBSPY] tramp WaitForJobGroup falhou\n");
    } else {
      gk_waitjob_orig = tr;
      long pgsz = sysconf(_SC_PAGESIZE);
      void *page = (void *)(target & ~((uintptr_t)pgsz - 1));
      mprotect(page, pgsz * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
      hook_arm64(target, (uintptr_t)gk_waitjob_hook);
      mprotect(page, pgsz * 2, PROT_READ | PROT_EXEC);
      __builtin___clear_cache((char *)page, (char *)page + pgsz * 2);
      fprintf(stderr, "[GK_JOBSPY] hook libunity+0x%x instalado\n", GK_UN_WAITFORJOBGROUP);
    }
  }
...
/* GK_NATIVELOADSPY / GK_SCENESPY: RVAs reais do libunity.so do Graveyard Keeper.
 * Os offsets 0x873xxx herdados de outro Unity caem em codigo SIMD neste jogo. */
#define GK_UN_SCENECOUNT 0x0e0624
#define GK_UN_GETACTIVE  0x0e0628
#define GK_UN_GETSCENEAT 0x0e064c
#define GK_UN_LOADASYNC  0x0e068c

static void *(*gk_un_loadasync_orig)(void *, int, int, int);
static void *gk_un_loadasync_hook(void *scene, int index, int additive, int must) {
  char name[96];
  void *r = gk_un_loadasync_orig(scene, index, additive, must);
  g_gk_async_op = r;
  fprintf(stderr, "[GK_NATIVELOAD] LoadSceneAsyncNameIndexInternal(\"%s\", idx=%d add=%d must=%d) -> %p f=%d\n",
          il2cpp_str_ascii(scene, name, sizeof name), index, additive, must, r, g_render_frame);
  fsync(2);
  return r;
}

static void gk_nativespy_install(uintptr_t base) {
  uintptr_t target = base + GK_UN_LOADASYNC;
  void *tr = mk_tramp(target, "GK.LoadSceneAsyncNameIndexInternal");
  if (!tr) {
    fprintf(stderr, "[GK_NATIVELOAD] tramp LoadSceneAsyncNameIndexInternal falhou\n");
    return;
  }
  gk_un_loadasync_orig = tr;
  long pgsz = sysconf(_SC_PAGESIZE);
  void *page = (void *)(target & ~((uintptr_t)pgsz - 1));
  mprotect(page, pgsz * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
  hook_arm64(target, (uintptr_t)gk_un_loadasync_hook);
  mprotect(page, pgsz * 2, PROT_READ | PROT_EXEC);
  __builtin___clear_cache((char *)page, (char *)page + pgsz * 2);
  fprintf(stderr, "[GK_NATIVELOAD] hook libunity+0x%x instalado\n", GK_UN_LOADASYNC);
}

static void gk_asyncvt_install(uintptr_t base) {
  struct { uintptr_t rva; void *hook; void **orig; const char *nm; } T[] = {
    {0x619438, (void *)gk_async_vt11_hook, (void **)&gk_async_vt11_orig, "GK.AsyncVT.vt11"},
    {0x619c20, (void *)gk_async_vt12_hook, (void **)&gk_async_vt12_orig, "GK.AsyncVT.vt12"},
    {0x619cf0, (void *)gk_async_vt13_hook, (void **)&gk_async_vt13_orig, "GK.AsyncVT.vt13"},
  };
  unsigned ok = 0;
  for (unsigned i = 0; i < sizeof T / sizeof T[0]; i++) {
    ...
    if (!tr) {
      fprintf(stderr, "[GK_ASYNCVT] tramp %s falhou\n", T[i].nm);
      continue;
    }
    *T[i].orig = tr;
    long pgsz = sysconf(_SC_PAGESIZE);
  ...
  fprintf(stderr, "[GK_ASYNCVT] %u/%u hooks instalados\n", ok, (unsigned)(sizeof T / sizeof T[0]));
}
...
static void gk_vt11spy_install(uintptr_t base) {
  ...
  for (unsigned i = 0; i < sizeof T / sizeof T[0]; i++) {
    ...
    void *tr = mk_tramp(target, T[i].nm);
    if (!tr) {
      fprintf(stderr, "[GK_VT11SPY] tramp %s falhou\n", T[i].nm);
      continue;
    }
    *T[i].orig = tr;
    long pgsz = sysconf(_SC_PAGESIZE);
...
static void gk_scene_icall_dump(const char *tag) {
  if (!g_unity_base) return;
  maps_snapshot();
  int (*scene_count)(void) = (int (*)(void))(g_unity_base + GK_UN_SCENECOUNT);
  void (*get_active)(int *) = (void (*)(int *))(g_unity_base + GK_UN_GETACTIVE);
  void (*get_scene_at)(int, int *) = (void (*)(int, int *))(g_unity_base + GK_UN_GETSCENEAT);
  if (!addr_readable((uintptr_t)scene_count) ||
      !addr_readable((uintptr_t)get_active) ||
      !addr_readable((uintptr_t)get_scene_at)) {
    fprintf(stderr, "[GK_SCENESPY:%s] icalls ilegiveis\n", tag);
    fsync(2);
    return;
  }
  int count = scene_count();
  int active = 0;
  get_active(&active);
  fprintf(stderr, "[GK_SCENESPY:%s] f=%d count=%d active=%d\n", tag, g_render_frame, count, active);
  if (!getenv("GK_SCENESPY_DETAIL")) {
    fsync(2);
    return;
  }
  for (int i = 0; i < count && i < 8; i++) {
    int h = 0;
    get_scene_at(i, &h);
    fprintf(stderr, "[GK_SCENESPY:%s]  scene[%d].handle=%d%s\n",
            tag, i, h, h == active ? " (active)" : "");
  }
  fsync(2);
}
...
void *my_streamingAssetsPath(void) {
  if (!g_sa_string && g_il2cpp_base) {
    ...
    const char *p = getenv("GK_SAPATH"); if (!p) p = getenv("CUP_SAPATH");
    if (!p) p = ASSET_BASE_M;
    g_sa_string = isn(p);
...
/* PC_TLSLOG: instrumenta o TLS do Unity p/ achar o pthread_key do thread-index do job-system.
   Loga KEYCREATE (raro), todo SET (1×/thread/key) e o 1º GET de cada key NA MAIN.
   Hipótese: cada Job.Worker faz setspecific(key_X, &jobinfo) no startup; a main NÃO →
   getspecific(key_X) na main retorna NULL → self-execute usa índice inválido → deque órfã. */
static int g_tlslog = -1;
static int tlslog_on(void) { if (g_tlslog < 0) g_tlslog = getenv("PC_TLSLOG") ? 1 : 0; return g_tlslog; }
static const char *tls_symbuf(uintptr_t ra) {
  static __thread char b[48];
  if (g_unity_base && ra >= g_unity_base && ra < g_unity_base + 0x2000000) snprintf(b, sizeof b, "libunity+0x%lx", ra - g_unity_base);
  else if (g_il2cpp_base && ra >= g_il2cpp_base && ra < g_il2cpp_base + 0x3000000) snprintf(b, sizeof b, "libil2cpp+0x%lx", ra - g_il2cpp_base);
  else snprintf(b, sizeof b, "0x%lx", ra);
  return b;
}
static const char *tls_comm(void) {
  static __thread char c[20]; c[0] = 0;
  FILE *f = fopen("/proc/self/comm", "r");
  if (f) { if (fgets(c, sizeof c, f)) { char *nl = strchr(c, '\n'); if (nl) *nl = 0; } fclose(f); }
  return c;
}
static int sh_key_create(unsigned *k, void (*d)(void *)) { (void)d; pthread_mutex_lock(&g_slot_mtx);
  int n = g_slot_next++; pthread_mutex_unlock(&g_slot_mtx); if (n >= NSLOT) return 11; *k = (unsigned)n;
  if (tlslog_on()) { fprintf(stderr, "[TLS] KEYCREATE key=%d caller=%s tid=%d(%s)\n",
                             n, tls_symbuf((uintptr_t)__builtin_return_address(0)),
                             (int)syscall(SYS_gettid), tls_comm()); fflush(stderr); }
  return 0; }
static int sh_key_delete(unsigned k) { (void)k; return 0; }
static int g_mainworker = -1;
static int mainworker_on(void) { if (g_mainworker < 0) g_mainworker = getenv("PC_MAINWORKER") ? 1 : 0; return g_mainworker; }
static void *sh_getspecific(unsigned k) { if ((int)k <= 0 || (int)k >= NSLOT) return NULL;
  void *v = tls_slots()[(int)k];
  /* PC_MAINWORKER: o job-system marca a thread-info da MAIN com [24]=1 (thread EXTERNA),
     enquanto os workers (slots da tabela) têm [24]=0. A hipótese: schedule a partir de thread
     externa manda o trabalho p/ deque órfã. Limpar o flag p/ a main parecer worker. */
  if (k == 14 && v && mainworker_on() && (int)syscall(SYS_gettid) == getpid())
    ((unsigned char *)v)[24] = 0;
  if (tlslog_on()) {
    int tid = (int)syscall(SYS_gettid);
    /* dedupe por (tid,key) p/ ver o que CADA thread lê em cada key (1ª vez) */
    static struct { int tid; unsigned key; } seen[256]; static int ns;
    int dup = 0; for (int i = 0; i < ns; i++) if (seen[i].tid == tid && seen[i].key == k) { dup = 1; break; }
    if (!dup) { if (ns < 256) { seen[ns].tid = tid; seen[ns].key = k; ns++; }
      fprintf(stderr, "[TLS] GET tid=%d(%s) key=%u -> %p caller=%s%s\n",
              tid, tls_comm(), k, v, tls_symbuf((uintptr_t)__builtin_return_address(0)),
              tid == getpid() ? " <<MAIN" : ""); fflush(stderr); } }
  return v; }
static int sh_setspecific(unsigned k, const void *v) { if ((int)k <= 0 || (int)k >= NSLOT) return 22;
  tls_slots()[(int)k] = (void *)v;
  if (tlslog_on()) { static int cnt = 0; if (cnt++ < 4000)
    fprintf(stderr, "[TLS] SET tid=%d(%s) key=%u val=%p caller=%s\n",
            (int)syscall(SYS_gettid), tls_comm(), k, v,
            tls_symbuf((uintptr_t)__builtin_return_address(0))); fflush(stderr); }
  return 0; }
...
static void *choreo_driver_thread(void *arg) {
  ...
  /* il2cpp_domain_get/il2cpp_thread_attach são EXPORTS — resolver por NOME (offsets
     variam por build; offsets hardcoded de outro Unity batem em lixo -> blr crash). */
  if (g_il2cpp_base && g_m_il2cpp) {
    so_module *cur = so_save(); so_use(g_m_il2cpp);
    ...
    } else fprintf(stderr, "[CHOREO] il2cpp_domain_get/thread_attach NAO resolvidos (dom=%p att=%p)\n",
                   (void *)dom_get, (void *)thr_attach);
  } else fprintf(stderr, "[CHOREO] g_il2cpp_base/g_m_il2cpp=0 (sem attach)\n");
  int started = 0;
  for (;;) {
...
/* ===== resolvedor il2cpp endereço->método (diagnóstico): itera todos os métodos via
 * API EXPORTADA, acha o methodPointer mais próximo <= target -> revela QUAL método C#
 * contém o endereço (ex.: onde a main trava no boot). Pesado (~100k métodos), one-shot. */
static void pc_resolve_il2cpp_addr(uintptr_t target) {
  if (!g_m_il2cpp) { fprintf(stderr, "[RESOLVE] sem il2cpp\n"); return; }
  so_module *cur = so_save(); so_use(g_m_il2cpp);
  void *(*dom_get)(void) = (void*)so_find_addr_safe("il2cpp_domain_get");
  const void**(*dom_asms)(void*,size_t*) = (void*)so_find_addr_safe("il2cpp_domain_get_assemblies");
  const void*(*asm_img)(const void*) = (void*)so_find_addr_safe("il2cpp_assembly_get_image");
  size_t(*img_ccount)(const void*) = (void*)so_find_addr_safe("il2cpp_image_get_class_count");
  const void*(*img_class)(const void*,size_t) = (void*)so_find_addr_safe("il2cpp_image_get_class");
  const void*(*cls_methods)(const void*,void**) = (void*)so_find_addr_safe("il2cpp_class_get_methods");
  const char*(*meth_name)(const void*) = (void*)so_find_addr_safe("il2cpp_method_get_name");
  const char*(*cls_name)(const void*) = (void*)so_find_addr_safe("il2cpp_class_get_name");
  const char*(*cls_ns)(const void*) = (void*)so_find_addr_safe("il2cpp_class_get_namespace");
  so_use(cur); free(cur);
  if (!dom_get||!dom_asms||!asm_img||!img_ccount||!img_class||!cls_methods||!meth_name) {
    fprintf(stderr, "[RESOLVE] API il2cpp incompleta\n"); return; }
  void *dom = dom_get(); size_t na=0; const void**asms = dom_asms(dom,&na);
  uintptr_t best=0; const void *bestm=NULL, *bestc=NULL;
  for (size_t i=0;i<na;i++){ const void*img=asm_img(asms[i]); if(!img) continue;
    size_t nc=img_ccount(img);
    for (size_t c=0;c<nc;c++){ const void*kls=img_class(img,c); if(!kls) continue;
      void*it=NULL; const void*m;
      while ((m=cls_methods(kls,&it))){
        uintptr_t mp = *(uintptr_t*)m;  /* MethodInfo.methodPointer @ off 0 */
        if (mp && mp<=target && mp>best){ best=mp; bestm=m; bestc=kls; }
      }
    }
  }
  if (bestm) fprintf(stderr, "[RESOLVE] 0x%lx = %s%s%s::%s + 0x%lx\n", (unsigned long)target,
      bestc&&cls_ns(bestc)?cls_ns(bestc):"", (bestc&&cls_ns(bestc)&&cls_ns(bestc)[0])?".":"",
      bestc?cls_name(bestc):"?", meth_name(bestm), (unsigned long)(target-best));
  else fprintf(stderr, "[RESOLVE] 0x%lx: nenhum método <= addr\n", (unsigned long)target);
  fsync(2);
}
/* SIGUSR2: resolve o LR da thread sinalizada (= método C# onde ela está). Manda na main
 * bloqueada p/ saber QUAL método trava. */
static void resolve_handler(int sig, siginfo_t *si, void *uc_) {
  (void)sig;(void)si;(void)uc_;
  /* resolve os offsets il2cpp em PC_RESOLVE_OFF (csv hex) -> g_il2cpp_base+off -> método C#.
     mapeamento é ESTÁTICO (methodPointer), não precisa do endereço estar ativo. */
  const char *e = getenv("PC_RESOLVE_OFF");
  if (!e || !*e) { fprintf(stderr, "[RESOLVE] PC_RESOLVE_OFF vazio\n"); return; }
  char buf[256]; strncpy(buf, e, sizeof buf-1); buf[sizeof buf-1]=0;
  for (char *t=strtok(buf,","); t; t=strtok(NULL,",")) {
    uintptr_t off=(uintptr_t)strtoull(t,NULL,16);
    pc_resolve_il2cpp_addr(g_il2cpp_base + off);
  }
}
...
int main(int argc, char **argv) {
  ...
  g_pc_skipjobwait = getenv("PC_SKIPJOBWAIT") ? 1 : 0;
  g_kickworkers = getenv("PC_KICKWORKERS") ? atoi(getenv("PC_KICKWORKERS")) : 0;
  if (g_kickworkers) {
    fprintf(stderr, "[KICKWORKERS] mode=%d (Release worker sems p/ dispatch async)\n", g_kickworkers);
    pthread_t kt; pthread_create(&kt, NULL, kick_worker_thread, NULL); pthread_detach(kt);
  }
  ...
  if (!getenv("GK_NOLOGFILE") && !getenv("CUP_NOLOGFILE")) {
  ...
  { struct sigaction sr; memset(&sr, 0, sizeof sr); sr.sa_sigaction = resolve_handler;
    sr.sa_flags = SA_SIGINFO | SA_ONSTACK | SA_RESTART; sigaction(SIGUSR2, &sr, 0); }

  fprintf(stderr, "=== Graveyard Keeper Unity 2018.2 IL2CPP (arm64 GLES2) so-loader ===\n");
  ...
  if (so_load("lib/libunity.so", heap, hs) < 0) { fprintf(stderr, "so_load libunity FALHOU\n"); return 1; }
  ...
  if (getenv("GK_TERRARIA_STORAGEPATCH")) {
  ...
  if (getenv("GK_DLLOG") || getenv("CUP_DLLOG")) g_dllog = 1;
  ...
  /* O PSPY herdado usa RVAs 0x873xxx que no Graveyard Keeper caem em codigo
     SIMD, nao no PreloadManager. Mantido atras de escape explicito. */
  if ((getenv("CUP_PSPY") || getenv("CUP_GCEVERY")) && getenv("GK_ALLOW_OLD_PSPY")) {
    extern void so_make_text_writable(void), so_make_text_executable(void);
    so_make_text_writable();
  ...
  } else if (getenv("CUP_PSPY") || getenv("CUP_GCEVERY")) {
    fprintf(stderr, "[PSPY] desativado no GK: RVAs antigos invalidos; use GK_SCENESPY/GK_NATIVELOADSPY\n");
  }
  ...
  jni_shim_set_package("com.tinybuild.graveyardkeeper", 0);
  ...
  if (i2heap != MAP_FAILED && so_load("lib/libil2cpp.so", i2heap, i2s) >= 0) {
  ...
  patch_got("__FD_SET_chk", (void *)my_FD_SET_chk);
    ...
    if (getenv("GK_LOADSPY")) loadspy_install(g_il2cpp_base);
    if (getenv("GK_ASYNCPOLL") || getenv("GK_FORCE_ALLOW") || getenv("GK_ASYNCDUMP"))
      gk_async_tools_init(g_il2cpp_base);
  ...
  if (cup_use_kmsdrm()) {
    ...
    /* fbdev EGL multiplexer: só a thread de render usa a window fb0 REAL; workers do
       Unity (native gfx jobs) recebem pbuffer -> não travam o frame-builder do Mali. */
    g_egllog = getenv("CUP_EGLLOG") ? 1 : 0;
    /* força vsync off no swap (default) -> a present thread não segura o frame-builder
       lock esperando vsync -> evita o deadlock com a render thread (glClear). */
    if (!getenv("CUP_NOVSYNCPATCH")) {
      int n = so_patch_got("eglSwapInterval", (uintptr_t)mux_SwapInterval);
      fprintf(stderr, "[EGLMUX] eglSwapInterval -> forçado %s (%d slots)\n", getenv("CUP_VSYNC") ? getenv("CUP_VSYNC") : "0", n);
    }
    if (getenv("CUP_EGLMUX")) {   /* opt-in: adianta frames mas native-gfx-jobs ainda quebra (preto) */
      int np = 0;
      np += so_patch_got("eglCreateWindowSurface", (uintptr_t)mux_CreateWindowSurface);
      np += so_patch_got("eglMakeCurrent", (uintptr_t)mux_MakeCurrent);
      np += so_patch_got("eglSwapBuffers", (uintptr_t)mux_SwapBuffers);
      fprintf(stderr, "[EGLMUX] %d slots egl* da libunity -> multiplexer fbdev\n", np);
    }
  }
  ...
  if (getenv("CUP_PSPY") && getenv("GK_ALLOW_OLD_PSPY")) {
    pthread_t st; pthread_create(&st, NULL, preload_spy_thread, NULL);
    pthread_detach(st);
  } else if (getenv("CUP_PSPY")) {
    fprintf(stderr, "[PSPY] thread ignorada no GK: use GK_ASYNCDUMP/GK_NATIVELOADSPY\n");
  }
  if (getenv("CUP_PRELOAD_BG") && getenv("GK_ALLOW_OLD_PSPY")) {
    pthread_t bt; pthread_create(&bt, NULL, preload_bg_thread, NULL);
    pthread_detach(bt);
  } else if (getenv("CUP_PRELOAD_BG")) {
    fprintf(stderr, "[PRELOAD_BG] ignorado no GK: RVAs antigos invalidos\n");
  }
  void *render = jni_find_native("nativeRender");
  fprintf(stderr, "[F2] nativeRender=%p -> loop\n", render);
  const char *frames_s = getenv("GK_FRAMES") ? getenv("GK_FRAMES") : getenv("CUP_FRAMES");
  int max_f = frames_s ? atoi(frames_s) : 600;
  ...
  int old_preload_ok = getenv("GK_ALLOW_OLD_PSPY") ? 1 : 0;
  if (!old_preload_ok && (getenv("CUP_DRAINPRELOAD") || getenv("CUP_DRAINWAIT")))
    fprintf(stderr, "[DRAINPRELOAD] ignorado no GK: derive os offsets nativos antes\n");
  int drainN = (old_preload_ok && getenv("CUP_DRAINPRELOAD")) ? atoi(getenv("CUP_DRAINPRELOAD")) : 0;
  ...
  int drainWait = (old_preload_ok && getenv("CUP_DRAINWAIT")) ? 1 : 0;
  ...
  /* PC_INLINETASK (legado, default OFF no GK): trampolim no topo do loop de espera
     do per-object task/fence (libunity+0x651530) que chama pc_inline_task(x19=obj) p/
     fingir a conclusão (node[0]=1) e então executa a instrução original (ldr x8,[x19,#88])
     + volta a 0x651534. Destrava o frame 3 sem depender dos workers. */
  if ((getenv("PC_WREG") || getenv("PC_MAINREG")) && g_unity_base) {
    if (unity_install_hook4(0x63a260, (void *)my_wreg, (void **)&g_orig_wreg))
      fprintf(stderr, "[PC_WREG] RegisterWorkerThread 0x63a260 hookado\n");
    else fprintf(stderr, "[PC_WREG] FALHA hook 0x63a260\n");
    fsync(2);
  }
  if (getenv("PC_PADHOOK") && g_unity_base) {
    if (unity_install_hook4(0x63f308, (void *)my_mount_hook, (void **)&g_orig_mount))
      fprintf(stderr, "[PC_PADHOOK] mount datapack 0x63f308 hookado\n");
  ...
  if (getenv("PC_QREDIR") && g_unity_base) {
    ...
    else fprintf(stderr, "[PC_QREDIR] FALHA ao hookar d6e410\n");
    fsync(2);
  }
  if (getenv("PC_INLINETASK") && g_unity_base) {
    extern void pc_inline_task(void *);
    long pgsz = sysconf(_SC_PAGESIZE);
    uintptr_t patch = g_unity_base + 0x651530;
    uintptr_t hint = (g_unity_base + 0x2000000) & ~((uintptr_t)pgsz - 1);
    void *tp = mmap((void *)hint, pgsz, PROT_READ | PROT_WRITE | PROT_EXEC,
                    MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (tp == MAP_FAILED) tp = mmap(NULL, pgsz, PROT_READ | PROT_WRITE | PROT_EXEC,
    ...
    if (tp != MAP_FAILED && d > -0x7000000L && d < 0x7000000L) {
      ...
      t[3] = 0xD63F0200u;          /* blr x16             */
      t[4] = 0xF84107FEu;          /* ldr x30,[sp],#16    */
      t[5] = 0xF9402E68u;          /* ldr x8,[x19,#88] (instr original @0x651530) */
      t[6] = 0x58000090u;          /* ldr x16,[pc+0x10] -> dst@T+0x28 */
      t[7] = 0xD61F0200u;          /* br x16              */
      *(uint64_t *)((char *)tp + 0x20) = (uint64_t)(uintptr_t)pc_inline_task;
      *(uint64_t *)((char *)tp + 0x28) = (uint64_t)(g_unity_base + 0x651534);
      __builtin___clear_cache((char *)tp, (char *)tp + pgsz);
      void *pp = (void *)(patch & ~((uintptr_t)pgsz - 1));
      mprotect(pp, pgsz * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
      *(uint32_t *)patch = 0x14000000u | (uint32_t)(((d) >> 2) & 0x03FFFFFF);
      mprotect(pp, pgsz * 2, PROT_READ | PROT_EXEC);
      __builtin___clear_cache((char *)pp, (char *)pp + pgsz * 2);
      fprintf(stderr, "[PC_INLINETASK] trampolim @%p (d=0x%lx) patch 0x651530->b\n", tp, d);
    } else {
      fprintf(stderr, "[PC_INLINETASK] FALHOU mmap/alcance (tp=%p d=0x%lx)\n", tp, d);
    }
  }
  ...
  /* GK_SCENESPY: dump via icalls nativas reais do GK. CUP_SCENESPY/CUP_SETACTIVE
     dependem de layout antigo e ficam fora deste port. */
  int gk_scenespy = getenv("GK_SCENESPY") ? 1 : 0;
  int gk_asyncdump = getenv("GK_ASYNCDUMP") ? 1 : 0;
  int gk_asyncpoll = getenv("GK_ASYNCPOLL") ? 1 : 0;
  int gk_force_allow = getenv("GK_FORCE_ALLOW") ? 1 : 0;
  int scenespy = getenv("CUP_SCENESPY") ? 1 : 0;
  int setactive = getenv("CUP_SETACTIVE") ? 1 : 0;
  if (scenespy || setactive)
  ...
  for (int f = 0; render && (max_f <= 0 || f < max_f); f++) {
    ...
    if (gk_scenespy && f > 0 && f % 300 == 0) gk_scene_icall_dump("tick");
    ...
    { /* log de hits do SCENESKIP + MASKGUARD + NULLGUARD */
      ...
      if (g_sceneskip_hits != sg_last) {
        ...
        if (gk_scenespy) gk_scene_icall_dump("skip");
      }
    ...
    if (gk_vt11count_enabled) gk_vt11count_report("frame", 0);
    {
      ...
      if (gk_asyncdump && g_gk_async_op &&
          (gk_asyncdump_last_op != (void *)g_gk_async_op || (f % 60) == 0)) {
        gk_async_dump("frame");
        gk_asyncdump_last_op = (void *)g_gk_async_op;
      }
    }
