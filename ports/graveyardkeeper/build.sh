#!/bin/bash
# CI: set CC and SR env vars before invoking to override local toolchain.
# build arm64 do Graveyard Keeper (Unity 2018.2.2f1 IL2CPP) so-loader — toolchain NextOS Amlogic-old.
set -e
TC=~/NextOS-Elite-Edition/build.NextOS-Retro-Elite-Edition-Amlogic-old.aarch64-4/toolchain
CC=${CC:-$TC/bin/aarch64-libreelec-linux-gnu-gcc}
SR=${SR:-$TC/aarch64-libreelec-linux-gnu/sysroot}
cd "$(dirname "$0")"
[ -x "$CC" ] || { echo "toolchain não encontrado: $CC"; exit 1; }
SRCS=$(ls src/*.c)
$CC --sysroot="$SR" -D_GNU_SOURCE -I src -I "$SR/usr/include" -O2 -fPIC -fno-omit-frame-pointer -rdynamic \
    -o graveyardkeeper $SRCS \
    -lSDL2 -ldl -lm -lpthread -lgcc_s
echo "BUILD OK -> $(file graveyardkeeper | cut -d, -f1-3)"
