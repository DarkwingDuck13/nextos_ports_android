...
static const int id_playbackrate_tag = 9;
static const int id_seek_tag = 10;
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].obj_ptr == obj_ptr) {
      ...
      if (iid == sl_IID_PLAY) {
        ...
        *pInterface = &p->bq_ptr;
      } else if (iid == sl_IID_EFFECTSEND || iid == sl_IID_ANDROIDCONFIGURATION ||
                 iid == sl_IID_PLAYBACKRATE || iid == sl_IID_SEEK) {
