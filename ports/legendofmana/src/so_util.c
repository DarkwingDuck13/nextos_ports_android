...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  for (int i = 0; i < elf_hdr->e_phnum; i++) {
    if (prog_hdr[i].p_type != PT_LOAD)
      continue;
    const size_t end = prog_hdr[i].p_vaddr + prog_hdr[i].p_memsz;
    if (end > max_end)
      max_end = end;
  ...
  text_size = prog_hdr[exec_segno].p_memsz;
  text_base = (void *)((uintptr_t)load_base + prog_hdr[exec_segno].p_vaddr);
    ...
    if (strcmp(sh_name, ".rela.dyn") == 0 ||
      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_JUMP_SLOT:
          ...
          break;
        default:
          debugPrintf("so_relocate: ignored relocation type %x off=0x%lx\n",
